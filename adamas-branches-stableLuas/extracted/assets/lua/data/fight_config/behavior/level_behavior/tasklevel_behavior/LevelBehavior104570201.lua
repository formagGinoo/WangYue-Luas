LevelBehavior104570201 = BaseClass("LevelBehavior104570201",LevelBehaviorBase)
--Boss战贝露贝特
function LevelBehavior104570201:__init(fight)
	self.fight = fight
end

function LevelBehavior104570201.GetGenerates()
	local generates = {709200100}
	return generates
end

function LevelBehavior104570201.GetStorys()
	local storys = {101070102,101071001}
	return storys
end

----程序黑幕
--function LevelBehavior104570201.NeedBlackCurtain()
--return true
--end

function LevelBehavior104570201:Init()
	...
	self.dialogList =
	{
		...
		[5] = {Id = 104080301,state = self.dialogStateEnum.NotPlaying}, --衔接切天空timeline
		[6] = {Id = 104080401,state = self.dialogStateEnum.NotPlaying}, --播放战斗中旁白
	}
...
function LevelBehavior104570201:Update()
	...
	if self.missionState == 0.2 then
		...
		--世界等级偏移计算
		local npcTag = BehaviorFunctions.GetTagByEntityId(709200100)
		...
		self.beilubeite = BehaviorFunctions.CreateEntity(709200100,nil,pos.x,pos.y,pos.z,nil,nil,nil,self.levelId,monsterLevel)
	...
	if self.missionState == 10  then
		...
		--BehaviorFunctions.HideTip()
		----移除关卡
		--BehaviorFunctions.RemoveLevel(104570201)
		----传送回大世界地图
	...
	if self.missionState == 15 then
		...
		BehaviorFunctions.AddDelayCallByFrame(5,BehaviorFunctions,BehaviorFunctions.ExitDuplicate)
		self.missionState = 16
	end
	
end

function LevelBehavior104570201:WeakGuide(guideId)
...
function LevelBehavior104570201:RemoveEntity(instanceId)

end

function LevelBehavior104570201:__delete()

end

--死亡事件
function LevelBehavior104570201:Die(attackInstanceId,dieInstanceId)
...
--死亡动画后事件
function LevelBehavior104570201:Death(instanceId,isFormationRevive)
	if isFormationRevive == true then
		--BehaviorFunctions.SetDuplicateResult(true)
		BehaviorFunctions.SendTaskProgress(1045703,1,1)
		BehaviorFunctions.RemoveEntity(self.beilubeite)
		BehaviorFunctions.ShowBlackCurtain(false,0.5,true)
		self.missionState = 13
...
function LevelBehavior104570201:CastSkill(instanceId,skillId,skillType)
...
function LevelBehavior104570201:FinishSkill(instanceId,skillId,skillType)
	if instanceId == self.beilubeite and skillId == 92001916 then
		BehaviorFunctions.SendTaskProgress(1045702,1,1)
...
function LevelBehavior104570201:StoryStartEvent(dialogId)
...
function LevelBehavior104570201:StoryEndEvent(dialogId)
	for i,v in ipairs(self.dialogList) do
		if dialogId == v.Id then
...
function LevelBehavior104570201:StoryPassEvent(dialogId)
...
function LevelBehavior104570201:BgmChangeLogic(group,groupNum,memberNum)
	if group then
...
function LevelBehavior104570201:LevelLookAtPos(pos,type,frame,bindTransform)
...
function LevelBehavior104570201:DisablePlayerInput(isOpen,closeUI)
	--取消摇杆移动
...
--复活逻辑
function LevelBehavior104570201:Revive(instanceId, entityId)
...
--赋值
function LevelBehavior104570201:Assignment(variable,value)
	self[variable] = value
end

function LevelBehavior104570201:DuplicateCountdownFinish()
	LogError("111")
end
