...
function TcpConn:__init()
    ...
    self.try = nil
    self.maxTry = 50
	self.loopTime = 10--次数求余==0的时候重新创建tcp，connect
...
function TcpConn:OnConnect()
    ...
    --self:LoopConnect()
    self.connTimer = LuaTimerManager.Instance:AddTimer(self.maxTry, 0.5, self:ToFunc("LoopConnect"))
end
...
function TcpConn:OnDisconnect(err)
    Log("Tcp.Disconnect")
    self.socket:close()
    self.socket = nil
    self.packList:Clear()
