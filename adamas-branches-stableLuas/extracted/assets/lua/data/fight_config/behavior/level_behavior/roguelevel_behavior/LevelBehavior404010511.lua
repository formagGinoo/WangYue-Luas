LevelBehavior404010511 = BaseClass("LevelBehavior404010511",LevelBehaviorBase)
--烽火挑战

function LevelBehavior404010511.GetGenerates()
	local generates = {
...
function LevelBehavior404010511:__init(fight)
----敌人参数------------------------------------------------------------------------------------------------------------------------------------------
	...
	self.enemyList = {
		...
		--Pos2敌人和道具选择
		[4] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 900040, posName = "_EnemyBorn1", Id = nil, tage = "Enemy"},
		[5] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 900040, posName = "_EnemyBorn2", Id = nil, tage = "Enemy"},
		[6] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 900050, posName = "_EnemyBorn3", Id = nil, tage = "Enemy"},
		...
		--Pos3敌人和道具选择
		[9] = {towerPos = "Pos3", state = self.monsterStateEnum.Default, monsterId = 900042, posName = "_EnemyBorn1", Id = nil, tage = "Enemy"},
		[10] = {towerPos = "Pos3", state = self.monsterStateEnum.Default, monsterId = 900041, posName = "_EnemyBorn2", Id = nil, tage = "Enemy"},
		[11] = {towerPos = "Pos3", state = self.monsterStateEnum.Default, monsterId = 900050, posName = "_EnemyBorn3", Id = nil, tage = "Enemy"},
		}
...
function LevelBehavior404010511:Init()
	self.missionState = 0
...
function LevelBehavior404010511:Update()
...
--生成敌人
function LevelBehavior404010511:enemySpawn(enemyList,PosList)
...
--生成信号塔
function LevelBehavior404010511:TowerSpawn(towerList,PosList)
	for x,y in ipairs(PosList) do
...
function LevelBehavior404010511:EnterArea(triggerInstanceId, areaName, logicName)
...
function LevelBehavior404010511:ExitArea(triggerInstanceId, areaName, logicName)
	if areaName == "eventZone" and triggerInstanceId == self.role then
		self.leaveArea = true
	end
end
...
function LevelBehavior404010511:Die(attackInstanceId,dieInstanceId)
...
function LevelBehavior404010511:CheckTowerState()
	for i,v in ipairs(self.posList) do
...
--检查哪些塔完成了
function LevelBehavior404010511:WorldInteractClick(uniqueId,instanceId)
	for i,v in ipairs(self.posList) do
...
--全队死亡判定失败
function LevelBehavior404010511:Death(instanceId,isFormationRevive)
...
--任务前置肉鸽追踪指标
function LevelBehavior404010511:RogueGuidePointer(guidePos,guideDistance,guideType)
	local playerPos = BehaviorFunctions.GetPositionP(self.role)
...
--肉鸽追踪（给信号塔添加标记）
function LevelBehavior404010511:TowerGuidePointer()
	for i,v in ipairs(self.posList) do
		--如果信号塔已经被关闭则不添加标记
