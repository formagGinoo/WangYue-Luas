LevelBehavior200508001 = BaseClass("LevelBehavior200508001",LevelBehaviorBase)
--屋顶解谜战斗关卡-普通模版1

function LevelBehavior200508001.GetGenerates()
	local generates = {
		790012000, --噬脉从侍
		790008000, --箴石之劣_木
		791012000, --啸叫
...
function LevelBehavior200508001:__init(fight)
	...
	--关卡状态控制
	self.fight = fight
	self.missionState = 0             --关卡状态参数
	self.missionCreated = false       --关卡是否加载参数
	self.missionCreateDis = 80        --关卡提示创建距离
	self.missionUnloadDis = 190       --关卡卸载距离(停用)
	self.missionPos = "type_normal_1" --关卡创建位置
	self.currentTip = nil             --关卡当前tips
	...
	self.monsterList =
	{
		[1] = {entityId = 790006000 ,posName = "enemyPos1" ,wave = 1, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
		[2] = {entityId = 790006000 ,posName = "enemyPos2" ,wave = 1, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
		[3] = {entityId = 790012000 ,posName = "enemyPos4" ,wave = 2, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
		[4] = {entityId = 790006000 ,posName = "enemyPos5" ,wave = 2, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
		[5] = {entityId = 791012000 ,posName = "enemyPos1" ,wave = 3, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
		[6] = {entityId = 790008000 ,posName = "enemyPos3" ,wave = 3, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
	}
	...
	self.propList =
	{
		...
		[2] = {objName = "airWallFx_", type = "fx", count = 10},
	}
...
function LevelBehavior200508001:Init()
...
function LevelBehavior200508001:Update()
	...
	if self.missionCreated == true then
		...
		elseif self.missionState == 3 then
			...
			self:GuidePointer(self.guidePosName,30)
			--检测当前波次是否为最后一波
					...
					if self.waveTotalCount == self.waveDeathCount then
						if self.isCountDown == false then
							...
							if self.frame >= self.targetFrame then
								...
								self.LevelCommon:LevelCameraLookAtPos(22002,40,nil,self.guidePosName,0.4,0.6)

								--BehaviorFunctions.ActiveSceneObj("circal",false,self.levelId)
								--BehaviorFunctions.SceneObjPlayAnim("circal","FxYueNengCircal_01Ani_Stand",self.levelId)
...
--区域判断
function LevelBehavior200508001:EnterArea(triggerInstanceId, areaName, logicName)
	--判断玩家是否进入区域内
...
--区域退出判断
function LevelBehavior200508001:ExitArea(triggerInstanceId, areaName, logicName)
	----判断玩家是否抵达最大关卡卸载范围
	...
	for i,v in ipairs(self.monsterList) do
		if triggerInstanceId == v.id and v.tag == "enemy" and areaName == "roofArea" and triggerInstanceId ~= self.role then
...
--创建看向相机
function LevelBehavior200508001:WaveSpawnCamera(waveGuideList)
...
--检查死亡
function LevelBehavior200508001:Die(attackInstanceId,dieInstanceId)
	for i,v in ipairs(self.monsterList) do
...
--关卡追踪
function LevelBehavior200508001:GuidePointer(posName,radius)
...
--关卡移除追踪
function LevelBehavior200508001:RemoveGuidePointer(guide)
	if guide then
		BehaviorFunctions.RemoveEntity(self.guide)
		self.guide = nil
	end
...
--判断是否隐藏tips
function LevelBehavior200508001:TipDisCheck(posName)
	--判断距离玩法的位置
	local playerPos = BehaviorFunctions.GetPositionP(self.role)
	local guidePos = BehaviorFunctions.GetTerrainPositionP(posName, self.levelId)
	local distance = BehaviorFunctions.GetDistanceFromPos(playerPos,guidePos)
...
--世界交互事件检测
function LevelBehavior200508001:WorldInteractClick(uniqueId,instanceId)
...
--禁用角色移动
function LevelBehavior200508001:DisablePlayerInput(isOpen,closeUI)
	--取消摇杆移动
...
--赋值
function LevelBehavior200508001:Assignment(variable,value)
	self[variable] = value
end

--保证关卡仅有一个tipsid
function LevelBehavior200508001:AddlevelTipSingle(tipsId)
	if self.currentTip then
		BehaviorFunctions.RemoveLevelTips(self.currentTip)
	end
...
--------------关卡创建--------------------------------------------------------------------------------
--基于波次生成敌人
function LevelBehavior200508001:SpawnEnemy(wave)
	for i,v in ipairs(self.monsterList) do
...
--控制敌人战斗目标
function LevelBehavior200508001:SetEnemyFightTarget()
	for i,v in ipairs(self.monsterList) do
...
--检测此关卡总敌人数量
function LevelBehavior200508001:TotalEnemyCount()
	for i,v in ipairs(self.monsterList) do
		if v.tag == "enemy" then
...
--对比获取最大wave值
function LevelBehavior200508001:WaveLimitCount()
	self.waveLimit = 1
	for i,v in ipairs(self.monsterList) do
...
--------------关卡移除处理--------------------------------------------------------------------------------
function LevelBehavior200508001:RemoveTips()
...
--批次设置预设物件显示
function LevelBehavior200508001:setSceneObjDeActive(List,index)
...
--批次设置预设物件显示
function LevelBehavior200508001:setSceneObjActive(List,index)
	local count = List[index].count
	for i = count,1,-1 do
		local objName = List[index].objName..i
...
--关卡结束时显示关卡commontitle
function LevelBehavior200508001:FinishLevel(levelId)
...
function LevelBehavior200508001:RemoveLevel(levelId)
	if levelId == self.levelId then
		--移除空气墙
		self:setSceneObjDeActive(self.propList,1)
		self:setSceneObjDeActive(self.propList,2)
