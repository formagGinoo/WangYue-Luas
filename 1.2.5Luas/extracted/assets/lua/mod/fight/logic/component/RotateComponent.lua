...
function RotateComponent:DoModelXZRotate(x, z)
	self.entity.clientEntity.clientTransformComponent:SetOffsetRot(x, z)
end
...
function RotateComponent:LookAtTarget(entity, transformName, includeX)
	...
	if transformName then 
		pos1 = entity.clientEntity.clientTransformComponent:GetTransform(transformName).position
		if not pos1 then
...
function RotateComponent:Update()
	...
	if self.isLookAt then
		...
		if not self.lookAtNowSpeed then
			self.lookAtNowSpeed = self.lookAtInitSpeed
		end
		...
		if rotateSpeed < needSpeed then
			...
			UnityUtils.SetRotation(self.entity.clientEntity.clientTransformComponent.transform, self.lookAtRotate.x, self.lookAtRotate.y, self.lookAtRotate.z, self.lookAtRotate.w)
...
function RotateComponent:AfterUpdate()
	if self.followTarget then
		local followEntity = self.fight.entityManager:GetEntity(self.followTarget)
		if not followEntity then
			self.followTarget = nil
			LogError("------     	RotateComponent		self.followTarget = nil ")
			return
		end
		self.followRotation:Set()
		if self.transformComponent.position.y > followEntity.transformComponent.position.y then
			if Quat.Equals(self.followTargetLastRotation, followEntity.transformComponent.lastRotation) and
					Quat.Equals(self.followTargetRotation, followEntity.transformComponent.rotation)  then
				self.followTargetLastRotation:CopyValue(followEntity.transformComponent.lastRotation)
				self.followTargetRotation:CopyValue(followEntity.transformComponent.rotation)
				return
			end
			self.followTargetLastRotation:CopyValue(followEntity.transformComponent.lastRotation)
			self.followTargetRotation:CopyValue(followEntity.transformComponent.rotation)

			local inverse = followEntity.transformComponent.lastRotation:Normalize():Inverse()
			local followRotation = inverse * followEntity.transformComponent.rotation:Normalize()
			local euler = followRotation:ToEulerAngles()
			local yRotation = Quat.Euler(0, euler.y, 0)
			self.followRotation = yRotation
			--暂时不考虑应用倾斜
			local otherRotation = Quat.Euler(euler.x, 0, euler.z)
...
function RotateComponent:SetVector(x,y)
	if self.entity.stateComponent and 
		self.entity.stateComponent:IsState(FightEnum.EntityState.Aim) then
		return
	end
	...
	if self.entity.stateComponent and (self.entity.stateComponent:IsState(FightEnum.EntityState.Move) or self.entity.stateComponent:IsState(FightEnum.EntityState.Idle)) then
...
function RotateComponent:SetFollowTarget(instanceId)
	self.followTarget = instanceId
end

function RotateComponent:RemoveFollowTarget()
	self.followTarget = nil
end
