...
function Behavior2020501:WorldInteractClick(uniqueId, instanceId)
	...
	if self.canActive then
		if self.done  then
			if self.intercatDone == false then
				...
				self.interactUniqueId = BehaviorFunctions.WorldInteractActive(self.me,WorldEnum.InteractType.Check,nil,"领取据点奖励",1)
			end
		end
	end
end
