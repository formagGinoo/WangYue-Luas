LevelBehavior32000005= BaseClass("LevelBehavior32000005",LevelBehaviorBase)
--资源收集-约币收集：从士火本   --难度1

function LevelBehavior32000005.GetGenerates()
	local generates = {2000,900040,900041,900050,900051,910040}
	return generates
end

function LevelBehavior32000005:__init(fight)
	self.fight = fight
end


function LevelBehavior32000005:Init()
	...
	self.monsterList =
	{
		[1] = {Id = nil , state = self.monsterStateEnum.Default ,bp = "mon1" ,wave = 1 ,lev = 8 ,entityId = 900040},  --1从士
		[2] = {Id = nil , state = self.monsterStateEnum.Default ,bp = "mon2" ,wave = 1 ,lev = 8 ,entityId = 900040},  --1从士
		[3] = {Id = nil , state = self.monsterStateEnum.Default ,bp = "mon4" ,wave = 1 ,lev = 8 ,entityId = 900051},  --1火弓
		[4] = {Id = nil , state = self.monsterStateEnum.Default ,bp = "mon1" ,wave = 2 ,lev = 8 ,entityId = 900041},  --2火从士
		[5] = {Id = nil , state = self.monsterStateEnum.Default ,bp = "mon2" ,wave = 2 ,lev = 8 ,entityId = 900041},  --2火从士
		[6] = {Id = nil , state = self.monsterStateEnum.Default ,bp = "mon4" ,wave = 2 ,lev = 8 ,entityId = 900050},  --2木弓
		[7] = {Id = nil , state = self.monsterStateEnum.Default ,bp = "mon1" ,wave = 3 ,lev = 8 ,entityId = 910040},  --3精英从士
	}
	
end

function LevelBehavior32000005:Update()
	...
	if self.missionState == 0 then
		...
		--设置关卡相机
		local fp1 = BehaviorFunctions.GetTerrainPositionP("mon2",32000005)
	...
	elseif self.missionState == self.currentWave + 0.1 then
		...
		if self.allDie == true then
			...
			elseif self.wave == self.currentWave then
				--副本成功
				BehaviorFunctions.AddDelayCallByFrame(50,BehaviorFunctions,BehaviorFunctions.ResDuplicateResult,true)
...
function LevelBehavior32000005:CreatMonster(wave)
...
function LevelBehavior32000005:Death(instanceId,isFormationRevive)
	--计算怪物全部死亡
	for i,v in ipairs(self.monsterList) do
		--计算当前波次怪物死亡数
	...
	if isFormationRevive == true then
		...
		BehaviorFunctions.ResDuplicateResult(false)
...
function LevelBehavior32000005:__delete()
