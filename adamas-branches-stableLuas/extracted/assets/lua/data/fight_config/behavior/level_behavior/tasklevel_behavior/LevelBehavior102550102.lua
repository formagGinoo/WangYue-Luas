LevelBehavior102550102 = BaseClass("LevelBehavior102550102",LevelBehaviorBase)
--拍照2混混战斗

function LevelBehavior102550102.GetGenerates()
	local generates = {900140,790007001}
	return generates
end


function LevelBehavior102550102:__init(taskInfo)
	self.enemyList = {
		[1] = {bp = "enemy1", enemyId = 790007001, id = nil, lev = 0},
		[2] = {bp = "enemy2", enemyId = 790007001, id = nil, lev = 0},
	}
...
function LevelBehavior102550102:Update()
	...
	if self.taskState == 1 then
		if self.deathCount == self.enemyCount then
			--LogError("拍照2混混战斗".."___完成___")
			BehaviorFunctions.FinishLevel(self.levelId)
			--BehaviorFunctions.SendTaskProgress(1025501,6,1,1)
			self.taskState = 999
		end
	end
end

function LevelBehavior102550102:summonMonster()
...
--设置怪物战斗目标
function LevelBehavior102550102:SetCombatTarget(List)
	if List then
...
function LevelBehavior102550102:Death(instanceId,isFormationRevive)
	for i,v in ipairs(self.enemyList) do
...
function LevelBehavior102550102:EnterArea(triggerInstanceId, areaName, logicName)
	for i,v in ipairs(self.enemyList) do
