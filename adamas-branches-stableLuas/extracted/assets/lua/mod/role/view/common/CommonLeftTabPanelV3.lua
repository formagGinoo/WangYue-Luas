CommonLeftTabPanelV3 = BaseClass("CommonLeftTabPanelV3", BasePanel)
local colorMap = {
    [1] = Color(1, 1, 1, 1),
    [2] = Color(230/255, 1, 22/255, 1),
}

--初始化
function CommonLeftTabPanelV3:__init(parent)
    self:SetAsset("Prefabs/UI/Common/CommonLeftTabPanelV3.prefab")
    self.parent = parent
    self.curType = nil
    self.toggleList = {}
end
...
--添加监听器
function CommonLeftTabPanelV3:__BindListener()
	-- self:SetHideNode("CommonLeftTabPanelV3_out")
end

--缓存对象
function CommonLeftTabPanelV3:__CacheObject()
    self:SetCacheMode(UIDefine.CacheMode.destroy)
end

function CommonLeftTabPanelV3:__Create()

end

function CommonLeftTabPanelV3:__delete()
    self.parent = nil
    self.curType = nil
end

function CommonLeftTabPanelV3:__Hide()
...
function CommonLeftTabPanelV3:__Show()
...
--#region 创建切页
function CommonLeftTabPanelV3:CreateTypeList(tabList)
    self.typeObjList = {}
    local typeList = {}
...
function CommonLeftTabPanelV3:RefreshRedPoint()
...
-- TODO:映射表配置 
function CommonLeftTabPanelV3:InitRedPoint(typeInfo, typeObj)
...
function CommonLeftTabPanelV3:GetTypeObj()
...
function CommonLeftTabPanelV3:SelectType(typeId)
...
function CommonLeftTabPanelV3:OnToggle_Type(typeId, isEnter)
    ...
    typeObj.TypeName_txt.color = isEnter and colorMap[2] or colorMap[1]

    typeObj.callback(self.parentWindow, typeObj.SingleType_tog.isOn)
end

function CommonLeftTabPanelV3:ReSelectType()
    self:OnToggle_Type(self.curType,true)
end

function CommonLeftTabPanelV3:GetCurSelectTabId()
    return self.curType
end

function CommonLeftTabPanelV3:ShowOption(typeId, isShow)
    local typeObj = self.typeObjList[typeId]
    if not typeObj then
        return
    end
...
function CommonLeftTabPanelV3:OnClose()
