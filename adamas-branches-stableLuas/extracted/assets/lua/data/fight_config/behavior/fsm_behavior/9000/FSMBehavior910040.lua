FSMBehavior910040 = BaseClass("FSMBehavior910040",FSMBehaviorBase)
--资源预加载
function FSMBehavior910040.GetGenerates()
	local generates = {}
	return generates
end
--mgaic预加载
function FSMBehavior910040.GetMagics()
	local generates = {900000024,900000025,900000107}
	return generates
end

function FSMBehavior910040:Init()
	...
	self.warnLimitRange = 2           --近身警告距离
	self.warnShortRange = 5           --近身疑问距离（无视角度）
	self.warnLongRange = 10            --远距离警告距离（结合VisionAngle）
	self.warnVisionAngle = 60          --远距离警告视角
	self.warnLimitHeight = 2          --极限警告高度
	self.warnDelayTime = 2            --警告延迟时间
	self.warnSkillId = 91004091            --警告技能Id
	self.warnSitSkillId = nil         --坐下后起立警告技能Id
	self.tauntSkillId = nil           --嘲讽技能
	self.noWarnInFightRange = 100       --havewarn ==false时 进战斗的距离
	...
	self.maxRange = 25                  --游荡超远距离边界值
	self.minRange = 2.3                   --极限近身距离，追杀模式最小追踪距离
	self.canLRWalk = true               --左右走开关
	self.LRWalkSwitchTime = 3.2		--左右走切换时间
	self.switchDelayTime = 1.4		--延迟切换时间(前后走)
	self.walkDazeTime = 1			--移动发呆时间
	self.canRun = true                  --跑步开关
	self.haveRunAndHit = false           --是否有追杀模式(↓↓跑到脸上放defaultSkill↓↓)
	self.defaultSkillId = 91004002		--默认技能id，追杀模式使用
	...
	self.ExitFightRange = 30		--脱战距离
	self.targetMaxRange = self.ExitFightRange	--超过后会脱战
...
function FSMBehavior910040:LateInit()
...
function FSMBehavior910040:Update()
	...
	--自定義邏輯
	if BehaviorFunctions.GetSkillSign(self.me,9100400)
		and self.battleTargetDistance<10
...
function FSMBehavior910040:DeathEnter(instanceId,isFormationRevive)
...
function FSMBehavior910040:Die(attackInstanceId,dieInstanceId)
...
--检测暗杀目标移除
function FSMBehavior910040:RemoveBuff(entityInstanceId, buffInstanceId,buffId)
...
function FSMBehavior910040:BeforeDie(instanceId)
	if instanceId == self.me then
...
function FSMBehavior910040:PartHit(instanceId,partName,life,damage,MissileType)
	--瞄头削刃
...
function FSMBehavior910040:CastSkill(instanceId,skillId,skillSign,skillType)
...
function FSMBehavior910040:BreakSkill(instanceId,skillId,skillSign,skillType)
	if instanceId==self.me
		and skillId ==91004004
		or skillId ==91004005
		or skillId ==91004006 then
...
function FSMBehavior910040:FinishSkill(instanceId,skillId,skillSign,skillType)
	if instanceId==self.me
		and skillId ==91004004
		or skillId ==91004005
		or skillId ==91004006 then
...
function FSMBehavior910040:Warning(instance, targetInstance,sign,isEnd)
	--当子弹快打到角色身上时，出现跳反预警
	if instance == self.me
		and sign == 910040
		and BehaviorFunctions.GetBuffCount(self.battleTarget,1000036) == 0
