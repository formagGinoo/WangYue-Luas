Behavior40019001 = BaseClass("Behavior40019001",EntityBehaviorBase)
...
function Behavior40019001.GetGenerates()
end

function Behavior40019001.GetMagics()
	local generates = {}
	return generates
end

function Behavior40019001:Init()
	self.Me = self.instanceId
end

function Behavior40019001:Update()
	...
	if not BF.HasEntitySign(self.assingTarget,62001001) then
		if self.assingTarget and self.assingTarget ~= 0 and BF.CheckEntity(self.assingTarget) then
			...
			--if BF.GetHitType(self.assingTarget) ~= FightEnum.EntityHitState.StandUp and BF.GetHitType(self.assingTarget) ~= FightEnum.EntityHitState.Lie then
				BF.DoMagic(self.Me,self.assingTarget,40019004,self._level)
				--self.assingTarget = 0
...
--function Behavior40019001:FirstCollide(attackInstanceId,hitInstanceId,instanceId,attackType,skillType, atkElement)
