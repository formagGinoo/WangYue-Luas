...
function SkillSetComponent:__init()
...
--#region 基本生命周期
function SkillSetComponent:LateInit(nowSkillSet)
	self.skillComponent = self.entity.skillComponent
	self.nowSkillSet = 1
	self.setConfig = self.entity:GetComponentConfig(FightEnum.ComponentType.SkillSet).UISets[self.nowSkillSet]

	self.setButtonInfos = self.skillComponent:GetSetButtonInfos()
	
	self:SetCommonButton()
	self:SetSkillButton()
	self:ResetMap()
end

--初始化技能
function SkillSetComponent:SetSkillButton()
	for skillId, setButtonInfo in pairs(self.setButtonInfos) do
		local setButton = self.fight.objectPool:Get(SetButton)
		setButton:Init(self.fight,self.entity, setButtonInfo[1], skillId)
		self.setButtonList[skillId] = setButton
	end
end

-- 构建部分通用按钮
function SkillSetComponent:SetCommonButton()
	for btnName, keyEvent in pairs(SkillBtn2Event) do
		...
		if config and config.Active and config.SkillId == 0 then
			...
			setButton:Init(self.fight,self.entity, setButtonInfo, keyEvent) --没有指定skillId就使用keyEvent做为索引
...
--切换轮盘
function SkillSetComponent:SwitchUIRoulette(setId)
	-- if self.nowSkillSet == setId then
	-- 	return
	-- end
	local config = self.entity:GetComponentConfig(FightEnum.ComponentType.SkillSet).UISets[setId]
	if not config then
		return
	end
	local coreInfo = self.setConfig.CoreUIConfig
	self.setConfig = config
	self.setConfig.CoreUIConfig = coreInfo
	self.nowSkillSet = setId
	TableUtils.ClearTable(self.setButtonList)
	self:SetCommonButton()
	self:SetSkillButton()
	self:ResetMap()
...
function SkillSetComponent:OnCache()
	...
	TableUtils.ClearTable(self.KeyMap)
	TableUtils.ClearTable(self.keyEventMap)
	TableUtils.ClearTable(self.skillUpdateList)
...
--检查是否可以使用技能
function SkillSetComponent:CheckUseSkill(key, showEffect, isIgnoreCostCheck)
	local config = self.relevanceSkillMap[key]
	if config then
		local entity = BehaviorFunctions.GetEntity(config.instanceId)
		if entity and entity.skillSetComponent then
			return entity.skillSetComponent:CheckUseSkill(key, showEffect, isIgnoreCostCheck)
		end
	end
	...
	return self.setButtonList[key]:CheckUseSkill(showEffect, isIgnoreCostCheck)
end
...
function SkillSetComponent:CheckSkillResources(key)
	key = self:GetKey(key)
	if not self.setButtonList[key] then
		return false
	end
	return self.setButtonList[key]:CheckSkillResources()
end
...
function SkillSetComponent:GetBtnSkillCD(key)
	key = self:GetKey(key)
	if key and self.setButtonList[key] then
		return self.setButtonList[key]:GetBtnSkillCD()
	end
end

function SkillSetComponent:SetSkillTriggerLimitTime(key, time)
	key = self:GetKey(key)
	if key and self.setButtonList[key] then
		self.setButtonList[key]:SetSkillTriggerLimitTime(time)
	end
end

function SkillSetComponent:SetSkillCoolTime(key, time)
	key = self:GetKey(key)
	if key and self.setButtonList[key] then
		local isConfig = self.setButtonList[key]:SetSkillCoolTime(time)
		if isConfig then
			EventMgr.Instance:Fire(EventName.ChangeButtonConfig, self.entity.instanceId, key)
		end
	end
end
...
function SkillSetComponent:GetSkillNeedCostValue(key)
	local config = self.relevanceSkillMap[key]
	if config then
		local entity = BehaviorFunctions.GetEntity(config.instanceId)
		if entity and entity.skillSetComponent then
			return entity.skillSetComponent:GetSkillNeedCostValue(key)
		end
	end
	key = self:GetKey(key)
	if self.setButtonList[key] then
		local _, value = self.setButtonList[key]:GetCostValue()
		return value
	end
end

function SkillSetComponent:CastSkillCost(key, isIgnoreCost)
	local config = self.relevanceSkillMap[key]
	if config then
		local entity = BehaviorFunctions.GetEntity(config.instanceId)
		if entity and entity.skillSetComponent then
			return entity.skillSetComponent:CastSkillCost(key, isIgnoreCost)
		end
	end
	key = self:GetKey(key)
	if self.setButtonList[key] then
		self.setButtonList[key]:CastSkillCost(isIgnoreCost)
	end
end
...
function SkillSetComponent:AddSkillPoint(type, addValue, source)
    local attr =  self.entity.attrComponent
	local stateCmp = self.entity.stateComponent
    if type == FightEnum.RoleSkillPoint.Ex then
		local curValue, maxValue = attr:GetValueAndMaxValue(FightEnum.RoleSkillPoint.Ex)
		local pointValue = maxValue / 3
		if addValue > 0 then
			--pointValue = math.floor(pointValue + 1)
			local percent = 1
			percent = percent + attr:GetValue(AttrType.ExSkillPointPercent)

			addValue = addValue * percent
		end
    ...
    elseif type == FightEnum.RoleSkillPoint.Normal then
		...
		if addValue > 0 then
			--pointValue = math.floor(pointValue + 1)
			local percent = 1
			if stateCmp.backstage == FightEnum.Backstage.Foreground then
				percent = percent + attr:GetValue(AttrType.SkillPointFront)
			elseif stateCmp.backstage == FightEnum.Backstage.Background then
				percent = percent + attr:GetValue(AttrType.SkillPointBack)
			end
			if source and AttrType[source] then
				percent = percent + attr:GetValue(AttrType[source])
			end
