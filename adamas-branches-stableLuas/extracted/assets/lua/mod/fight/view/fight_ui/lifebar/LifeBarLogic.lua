...
local HitStayFrame = 10 --受击红色保留时间
local HitMaxStayFrame = 60 --最长保留时间
local DecPercent = 0.5 --衰减比例
...
function LifeBarLogic:DoDecLifeAnim()
	...
	local dec = math.abs(self.rLife - self.recordLife)
	if dec < 1 then 
		self.playDec = false
		return self:GetPercent(self.rLife, self.recordMaxLife)
	end
	local frame = BehaviorFunctions.GetFightFrame()
	local percent = self:GetPercent(self.rLife, self.recordMaxLife)
	if self.lifeDec <= frame or self.lifeStayTime <= frame then
		if not self.rLife or dec < 1 then
			self.playDec = false
		else
			self.playDec = true
