LevelBehavior32000105 = BaseClass("LevelBehavior32000105",LevelBehaviorBase)
--无buff神荼挑战

function LevelBehavior32000105.GetGenerates()
	local generates = {709200300}
	return generates
end


function LevelBehavior32000105:__init(taskInfo)
	self.enemyList = {
	...
	self.monsterLevelBias = {
		...
		[FightEnum.EntityNpcTag.Boss] = 50,
	}
...
function LevelBehavior32000105:Update()
...
function LevelBehavior32000105:summonMonster()
	for i,v in ipairs(self.enemyList) do
		...
		if pos and rot then
			v.id = BehaviorFunctions.CreateEntity(v.enemyId, nil, pos.x, pos.y, pos.z, nil, nil, nil, self.levelId,55)
...
function LevelBehavior32000105:Death(instanceId,isFormationRevive)
	for i,v in ipairs(self.enemyList) do
...
function LevelBehavior32000105:AddEntitySign(instanceId,sign)
	--进入神荼2阶段处理
...
function LevelBehavior32000105:RemoveEntitySign(instanceId,sign)
	if instanceId == self.enemyList[1].id and sign == 92003018 then
...
function LevelBehavior32000105:LevelLookAtPos(pos,type,frame,bindTransform)
