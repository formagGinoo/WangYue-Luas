...
function JumpFSM:StartJump(isSkill, forceJump, forceDown)
    ...
    if self.isPlayer then
        ...
        elseif isSprint and not self.enterJump then
            speedY = JumpParam.JumpUpSprintSpeed
            self:SwitchState(JumpState.JumpUpSprint)
        elseif moveMode == FightEnum.EntityMoveMode.Run and isMoving and not self.enterJump then
            speedY = JumpParam.JumpUpRunSpeed
            self:SwitchState(JumpState.JumpUpRun)
        elseif not self.enterJump then
    ...
    local coverSpeedY = self.yMoveComponent.coverJumpUpSpeed
    local coverAcc = self.yMoveComponent.coverJumpAcc
    local paramsTable = {
        speedY = coverSpeedY and coverSpeedY or speedY,
        gravity = JumpParam.Gravity,
        accelerationY = coverAcc and coverAcc or JumpParam.JumpSpeedAcceleration,
        maxFallSpeed = JumpParam.MaxFallSpeed,
    }
    self.yMoveComponent:SetConfig(paramsTable, true)
    self.yMoveComponent:SetPlaneSpeed(self.speedZ)
end
...
function JumpFSM:Update()
    ...
    if self.isPlayer then
        ...
        elseif self.yMoveComponent:CheckDoDoubleJump() and self.stateComponent:AllowDoubleJump() then
...
function JumpFSM:OnLand(instanceId)
    if instanceId ~= self.entity.instanceId or not self.jumping then
        return
    end

    if not self.stateComponent or not self.stateComponent:IsState(FightEnum.EntityState.Skill) then
        self:DoJumpLand()
    end
...
function JumpFSM:DoJumpLand()
    if not self.jumping then
        return
    end

    -- todo 这里的玩家和怪物识别是不是可以优化一下
    ...
    if self.isPlayer then
        ...
        elseif self.fight.operationManager:CheckMove() and 
            (not self.entity.handleMoveInputComponent or self.entity.handleMoveInputComponent.enabled) and 
            (moveMode ~= FightEnum.EntityMoveMode.Walk or isSprint) and self.speedZ ~= 0 then
...
local CanJumpState =
{
    JumpState.JumpLandRoll,
    JumpState.SprintStartLand,
    JumpState.ProactiveLand,
    JumpState.RunStartLand,
    JumpState.None
}
function JumpFSM:CanJump()
    if CanJumpState[self.curState] or self.moveComponent.isAloft then
        return false
    end
