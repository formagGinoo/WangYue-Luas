...
local ResolutionRenderWin = {
    ["RoleMainWindow"] = true,
    ["FormationWindowV2"] = true,
    ["PartnerBagMainWindow"] = true,
}
...
--打开窗口
function WindowManager:OpenWindow(win,args,isCache, preloadAssets)
    local winName = win.__className
    self:OpenWindowByName(winName,args,isCache, preloadAssets)
   
end

--通过窗口名字打开创建
function WindowManager:OpenWindowByName(winName,args,isCache, preloadAssets)
    ...
    local win = self.windows[windowClass.__className] or self:CreateWindow(windowClass, preloadAssets)
    ...
    for _, window in pairs(self.windows) do
        if not WindowDefine.IgnorePauseWindow[window.__className] then
            if window.active or window.assetLoader and window.assetLoader.isLoading then
                alreadyPause = true
            end
        end
    end
    ...
    if ResolutionRenderWin[winName] then
        QualityManager.Instance:SetResolutionRenderScale(0)
    end

	if not alreadyPause then
        BehaviorFunctions.StoryResumeDialog()
        if not WindowDefine.IgnorePauseWindow[winName] then
            SystemStateMgr.Instance:OpenSystem()
            self:SetFightState(true, winName)
            BehaviorFunctions.SetCoreUIPanelActive(false)
            BehaviorFunctions.CancelJoystick()
            if InputManager.Instance then
                InputManager.Instance:AddLayerCount(InputDefine.ActionMap.UI)
            end
        end
	end
    return true
...
function WindowManager:CreateWindow(windowClass, preloadAssets)
    local window = windowClass.New()
    if preloadAssets and type(preloadAssets) == "table" then
        for _, value in pairs(preloadAssets) do
            window:AddAsset(value.path,value.type)
        end
    end
...
function WindowManager:CloseWindowByName(winName)
    ...
    if ResolutionRenderWin[winName] then
        local val = mod.GameSetCtrl:GetView(GameSetConfig.SaveKey.ViewRenderScale)
        QualityManager.Instance:SetResolutionRenderScale(val)
    end

    win:CloseWindow(not isCurWindow)
    EventMgr.Instance:Fire(EventName.TipHideEvent, winName)
    for _, window in pairs(self.windows) do
        if not WindowDefine.IgnorePauseWindow[window.__className] then
    ...
    if not WindowDefine.IgnorePauseWindow[winName] then
        SystemStateMgr.Instance:CloseSystem()
        self:SetFightState(false)
        BehaviorFunctions.SetCoreUIPanelActive(true)
        if InputManager.Instance then
...
function WindowManager:SetFightState(isPause, windowName)
    if isPause then
