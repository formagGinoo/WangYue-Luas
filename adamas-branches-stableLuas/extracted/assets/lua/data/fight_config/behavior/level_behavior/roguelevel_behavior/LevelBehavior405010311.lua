LevelBehavior405010311 = BaseClass("LevelBehavior405010311",LevelBehaviorBase)
--见义勇为，需要必须让玩家回到载具？
function LevelBehavior405010311:__init(fight)
	self.fight = fight
end


function LevelBehavior405010311.GetGenerates()
	local generates = {2040802,2040803,900070,8011001,203020601,600080}
	return generates
end


function LevelBehavior405010311:Init()
	self.role = 0
	self.initState = 0
	self.missionState = 0
	self.createState = 0
...
function LevelBehavior405010311:Update()
	...
	if self.initState == 0 then
		...
		local carPos = BehaviorFunctions.GetTerrainPositionP("R2Car1",10020005,"RogueDrive")
		local carlook = BehaviorFunctions.GetTerrainPositionP("R2CarLook1",10020005,"RogueDrive")
		...
		local aiCarPos = BehaviorFunctions.GetTerrainPositionP("R2AICar1",10020005,"RogueDrive")
		local look = BehaviorFunctions.GetTerrainPositionP("R2AICarLook1",10020005,"RogueDrive")
		...
		local monPos = BehaviorFunctions.GetTerrainPositionP("R2Mon1",10020005,"RogueDrive")
		local npcPos = BehaviorFunctions.GetTerrainPositionP("R2NPC1",10020005,"RogueDrive")
		self.monster1 = BehaviorFunctions.CreateEntity(900070,nil,monPos.x,monPos.y,monPos.z,npcPos.x,npcPos.y,npcPos.z,self.levelId,5)
		self.npc = BehaviorFunctions.CreateEntity(8011001,nil,npcPos.x,npcPos.y,npcPos.z,monPos.x,monPos.y,monPos.z,self.levelId,nil)
	...
	if self.initState == 3 then
		...
		self.position.y = y
		self.position.z = z
		BehaviorFunctions.DoLookAtTargetImmediately(self.monster1,self.aiCar)  --临时
			...
			if self.dis ~= 0 and self.dis < 30 then		
				...
				if self.inArea == 1 then
					...
					elseif self.progress2 >= 100 then
						...
						self.monster2 = BehaviorFunctions.CreateEntity(900070,nil,self.x,self.y,self.z,lookPos.x,lookPos.y,lookPos.z,self.levelId,5)
...
function LevelBehavior405010311:Death(instanceId, isFormationRevive)
...
function LevelBehavior405010311:WorldInteractClick(uniqueId, instanceId)
...
function LevelBehavior405010311:EndRogue()
...
function LevelBehavior405010311:StoryEndEvent(dialogId)
...
--赋值
function LevelBehavior405010311:Assignment(variable,value)
	self[variable] = value
end


--驾驶骇入
function LevelBehavior405010311:DriveHack(bindTarget, hackTarget)
...
--使用骇入连线
function LevelBehavior405010311:DriveHackLine(target,targetDot)
	if self.zszlOn == 1 then
		--出战角色装备箴石之劣
...
--退出驾驶骇入
function LevelBehavior405010311:QuitDriveHack()
	if self.zszlOn == 1 then
...
--肉鸽追踪
function LevelBehavior405010311:RogueGuidePointer(guidePos,guideDistance,guideType)
...
--车辆按路点移动结束后回调
function LevelBehavior405010311:onTrackPointMoveEnd(targetInstanceId)
