...
--ButtonInfo["ZhongMoButton_btn"] = { systemOpenId = SystemConfig.SystemOpenId.Partner, systemWindow = RoleMainWindow, type = ButtonType.Center }                  ---月灵
...
ButtonInfo["ActivityButton_btn"] = { systemOpenId = SystemConfig.SystemOpenId.HuoDong, systemWindow = ActivityMainWindow, type = ButtonType.Center }                ---冒险
ButtonInfo["GuideButton_btn"] = { systemOpenId = SystemConfig.SystemOpenId.Teach, systemWindow = TeachWindow, type = ButtonType.Center }            ---活动
ButtonInfo["GashaponButton_btn"] = { systemOpenId = SystemConfig.SystemOpenId.QiYuan, systemWindow = DrawMainWindow, type = ButtonType.Center }                ---祈愿
ButtonInfo["ShopButton_btn"] = { systemOpenId = SystemConfig.SystemOpenId.ShangCheng, systemWindow = PurchaseMainWindow, type = ButtonType.Center }                    ---商店
...
ButtonInfo["IdentityButton_btn"] = { systemOpenId = SystemConfig.SystemOpenId.Identity, systemWindow = IdentityWindow, type = ButtonType.Center }                  ---道途
...
function SystemMenuWindow:__BindListener()
    ...
    --self.NameBtn_btn.onClick:AddListener(self:ToFunc("OnClick_OpenPlayerNameEditorPanel"))
    ...
    --self.HeadImage_btn.onClick:AddListener(self:ToFunc("OnClick_OpenPlayerHeadImageEditorPanel"))
...
function SystemMenuWindow:__ShowComplete()
    UtilsUI.SetActive(self.SystemMenuWindowShowNode, true)
    ...
    if self:CheckNeedPlayAnimation() then
        ...
        player.clientEntity.clientAnimatorComponent:PlayAnimation("MenuOpen", 0, 0, 0, false, true)
...
function SystemMenuWindow:UpdatePlayerInfo(data)
    ...
    if info.signature and info.signature == "" then 
        self.SignatureText_txt.text = TI18N("尚未设置签名")
    end
    if info.signature and info.signature ~= "" then
    ...
    if info.avatar_id and RoleConfig.HeroBaseInfo[info.avatar_id] then
        local path = RoleConfig.HeroBaseInfo[info.avatar_id].shead_icon
...
function SystemMenuWindow:onClickSystemButton(button)
    ...
    if button.systemWindow then
        ...
        --     --TODO 临时改成跳转到月灵页签
        --     RoleMainWindow.OpenWindow(mod.RoleCtrl:GetCurUseRole(), {_jump = {RoleConfig.PageType.ZhongMo}})
...
function SystemMenuWindow:OnClick_PalyerInfo()
    self.PlayerInfoEditorPanel:SetActive(false)
