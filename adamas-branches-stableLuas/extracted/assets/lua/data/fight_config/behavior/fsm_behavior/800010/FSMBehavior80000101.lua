FSMBehavior80000101 = BaseClass("FSMBehavior80000101",FSMBehaviorBase)
--NPC通用AI-子状态机-默认状态：播动作

--初始化
function FSMBehavior80000101:Init()
...
--初始化结束
function FSMBehavior80000101:LateInit()
...
--帧事件
function FSMBehavior80000101:Update()
	--默认动作
	if self.actList and next(self.actList) 
		and self.ParentBehavior
		and self.ParentBehavior.callState == 0 and self.ParentBehavior.textState == 0 then
		self:DefaultAct()
	end	
end

--默认动作轮播
function FSMBehavior80000101:DefaultAct()
	if BehaviorFunctions.CanCtrl(self.me) then
		...
		if self.defaultActState == self.defaultActStateEnum.In and not self.defaultActing then
			...
			local nameLen = string.len(aniName)
			if string.sub(aniName, nameLen - 2, nameLen) ~= "_in" then
				aniName = self.actList[self.defaultActNum].aniName.."_in"
			end
...
--解析ActList
function FSMBehavior80000101:GetActList(list)
...
--赋值
function FSMBehavior80000101:Assignment(variable,value)
	self[variable] = value
end