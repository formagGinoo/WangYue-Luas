...
function TimeUtils.getNextMondayAtHour(hour)
    hour = hour or 4
    -- 获取输入时间戳对应的日期表
    local inputDate = os.date("*t",TimeUtils.GetCurTimestamp())
...
function TimeUtils.getTimestampForNextMonthDayHour(day, hour)
    ...
    hour = hour or 4
    -- 获取当前本地时间表
    local currentDate = os.date("*t",TimeUtils.GetCurTimestamp())
    local offset = GetTimezoneOffset()
...
function TimeUtils.SecondsToMinutesSeconds(seconds)
	local result = {}
	result.minutes = math.floor(seconds / minuteSeconds)
	result.seconds = math.floor(seconds % minuteSeconds)
...
function TimeUtils.GetTimeDesc(seconds)
