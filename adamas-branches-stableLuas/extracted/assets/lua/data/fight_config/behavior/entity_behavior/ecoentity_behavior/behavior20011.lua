Behavior20011 = BaseClass("Behavior20011",EntityBehaviorBase)
--受击次数限制及闪避挑战

function Behavior20011.GetGenerates()
	local generates = {900040,910040}
	return generates
end

function Behavior20011:Init()
...
function Behavior20011:Update()
	...
	if self.challengeSuccece ~= nil and self.challengeEnd == false then
		if self.challengeSuccece == true then
			...
			--挑战成功后创建宝箱
			BehaviorFunctions.ChangeEcoEntityCreateState(2002060003,true)
...
function Behavior20011:WorldInteractClick(uniqueId, instanceId)
...
--清理战场
function Behavior20011:ResetBattle(monsterList)
...
--召唤敌人
function Behavior20011:SummonMonster(monsterList,monsterNumLimit)
...
function Behavior20011:Hit(attackInstanceId,hitInstanceId,hitType)
	--角色受伤计数
...
--击杀全部怪物挑战检查
function Behavior20011:KillAllMonsterChallenge(monsterList)
	for i,v in pairs(monsterList) do
...
--挑战开始
function Behavior20011:ChallengeStart(challangeName)
	--开始挑战
...
--挑战结束
function Behavior20011:ChallengeFinish(challangeName,isSuccese)
	BehaviorFunctions.HideTip()
	self.showSelf = true
	if isSuccese then
...
function Behavior20011:Die(attackInstanceId,instanceId)
...
function Behavior20011:Death(instanceId,isFormationRevive)
	--角色死亡判负
...
function Behavior20011:Dodge(attackInstanceId,hitInstanceId,limit)
	--角色闪避成功计数
	if self.dodgeTimes < self.dodgeTimesCon then
		if self.role == hitInstanceId then
