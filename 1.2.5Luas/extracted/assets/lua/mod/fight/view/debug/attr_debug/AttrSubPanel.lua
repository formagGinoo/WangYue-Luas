...
local Idx2Setting = {
	...
	[2] = {id = 2, title = "基础", pos = Vector3(-624, 0, 0), length = 180},
	[3] = {id = 3, title = "属性", pos = Vector3(346, 232, 0), length = 520},
...
local buffState2Desc =
{
	...
	[FightEnum.EntityBuffState.IgnoreCommonEnemyTimeScale] = "忽略通用怪物时间缩放",
...
function AttrSubPanel:__init(main, parent, color)
	...
	self.attrSort = {
		[1] = {name = "基础属性", clamp = {1, 5}, show = false, attr = {}},
		[2] = {name = "其他属性", clamp = {6, 20}, show = false, attr = {}},
		[3] = {name = "基础加成属性", clamp = {21, 24}, show = false, attr = {}},
		[4] = {name = "进阶属性", clamp = {25, 50}, show = false, attr = {}},
		[5] = {name = "元素属性", clamp = {51, 100}, show = false, attr = {}},
		[6] = {name = "部位属性", clamp = {101, 200}, show = false, attr = {}},
		[7] = {name = "技能属性", clamp = {201, 210}, show = false, attr = {}},
		[8] = {name = "角色自定义能量", clamp = {211, 999}, show = false, attr = {}},
		[9] = {name = "当前属性", clamp = {1001, 9999}, show = false, attr = {}},
	}
...
function AttrSubPanel:UpdateBuffData(tagIndex)
	...
	if self.entity.buffComponent then
		...
		for k, v in pairs(self.entity.buffComponent.buffs) do
			content = string.format("%d->%d[%d]: %d", v.relEntity.instanceId, v.buffId, v.level or 1, math.ceil(v.durationFrame))
			self:AddContent(tagIndex, index, content)
			index = index + 1
		end
	end
			...
			if i < #self.attrSort then
				for j = 1, #attrSortItem.attr do
					...
					if k < EntityAttrsConfig.AttrType.CurTypeBegin then
						...
						if val and val ~= 0 then
							...
							content = string.format("[%d]%s: %0.2f", k, v, val)
							self:AddContent(tagIndex, index, content)
							index = index + 1
						end
					end
				...
				for k, v in pairs(EntityAttrsConfig.AttrType2MaxType) do
					...
					if maxVal and maxVal ~= 0 then
						content = string.format("[%d]%s: %d/%d", k, self:_GetEnumName(EntityAttrsConfig.AttrType, k), math.ceil(val), maxVal)
						local contentIndex = index
						local callback = function()
				...
				if self.entity.elementStateComponent then
					...
					for k, v in pairs(elementState) do
						content = string.format("[ET]%s: %d/%d", self:_GetEnumName(FightEnum.ElementType, k), v.count, v.maxCount)
						local contentIndex = index
						local callback = function()
...
function AttrSubPanel:UpdateAttrDataBySearch(tagIndex, keyWord, id)
	...
	for k, v in pairs(EntityAttrsConfig.AttrType) do
		if (keyWord and string.find(string.lower(k), string.lower(keyWord))) or 
			...
			if val and val ~= 0 then
				local content = string.format("[%d]%s: %0.2f", v, k, val)
				self:AddContent(tagIndex, index, content)
				index = index + 1
			end
		end
