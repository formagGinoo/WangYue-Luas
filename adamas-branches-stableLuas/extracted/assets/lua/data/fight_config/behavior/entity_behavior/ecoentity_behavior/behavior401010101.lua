Behavior401010101 = BaseClass("Behavior401010101",EntityBehaviorBase)
--大悬钟电梯
function Behavior401010101.GetGenerates()
...
function Behavior401010101:Init()
...
function Behavior401010101:Update()
	...
	if self.missionState == 0 then
		...
		for i,v in ipairs(self.monsterList) do
			if v.state == self.monsterStateEnum.Default then
				...
				v.Id = BehaviorFunctions.CreateEntityByEntity(self.me,v.entityId,mpos.x,mpos.y,mpos.z)
	...
	if self.missionState == 2 then
		...
		if magicState == 0 then
			...
			if BehaviorFunctions.CheckEntity(self.hurtnpc) then	
				BehaviorFunctions.PlayAnimation(self.hurtnpc,"Squat_loop")
				--设置npc属性
		...
		--npc四散而逃
		local pos = Vec3.New(1463.5,96.6537628,1369.69214)
	...
	--播npc救命音效
	if self.missionState >= 2 and self.missionState < 6 then
		self.helpTime =self.helpTime + 1
		if self.helpTime > 15*30 then
			BehaviorFunctions.DoEntityAudioPlay(self.hurtnpc,"Play_v_pvnpc4_001",true,FightEnum.SoundSignType.Language)
			self.helpTime = 0
		end	
	end
	...
	if self.missionState == 5 then
		...
		if self.allDie == true and self.Over == false then
			...
			--移除恐惧表演
			BehaviorFunctions.PlayAnimation(self.hurtnpc,"Squat_out")
	...
	--超出范围卸载
	--暂时不用了，因为用实体创建实体接口，会自动移除实体里创建的东西
	if self.missionState >= 2 then
...
function Behavior401010101:Leave()
	--移除怪物
	for i,v in ipairs(self.monsterList) do
		if v.Id then
...
--判断任务完成条件1：怪物全部死亡
function Behavior401010101:Death(instanceId,isFormationRevive)
...
--function LevelBehavior302060201:Die(attackInstanceId,instanceId)
...
--设置攻击目标
function Behavior401010101:ChangeTarget(monsterList)
...
--受击转换攻击目标
function Behavior401010101:Hit(attackInstanceId,hitInstanceId,hitType)
	if self.missionState == 5 then
		for i,v in ipairs(self.monsterList) do
...
function Behavior401010101:BigRange()
	local rolepos = BehaviorFunctions.GetPositionP(self.role)
	local pos = BehaviorFunctions.GetTerrainPositionP("npc",10020004,"Logic10020004_Savenpc")
	local distance = BehaviorFunctions.GetDistanceFromPos(rolepos,pos)
	if distance < 80 then
...
function Behavior401010101:SmallRange()
	local rolepos = BehaviorFunctions.GetPositionP(self.role)
	local pos = BehaviorFunctions.GetTerrainPositionP("npc",10020004,"Logic10020004_Savenpc")
	local distance = BehaviorFunctions.GetDistanceFromPos(rolepos,pos)
	if distance < 30 then
...
----npc受击表演
--function Behavior401010101:FirstCollide(attackInstanceId,hitInstanceId,instanceId)
...
function Behavior401010101:StoryEndEvent(dialogId)
...
--对话时播动作
function Behavior401010101:StoryPassEvent(dialogId)
	if dialogId == 601010701 then
		if not self.play1  then
			BehaviorFunctions.PlayAnimation(self.hurtnpc,"Motou_in",FightEnum.AnimationLayer.PerformLayer)
			BehaviorFunctions.AddDelayCallByFrame(45,BehaviorFunctions,BehaviorFunctions.PlayAnimation,self.hurtnpc,"Motou_end",FightEnum.AnimationLayer.PerformLayer)
			self.play1 = true
		end
	end
	if dialogId == 601010702 then
		if not self.play2 then
			BehaviorFunctions.PlayAnimation(self.hurtnpc,"Baoshou_in",FightEnum.AnimationLayer.PerformLayer)
			BehaviorFunctions.AddDelayCallByFrame(45,BehaviorFunctions,BehaviorFunctions.PlayAnimation,self.hurtnpc,"Baoshou_end",FightEnum.AnimationLayer.PerformLayer)
			self.play2 = true
		end
	end
end
