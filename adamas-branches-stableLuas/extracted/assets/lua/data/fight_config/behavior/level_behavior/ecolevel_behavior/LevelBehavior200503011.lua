LevelBehavior200503011 = BaseClass("LevelBehavior200503011",LevelBehaviorBase)
--NPC搬运玩法1

function LevelBehavior200503011:__init(fight)
	self.fight = fight
end

function LevelBehavior200503011.GetGenerates()
	local generates = {808012004, 8011003, 2080109, 2080102, 2041201, 2041202, 2080103, 2080106, 2080107}
	return generates
end

function LevelBehavior200503011:Init()
	...
	self.logic = 200503011
	self.positionLevel = 200503011
...
function LevelBehavior200503011:Update()
...
--创建实体
function LevelBehavior200503011:CreateActor(list)
...
--空实体创建
function LevelBehavior200503011:LookAtInstance()
...
--移除收集物函数
function LevelBehavior200503011:ResetLevel()
...
--黑幕移除物品
function LevelBehavior200503011:Black()
...
--黑幕移除物品
function LevelBehavior200503011:BlackStart()
...
--失败传送
function LevelBehavior200503011:FailTransfer(position,lookAtPos)
	...
	BehaviorFunctions.AddDelayCallByTime(0.7,BehaviorFunctions,BehaviorFunctions.ShowBlackCurtain,false,1)
end

--镜头调用
function LevelBehavior200503011:levelCamera(type,cameraInstanceId,target)
...
--引导管理
function LevelBehavior200503011:guide()
	...
	if 15 < distanceBetweenPos and distanceBetweenPos <= 40 then
...
--关卡开始
function LevelBehavior200503011:LevelStart()
...
--关卡胜利判断
function LevelBehavior200503011:LevelSuccessCondition(...)
	local condition = {...}
...
--自定义关卡胜利条件1：收集物全收集
function LevelBehavior200503011:SuccessCondition_Collection(num)
	--区域内收集物数量重置
	--self.inareaNum = 0
	self.collectionCtrlState = false
	if self.collectionList then
...
--关卡失败判断
function LevelBehavior200503011:LevelFailCondition(...)
	local condition = {...}
	--计算完成条件的数量
	for i,v in ipairs(condition) do
		if v == true then
...
function LevelBehavior200503011:FailCondition_Distance1(pos,warndistance,faildistance,timeLimit)
	local characterPos = BehaviorFunctions.GetPositionP(self.role)
...
function LevelBehavior200503011:FailCondition_Distance2(timeLimit)
	--重置状态收集物计数
	self.resetNPCNum = 0
	self.collectionCtrlState = false
	for k,v in ipairs(self.collectionList) do
...
--function LevelBehavior200503011:OnBuildDisconnect(instanceId_1, instanceId_2, point1, point2, disconnectType)
...
--关卡状态机
function LevelBehavior200503011:ChallengeLevelFSM()
	--关卡处于默认状态时
	if self.levelState == self.levelStateEnum.Default then
...
--赋值
function LevelBehavior200503011:Assignment(variable,value)
	self[variable] = value
end
...
--提示光圈回调
function LevelBehavior200503011:EnterArea(triggerInstanceId, areaName)
	for k,v in ipairs(self.collectionList) do
...
--交互实体回调
function LevelBehavior200503011:WorldInteractClick(uniqueId,instanceId)
	--初次对话阶段按钮
...
--教学关闭回调
function LevelBehavior200503011:LevelInstructionComplete(tipld)
...
--对话节点
function LevelBehavior200503011:StoryPassEvent(dialogId)
	--关卡加载后初次交互
	if dialogId == self.firstInteractId then
		...
		--BehaviorFunctions.CameraEntityLockTarget(self.LevCamera1,self.lookAtList[1].id)--让关卡相机看向目标
		--BehaviorFunctions.DoLookAtTargetImmediately(self.role,self.lookAtList[1].id)--转向目标
...
--节奏控制
function LevelBehavior200503011:StoryEndEvent(dialogId)
	--自定义失败条件2：NPC超出距离限制
	...
	elseif dialogId == self.disconnectInteractId2 then
		--失败传送
		--self:FailTransfer(self.lookAtList[4].bornPos, self.npc)
		--BehaviorFunctions.ShowCommonTitle(WorldTitlePanel.TitleType.challengeEnd,self.levelName,false)
		--self.levelState = self.levelStateEnum.LevelEnd
		--BehaviorFunctions.AddDelayCallByTime(0.3,self,self.Assignment,"levelState",5)
