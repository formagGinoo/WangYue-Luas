LevelBehavior102600101 = BaseClass("LevelBehavior102600101",LevelBehaviorBase)
--击败集市门口噬脉生物

function LevelBehavior102600101.GetGenerates()
	local generates = {790012000, 790013000}
	return generates
end


function LevelBehavior102600101:__init(taskInfo)
	self.enemyList = {
...
function LevelBehavior102600101:Update()
	if self.taskState == 0 then
		self:summonMonster()
		--BehaviorFunctions.SendTaskProgress(1025401,3,1,1)
		
		BehaviorFunctions.StartStoryDialog(self.storyId)
	...
	if self.taskState == 1 then
		if self.deathCount == self.enemyCount then
			--LogError("击败集市门口噬脉生物".."___完成___")
			BehaviorFunctions.FinishLevel(self.levelId)
			--BehaviorFunctions.SendTaskProgress(1026001,3,1,1)
			self.taskState = 999
		end
	end
end

function LevelBehavior102600101:summonMonster()
...
function LevelBehavior102600101:Death(instanceId,isFormationRevive)
	for i,v in ipairs(self.enemyList) do
