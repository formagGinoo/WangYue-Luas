Behavior600071 = BaseClass("Behavior600071",EntityBehaviorBase)
--资源预加载
function Behavior600071.GetGenerates()
	local generates = {}
	return generates
end

function Behavior600071.GetMagics()
	local generates = {}
	return generates
end

function Behavior600071:Init()
...
function Behavior600071:Update()
	self.PartnerAllParm:Update()
	self.PartnerCastSkill:Update()

	--佩从被动
	if BehaviorFunctions.HasEntitySign(self.me,60007101) then
		if not self.initBuff then
			local role = BehaviorFunctions.GetEntityTemplateId(self.PartnerAllParm.role)
			--临时逻辑-单手剑角色
			if role == 1001 or role == 1009 then
				BehaviorFunctions.AddBuff(self.me,self.PartnerAllParm.role,60007102,1)
				self.initBuff = true
			end
		end
	end
