...
function RoleNewUpgradePanel:__ShowComplete()
    ...
    local blurConfig = RoleConfig.GetRoleBlurConfig(self.heroId, RoleConfig.PageBlurType.LevelUp)
...
function RoleNewUpgradePanel:initCostItem()
    ...
    for i, itemId in ipairs(RoleConfig.UpgradeItems) do
        if DataItem[itemId] then
            ...
            local frontImg, backImg, itemFrontImg, itemFrontImg2 = ItemManager.GetItemColorImg(DataItem[itemId].quality)
...
--[[    if self.laseCostExp == 0 and costExp > 0 and targetLevel > self.curRoleInfo.lev then
    ...
    --[[if cost > haveGold then
        self.MoneyCostText_txt.text = string.format("<color=#000000>%s</color>", cost)
    else
    ...
    if config then
        ...
        UnityUtils.SetSizeDelata(self.AddExp.transform, (curPercent > needPercent and needPercent or curPercent) * 348, 8)
    end
...
function RoleNewUpgradePanel:onRoleInfoUpdate(isUnlock)
    ...
    UnityUtils.SetSizeDelata(self.CurExp.transform, explorePercent * 348, 8)
    UnityUtils.SetAnchoredPosition(self.AddExp_rect, explorePercent * 348, 1)
...
function RoleNewUpgradePanel:updateAttributeChange()
    ...
    if self.curTargetLevel > self.curRoleInfo.lev then
        ...
        for index, attr_id in pairs(DataHeroLevAttr[attrId].ui_show_attr) do
            ...
            local animInfo = item._anim:GetCurrentAnimatorStateInfo(0)
            if (self.lastTargetLevel == 0 or self.lastTargetLevel == self.curRoleInfo.lev) and not animInfo:IsName("UI_AttributeItem_shengji_shuzi_in_PC") then
                --item.AttributeItem_Select:SetActive(true)
                item._anim:Play("UI_AttributeItem_shengji_shuzi_in_PC", 0, 0)
                self.ExpBg_anim:Play("UI_ExpBg_qeihuan_in_PC", 0, 0)
            end
        end
    else
        ...
        for index, attr_id in pairs(DataHeroLevAttr[attrId].ui_show_attr) do
            ...
            local animInfo = item._anim:GetCurrentAnimatorStateInfo(0)
            if self.lastTargetLevel > self.curRoleInfo.lev and not animInfo:IsName("UI_AttributeItem_shengji_shuzi_out_PC") then
                --item.AttributeItem_fanhui:SetActive(true)
                item._anim:Play("UI_AttributeItem_shengji_shuzi_out_PC", 0, 0)
                self.ExpBg_anim:Play("UI_ExpBg_qeihuan_out_PC", 0, 0)
            end
        end
    end
    self.Line_rect:SetParent(self.AttributeList.transform)
end
...
function RoleNewUpgradePanel:__AfterExitAnim()
    LuaTimerManager.Instance:RemoveTimer(self.unlockTimer)
