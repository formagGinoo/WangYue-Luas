...
function CatchPartnerWindow:__ShowComplete()
    EventMgr.Instance:Fire(EventName.PauseTipQueue)   -- 阻塞弹窗
...
function CatchPartnerWindow:BtnDown()
    ...
    for i = 0, self.animatorList.Length - 1 do
        ...
        if isChangeAnim and stateInfo.normalizedTime > 0 then
            isPlaySound = false
            break
        end
    end
    ...
    if isPlaySound then
        SoundManager.Instance:PlaySound("UIPartnerCatch")
        SoundManager.Instance:StopSound("UIPartnerRevert")
    end
...
function CatchPartnerWindow:onPointerUp()
    ...
    SoundManager.Instance:StopSound("UIPartnerCatch")
    SoundManager.Instance:PlaySound("UIPartnerRevert")
...
function CatchPartnerWindow:UpdateAnimatorStopState()
    if not self.isStoping then return end
    local isEnd = true
    for i = 0, self.animatorList.Length - 1 do
        local animator = self.animatorList[i]
        local stateInfo = animator:GetCurrentAnimatorStateInfo(0)
        if stateInfo.normalizedTime > 0 then
            isEnd = false
            break
        end
    end
...
function CatchPartnerWindow:BlendingDefCamera()
    ...
    self.cameraMgr:SetMainTarget(playerObject.clientTransformComponent.transform)
end
...
function CatchPartnerWindow:__Show()
    ...
    if EffectCamera then
        ...
        UnityUtils.SetEulerAngles(self.effectTrans, 0, 0, curEulerAngles.z)

        local playerObject = self.player:GetCtrlEntityObject()
        -- playerObject.clientTransformComponent:SetActive(false)
        ...
        local eulerAngles = self.effectTrans.localRotation.eulerAngles
        state:SetCameraPovVal(eulerAngles)
    end
    ...
    local tagMap = {
        [FightEnum.EntityTag.SceneObj] = true,
    }

    self.fight.entityManager:HideAllEntity(map, tagMap)
...
function CatchPartnerWindow:__Hide()
    ...
    -- playerObject.clientTransformComponent:SetActive(true)
    ...
    EventMgr.Instance:Fire(EventName.ResumeTipQueue)  -- 恢复弹窗
