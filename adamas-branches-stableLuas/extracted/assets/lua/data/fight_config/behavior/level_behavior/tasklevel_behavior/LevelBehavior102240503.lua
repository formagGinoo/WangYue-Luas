LevelBehavior102240503 = BaseClass("LevelBehavior102240503",LevelBehaviorBase)
--中据点第三波关卡


function LevelBehavior102240503:__init(fight)
	self.fight = fight
end

function LevelBehavior102240503.GetGenerates()
	local generates = {900040,900041,900042,900050}
	return generates
end

function LevelBehavior102240503.GetStorys()
	local storys = {}
	return storys
end

function LevelBehavior102240503:Init()
	...
	self.dialogId = 102230401 --怪物死完后播
...
function LevelBehavior102240503:Update()
	self.role = BehaviorFunctions.GetCtrlEntity()
	self.frame = BehaviorFunctions.GetFightFrame()
	--召怪
	if self.initState == 0 then
		self.monsters = self:CreatMonster({
		{id = 900040 ,posName = "Drug1Mon1",wave = 1},
		{id = 900042 ,posName = "Drug1Mon2",wave = 1},
		{id = 900040 ,posName = "Drug1Mon3",wave = 1},
		})
	...
	elseif self.initState == 5 then
		if BehaviorFunctions.GetEcoEntityByEcoId(3001002020007) then
			self.ecoId = BehaviorFunctions.GetEcoEntityByEcoId(3001002020007)
	...
	elseif self.initState == 10  then
		...
		local tarPos = BehaviorFunctions.GetTerrainPositionP("Drug1",10020001,"TaskMain002")
...
function LevelBehavior102240503:__delete()

end

function LevelBehavior102240503:Finish()
	BehaviorFunctions.HideTip()
	BehaviorFunctions.SendTaskProgress(102240503,1,1)
	BehaviorFunctions.RemoveLevel(self.levelId)
end

--刷怪
function LevelBehavior102240503:CreatMonster(monsterList)
	local MonsterId = 0
...
--波数计数
function LevelBehavior102240503:WaveCount(waveNum)
	local count	= 0
...
--死亡事件
function LevelBehavior102240503:Death(instanceId,isFormationRevive)
	local i = 0
	for i = #self.monsters,1,-1 do
...
function LevelBehavior102240503:EnterArea(triggerInstanceId,areaName,logicName)
	if triggerInstanceId == self.role and areaName == "DrugArea1" and logicName == "TaskMain002" then
