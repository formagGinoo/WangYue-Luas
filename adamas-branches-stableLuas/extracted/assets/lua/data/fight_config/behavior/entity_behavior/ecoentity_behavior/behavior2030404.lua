Behavior2030404 = BaseClass("Behavior2030404",EntityBehaviorBase)
--输出窗口挑战

function Behavior2030404.GetGenerates()
	local generates = {900040,910040}
	return generates
end

function Behavior2030404:Init()
...
function Behavior2030404:Update()
	...
	if self.challengeSuccece ~= nil and self.challengeEnd == false then
		...
		if self.challengeSuccece == true then
			...
			--挑战成功后创建宝箱
			BehaviorFunctions.ChangeEcoEntityCreateState(2002060086,true)
...
function Behavior2030404:WorldInteractClick(uniqueId, instanceId)
...
function Behavior2030404:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
function Behavior2030404:ExitTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
	if self.showSelf then
...
--清理战场
function Behavior2030404:ResetBattle(monsterList)
...
--召唤敌人
function Behavior2030404:SummonMonster(monsterList,monsterNumLimit)
...
--击杀全部怪物挑战检查
function Behavior2030404:KillAllMonsterChallenge(monsterList)
	for i,v in pairs(monsterList) do
...
--挑战开始
function Behavior2030404:ChallengeStart(challangeName)
	--开始挑战
...
--挑战结束
function Behavior2030404:ChallengeFinish(challangeName,isSuccese)
	BehaviorFunctions.HideTip()
	self.showSelf = true
	if isSuccese then
...
function Behavior2030404:Die(attackInstanceId,instanceId)
	for i,v in pairs(self.monsterList) do
...
function Behavior2030404:Death(instanceId,isFormationRevive)
	--角色死亡判负
...
function Behavior2030404:CycleBuff(monsterList)
	for i,v in pairs(monsterList) do
		if v.state == self.monsterStateEnum.Live then
