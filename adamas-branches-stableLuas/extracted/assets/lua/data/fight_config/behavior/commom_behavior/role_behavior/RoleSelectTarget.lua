...
function RoleSelectTarget:Update(LockDistance,CancelLockDistance)
	...
	if BF.CheckEntityForeground(self.Me) then
		...
		--常规搜索逻辑
		self:NormalSearch(LockDistance,CancelLockDistance,0.2)
	...
	if self.RAP.CtrlRole == self.Me then
		...
		--判断是否处于PC模式，切换锁定的逻辑区分
		if not BehaviorFunctions.CheckPCPlatform() then
			--滑动屏幕镜头移除锁定状态，清除目标信息
		...
		--PC模式
			...
			--指针开启状态，判断按压屏幕
			if BehaviorFunctions.CheckMouseDisplay() then
				--滑动屏幕镜头移除锁定状态，清除目标信息
				if BF.CheckKeyPress(FK.ScreenPress) and BF.CheckKeyPress(FK.ScreenMove)
					and (not BF.HasEntitySign(1,10000007) or BF.HasEntitySign(1,10000027)) then
					self.RAP.CancelLockFrame = self.RAP.RealFrame - 1
			...
			--指针未开启状态，判断滑动
				...
				local moveVal = BehaviorFunctions.GetInputScreenMove()
				--获取每一帧鼠标移动的距离
				local dis = _sqrt((moveVal.x * moveVal.x) + (moveVal.y * moveVal.y))
				--print("dis" .. dis)
				if dis > 40
					and (not BF.HasEntitySign(1,10000007) or BF.HasEntitySign(1,10000027)) then
					self.RAP.CancelLockFrame = self.RAP.RealFrame - 1
...
--常规搜索目标逻辑
function RoleSelectTarget:NormalSearch(LockDistance,CancelLockDistance,ViewWeight) --画面内权重
	--指定锁定目标获取标记判断
	if not BF.HasEntitySign(1,10000001) then
		--搜索实体、部位
		if BF.HasEntitySign(1,10000007) then --存在持续锁定按钮标记
			--搜索合集：半径999，角度0-360，阵营2，实体标签1(Npc)，存在buff无(nil)，不存在buff1004(隐身类)，
			--距离权重1，角度权重0.3，BOSS标签，检查锁定开启，头部射线开启，高度权重1，画面内权重ViewWeight，部位锁定权重关闭，部位锁定权重开启
			self.AllTarget = BF.SearchEntities(self.Me,999,0,360,{2,3},1,nil,1004,
				1,0.3,4,false,true,1,ViewWeight,false,true)
		else
			--搜索合集：半径15，角度0-360，阵营2，实体标签1(Npc)，存在buff无(nil)，不存在buff1004(隐身类)，
			--距离权重1，角度权重0.3，任何实体标签，检查锁定开启，头部射线开启，高度权重1，画面内权重ViewWeight，部位锁定权重关闭，部位锁定权重开启
			self.AllTarget = BF.SearchEntities(self.Me,LockDistance,0,360,{2,3},1,nil,1004,
				1,0.3,{2,3,4},false,true,1,ViewWeight,false,true)
		end
		--如果搜索合集不为空
...
function RoleSelectTarget:JoystickSearch(LockDistance,isLook)
	...
	if self.RAP.LockTarget ~= 0 and self.RAP.LockTarget ~= nil and BF.CheckMove() and not BF.HasEntitySign(1,10000007) then
		...
		if (r >= 30 and r < 90) or (r <= 330 and r > 270) then
			...
			T1 = BF.SearchEntities(self.Me,LockDistance/2,r1,r2,{2,3},1,nil,1004,
				1,3,{2,3,4},false,true,2,0.5,false,true)
			d = 1		
