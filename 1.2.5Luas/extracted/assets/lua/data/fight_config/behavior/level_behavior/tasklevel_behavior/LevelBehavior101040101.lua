...
function LevelBehavior101040101:Init()
	...
	self.dialogList =
	{
		[1] = {Id = 101040101,state = self.dialogStateEnum.NotPlaying},
		[2] = {Id = 101040401,state = self.dialogStateEnum.NotPlaying},
		[3] = {Id = 101040301,state = self.dialogStateEnum.NotPlaying},
		[4] = {Id = 101040701,state = self.dialogStateEnum.NotPlaying},
		[5] = {Id = 101040801,state = self.dialogStateEnum.NotPlaying},
	}
...
function LevelBehavior101040101:Update()
	...
	if self.missionState == 1 then
		...
		if inArea then
			...
			self:LevelLookAtPos("Task1010401Mb1","Logic10020001_6",22002,90)
	...
	if self.missionState == 4 then
		...
		--第一段timeline播放结束后
		if self.dialogList[4].state == self.dialogStateEnum.PlayOver then
	...
	if self.missionState == 5 and self.time - self.timeStart >= 15 then
		...
		BehaviorFunctions.HideTip()
		--self.flower = BehaviorFunctions.GetEcoEntityByEcoId(self.flowerEcoId)
...
function LevelBehavior101040101:LevelLookAtPos(pos,logic,type,frame,bindTransform)
...
function LevelBehavior101040101:OnGuideImageTips(tipsId,isOpen)
	----日相图片引导关闭
	--if tipsId == 2000110 and isOpen == false then
		--for i,v in ipairs (self.waveList[1]) do
			--BehaviorFunctions.SetFightMainNodeVisible(2,"I",true) --技能
			--BehaviorFunctions.SetFightMainNodeVisible(2,"PowerGroup",true,1)--恢复能量条
			----移除暂停行为树
