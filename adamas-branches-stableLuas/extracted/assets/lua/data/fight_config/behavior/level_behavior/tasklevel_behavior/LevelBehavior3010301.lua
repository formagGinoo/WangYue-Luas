LevelBehavior3010301 = BaseClass("LevelBehavior3010301",LevelBehaviorBase)

function LevelBehavior3010301.GetGenerates()
	local generates = {900140,910040}
	return generates
end

function LevelBehavior3010301:__init(fight)
	self.fight = fight
end

function LevelBehavior3010301:Init()
	...
	self.monsterList =
	{
		[1] = {id = 900140 ,posName = "Mon0101" ,wave = 1},
		[2] = {id = 900140 ,posName = "Mon0102" ,wave = 1},
		[3] = {id = 900140 ,posName = "Mon0103" ,wave = 1},
		[4] = {id = 900140 ,posName = "Mon0105" ,wave = 2},
		[5] = {id = 900140 ,posName = "Mon0106" ,wave = 2},
		[6] = {id = 900140 ,posName = "Mon0107" ,wave = 2},		
	}
	...
	self.dialogList =
	{
		[1] = {Id = 602080501,state = self.dialogStateEnum.NotPlaying}, --战斗前对话
		[2] = {Id = 602080601,state = self.dialogStateEnum.NotPlaying}, --战斗结束对话
	}
...
function LevelBehavior3010301:Update()
	...
	elseif self.missionState == 8 then
		...
		--if self.LevelCommon:SuccessCondition_DefeatAllEnemy(self.monsterListInfo) then
			...
			--BehaviorFunctions.FinishLevel(3010301)	
	...
	if self.missionState == 2 then
		...
		if checkPRange < self.maxRange then
			...
			if self.camera == false then
				self.LevelCommon:LevelCameraLookAtPos(22002,40,nil,"TP0102",0.75,0.75)
	...
	if self.LevelCommon:SuccessCondition_DefeatAllEnemy(self.monsterListInfo) and self.missionState == 5 then
		...
		--传送角色
		local pos = BehaviorFunctions.GetTerrainPositionP("Trans0101",10020005,"LogicDelegateTask01")
		local pos2 = BehaviorFunctions.GetTerrainPositionP("Mon0102",self.levelId)
		local rotate = BehaviorFunctions.GetTerrainRotationP("Trans0101",10020005,"LogicDelegateTask01")
		BehaviorFunctions.InMapTransport(pos.x,pos.y,pos.z)
		BehaviorFunctions.SetEntityEuler(self.role,rotate.x,rotate.y,rotate.z)
		--相机朝向目标点

		self.LevelCommon:LevelCameraLookAtPos(22001,40,"CameraTarget","Mon0102",0.75,0.75)
		BehaviorFunctions.PlayAnimation(self.monsterListInfo.list[1].instanceId,"Stun",FightEnum.AnimationLayer.BaseLayer)
		BehaviorFunctions.PlayAnimation(self.monsterListInfo.list[2].instanceId,"Lie",FightEnum.AnimationLayer.BaseLayer)
		BehaviorFunctions.PlayAnimation(self.monsterListInfo.list[3].instanceId,"Stun",FightEnum.AnimationLayer.BaseLayer)
...
function LevelBehavior3010301:StoryEndEvent(dialogId)
...
--自定义胜利条件
function LevelBehavior3010301:CustomSuccessCondition1()
