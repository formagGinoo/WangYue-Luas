LevelBehavior404010604 = BaseClass("LevelBehavior404010604",LevelBehaviorBase)

function LevelBehavior404010604:__init(fight)
	self.fight = fight
...
function LevelBehavior404010604.GetGenerates()
	local generates = {790010200,808011001}
	return generates
end

function LevelBehavior404010604.GetMagics()
	local generates = {900000001,900000007,900000013,900000020,900000022,900000023}
	return generates
end

function LevelBehavior404010604:Init()
	--肉鸽事件信息获取
	...
	self.monsterentityid = 790010200
	self.mapname = Logic404010604
	self.mappositionid = 404010604
	self.wave = 1                        --总波次
	----可修改参数 ↑-----
...
function LevelBehavior404010604:Update()
	self.role = BehaviorFunctions.GetCtrlEntity()
	self.time = BehaviorFunctions.GetFightFrame()
	self.realtime = BehaviorFunctions.GetFightFrame()/30
	-- print("LevelBehavior404010604初始化")
...
function LevelBehavior404010604:__delete()
end

-------------------------函数--------------------------------

-- -- 更新距离函数，该函数直接对比 self.range， self.range<=30 , 30 < self.range <= 50, self.range > 50
-- function LevelBehavior404010604:UpdateRange()
...
-- --开场自定义功能函数
-- function LevelBehavior404010604:CustomLevelFunctions()
--     --如果需要开场图文教学
...
--创怪函数
function LevelBehavior404010604:CreateMonster(wave)
	--print("创怪物函数")
...
----npc受伤函数
function LevelBehavior404010604:FirstCollide(attackInstanceId,hitInstanceId,instanceId)
	--print("进入受伤函数")
...
--卸载内容
function LevelBehavior404010604:Leave()
	--移除相机
...
-- --关卡结束函数
-- function LevelBehavior404010604:LevelFinish()
--     BehaviorFunctions.Victory()
-- end

--设置攻击目标
function LevelBehavior404010604:ChangeTarget(monsterList)
...
--关卡胜利表现
function LevelBehavior404010604:Victory()
...
--关卡失败表现
function LevelBehavior404010604:Fail()
	BehaviorFunctions.HideTip(32000007) --隐藏敌人数量的tips
...
--相机函数
function LevelBehavior404010604:LevelLookAtPos(pos,logic,type,bindTransform)
...
---------------------回调----------------------------------
--结尾对话播放完毕
function LevelBehavior404010604:StoryEndEvent(dialogId)
...
--死亡回调
function LevelBehavior404010604:Death(instanceId,isFormationRevive)
...
--任务前置肉鸽追踪指标，以封装函数，不管
function LevelBehavior404010604:RogueGuidePointer(guidePos,guideDistance,guideType)
...
----随机获取怪物，并播放攻击动画--函数
function LevelBehavior404010604:GetRandomAliveMonster()
    local aliveMonsters = {}
    -- 筛选出活着的怪物
    for i, v in pairs(self.monsterList) do
...
function LevelBehavior404010604:MakeRandomMonsterAttack()
