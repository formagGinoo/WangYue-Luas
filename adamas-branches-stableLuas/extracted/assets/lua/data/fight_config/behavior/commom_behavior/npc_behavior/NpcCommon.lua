NpcCommon = BaseClass("NpcCommon",EntityBehaviorBase)

--初始化
function NpcCommon:Init()
	--逻辑参数
...
function NpcCommon:LateInit()	
	...
	--临时判断
	if self.npcId == 8010250 then
		BehaviorFunctions.SetNpcMailState(self.me,true)
	end
end

--帧事件
function NpcCommon:Update()
...
function NpcCommon:NpcBindTask(isBind, npcId)
	if not self.npcId or self.npcId ~= npcId then
		return
	end
	...
	if not isBind and self.interactUniqueId then
		BehaviorFunctions.WorldInteractRemove(self.me, self.interactUniqueId)
	end
end

--点击交互:播放默认对话
function NpcCommon:WorldInteractClick(uniqueId, instanceId)
...
--进入交互范围，添加交互列表
function NpcCommon:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--退出交互范围，移除交互列表
function NpcCommon:ExitTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--对话结束
function NpcCommon:StoryEndEvent(dialogId)
...
function NpcCommon:Patrol()
	if self.myState == self.myStateEnum.Default or self.myState == self.myStateEnum.Patrol then
		...
		--目标点
		local nextPosition = BehaviorFunctions.GetTerrainPositionP(self.patrolList[self.patrolNum].posName,self.mapId,self.logicName)
		--当前点
		...
		if self.patrolState == self.patrolStateEnum.Arrive then
			...
			if self.patrolList[self.patrolNum].aniName then
				--玩家出生且看向		
				local rotate = BehaviorFunctions.GetTerrainRotationP(self.patrolList[self.patrolNum].posName,self.mapId,self.logicName)
				local tpos = BehaviorFunctions.GetTerrainPositionP(self.patrolList[self.patrolNum].posName,self.mapId,self.logicName)
			...
			--不会表演，转向下一个目标点，完成后继续巡逻
				...
				self:NextPatrolNum()
				local targetPos = BehaviorFunctions.GetTerrainPositionP(self.patrolList[self.patrolNum].posName,self.mapId,self.logicName)
				self:Turn(pos,targetPos)
				self.patrolState = self.patrolStateEnum.Turn
			end
		end
...
function NpcCommon:PathFindingBegin()
	local nextPosition = BehaviorFunctions.GetTerrainPositionP(self.patrolList[self.patrolNum].posName,self.mapId,self.logicName)
...
function NpcCommon:PathFindingEnd(instanceId,result)
...
function NpcCommon:NextPatrolNum()
	--获取下一个目标点
	--不会折返
...
--受击
function NpcCommon:FirstCollide(attackInstanceId,hitInstanceId,instanceId)
...
--移动碰撞
function NpcCommon:OnEntityCollision(instanceId,collisionInstanceId)
...
--移动碰撞条件
function NpcCommon:BeCollideRule()
...
--位移转身
function NpcCommon:Turn(pos,lookPos)
...
--注视IK
function NpcCommon:CanLook(Can)

end

--解析patrolList
function NpcCommon:GetPatrolList(list)
...
--解析AniList
function NpcCommon:GetAniList(list)
	local beforeList = list
	local afterList = {}
	for i = 1, #beforeList do
		local stepList = self:SplitParam(beforeList[i],"-")
...
--拆分字符串
function NpcCommon:SplitParam(string,sep)
	local list = {}
	if string == nil then
		return nil
	end
...
--赋值
function NpcCommon:Assignment(variable,value)
	self[variable] = value
