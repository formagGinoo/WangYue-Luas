...
function TimeoutDeathComponent:Init(fight,entity)
	...
	self.isDeath = self.remainingFrame == 0 and true or false
	self.isNaturalDeath = true
end

function TimeoutDeathComponent:SetDeath()
	self.isNaturalDeath = false
...
function TimeoutDeathComponent:SetRemainFrame(frame)
	self.remainingFrame = frame
	self.lastRemainingFrame = frame
end
...
function TimeoutDeathComponent:AfterUpdate()
	...
	if self.isDeath and self.remainingFrame <= 0 then
	...
	if self.config.Frame > 0 and self.remainingFrame <= 0 then
		self.isDeath = true
	end
end

function TimeoutDeathComponent:RemoveEntity()
	if self.isNaturalDeath and self.config.CreateEntites then
		local position = self.entity.transformComponent.position
		local rotation = self.entity.transformComponent.rotation

		for k, v in ipairs(self.config.CreateEntites) do
			local entity = self.fight.entityManager:CreateEntity(v)
			entity.transformComponent:SetPosition(position.x, position.y, position.z)
			entity.transformComponent:SetStartRotate(rotation)
