...
function WeaponUpgradePanelV2:ShowDetail(uniqueId)
    ...
    if RoleConfig.GetStageConfig(weaponData.template_id, weaponData.stage + 1) then
        ...
        self.StageTipText_txt.text = string.format(TI18N("升至<color=#ffa234>LV.%s</color>可进行突破"), limitLev)
    else
    ...
    self.CurExp_img.fillAmount = needExp and weaponData.exp / needExp or 1
...
function WeaponUpgradePanelV2:ShowAttrChange()
    ...
    if newAttrTable then
        for index, value in pairs(newAttrTable) do
            ...
            local show = (oldValue ~= value)
            UtilsUI.SetActive(node.Content1,show)
            UtilsUI.SetActive(node.Content2,not show)
            node.arrow2:SetActive(show)
            node.BG:SetActive(math.fmod(count, 2) ~= 0)
            local animInfo = node.AttrObject_anim:GetCurrentAnimatorStateInfo(0)
            if not animInfo:IsName("UI_AttrObject_shengji_shuzi_in_PC")then
                node.AttrObject_anim:Play("UI_AttrObject_shengji_shuzi_in_PC", 0, 0)
            end
        ...
        for index, value in pairs(oldAttrTable) do
            ...
            local node = self.attrList[count] or self:GetAttrObj()
            UtilsUI.SetActive(node.Content1,false)
            UtilsUI.SetActive(node.Content2,true)
            node.AttrText_txt.text, node.CurValue_txt.text = RoleConfig.GetShowAttr(index, value)
            node.BG:SetActive(math.fmod(count, 2) ~= 0)
            local animInfo = node.AttrObject_anim:GetCurrentAnimatorStateInfo(0)
            if not animInfo:IsName("UI_AttrObject_shengji_shuzi_out_PC") then
                node.AttrObject_anim:Play("UI_AttrObject_shengji_shuzi_out_PC", 0, 0)
            end
            table.insert(self.attrList, node)
        end
    end
...
function WeaponUpgradePanelV2:__AfterExitAnim()
