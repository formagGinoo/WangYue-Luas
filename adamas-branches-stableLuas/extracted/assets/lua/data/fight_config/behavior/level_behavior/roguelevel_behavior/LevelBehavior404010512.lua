LevelBehavior404010512 = BaseClass("LevelBehavior404010512",LevelBehaviorBase)
--烽火挑战

function LevelBehavior404010512.GetGenerates()
	local generates = {
...
function LevelBehavior404010512:__init(fight)
----敌人参数------------------------------------------------------------------------------------------------------------------------------------------
	...
	self.enemyList = {
		...
		[12] = {towerPos = "Pos3", state = self.monsterStateEnum.Default, monsterId = 2030206, posName = "_ItemBorn1", Id = nil, tage = "Item"},
		}
...
function LevelBehavior404010512:Init()
	self.missionState = 0
...
function LevelBehavior404010512:Update()
...
--生成敌人
function LevelBehavior404010512:enemySpawn(enemyList,PosList)
...
--生成信号塔
function LevelBehavior404010512:TowerSpawn(towerList,PosList)
	for x,y in ipairs(PosList) do
...
function LevelBehavior404010512:EnterArea(triggerInstanceId, areaName, logicName)
...
function LevelBehavior404010512:ExitArea(triggerInstanceId, areaName, logicName)
	if areaName == "eventZone" and triggerInstanceId == self.role then
		self.leaveArea = true
	end
end
...
function LevelBehavior404010512:Die(attackInstanceId,dieInstanceId)
...
function LevelBehavior404010512:CheckTowerState()
	for i,v in ipairs(self.posList) do
...
--检查哪些塔完成了
function LevelBehavior404010512:WorldInteractClick(uniqueId,instanceId)
	for i,v in ipairs(self.posList) do
...
--全队死亡判定失败
function LevelBehavior404010512:Death(instanceId,isFormationRevive)
...
--任务前置肉鸽追踪指标
function LevelBehavior404010512:RogueGuidePointer(guidePos,guideDistance,guideType)
	local playerPos = BehaviorFunctions.GetPositionP(self.role)
...
--肉鸽追踪（给信号塔添加标记）
function LevelBehavior404010512:TowerGuidePointer()
	for i,v in ipairs(self.posList) do
		--如果信号塔已经被关闭则不添加标记
