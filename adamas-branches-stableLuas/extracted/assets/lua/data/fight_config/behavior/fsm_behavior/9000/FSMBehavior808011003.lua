FSMBehavior808011003 = BaseClass("FSMBehavior808011003",FSMBehaviorBase)
--资源预加载
function FSMBehavior808011003.GetGenerates()
	local generates = {}
	return generates
end
--mgaic预加载
function FSMBehavior808011003.GetMagics()
	local generates = {900000024,900000025,900000107}
	return generates
end

function FSMBehavior808011003:Init()
	...
	self.LRWalkSwitchTime = 36 * 0.033		--左右走切换时间
	self.switchDelayTime = 0.97		--延迟切换时间(前后走)
	self.walkDazeTime = 1			--移动发呆时间
	self.canRun = true                  --跑步开关
	self.haveRunAndHit = false           --是否有追杀模式(↓↓跑到脸上放defaultSkill↓↓)
	self.defaultSkillId = 80801100306		--默认技能id，追杀模式使用
	self.visionAngle = 50               --视野范围，不在视野范围内会进行转向
...
function FSMBehavior808011003:LateInit()
...
function FSMBehavior808011003:Update()
...
function FSMBehavior808011003:DeathEnter(instanceId,isFormationRevive)
...
function FSMBehavior808011003:Die(attackInstanceId,dieInstanceId)
...
--检测暗杀目标移除
function FSMBehavior808011003:RemoveBuff(entityInstanceId, buffInstanceId,buffId)
...
function FSMBehavior808011003:BeforeDie(instanceId)
	if instanceId == self.me then
