...
function TalentUnlockPanel:UpdateData()
    ...
    if self.args.talentLev > 1 then
        self.TitleText_txt.text = string.format(TI18N("天赋%s"),TalentConfig.LvUpType.lvUp[1])
        self.SubTitleText_txt.text = TalentConfig.LvUpType.lvUp[2]
    else
        self.TitleText_txt.text = string.format(TI18N("天赋%s"),TalentConfig.LvUpType.unLock[1])
