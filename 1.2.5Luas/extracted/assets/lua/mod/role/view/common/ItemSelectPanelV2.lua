    ...
    for index = 1, 4, 1 do
        self["Toggle" ..index.. "_tog"].onValueChanged:AddListener(function (isEnter)
			if isEnter then
                ...
                UtilsUI.SetTextColor(self["SortText"..index.."_txt"], "#757C86")
			end
        end)
        if index <= 3 then
...
function ItemSelectPanelV2:__Show()
    ...
    if config.bagType == BagEnum.BagType.Partner then
        local item = self.Template.transform:Find("PartnerItem_")
...
function ItemSelectPanelV2:SortToggle(index)
    UtilsUI.SetTextColor(self["SortText"..index.."_txt"], "#FFFFFF")
...
function ItemSelectPanelV2:SingleSelect(index, commonItem)
    ...
    itemInfo.btnFunc = function ()
        ...
        if self.curIndex and self.curIndex ~= index then
            ...
            self:UpdateCommonItem(self.curIndex)
        end
        self.curIndex = index
...
function ItemSelectPanelV2:PluralSelect(index, commonItem)
    ...
    local reduceFunc = function ()
        ...
        if type == BagEnum.BagType.Item then
            ...
            if itemInfo.selectCount == 0 then
                itemInfo.selectBoxGreen = false
                itemInfo.reduceFunc = nil
            end
        else
            itemInfo.selectedMarkGreen = false
            itemInfo.selectBoxGreen = false
            itemInfo.reduceFunc = nil
        end
    ...
    itemInfo.btnFunc = function ()
        ...
        if type == BagEnum.BagType.Item then
            ...
            itemInfo.selectCount = itemInfo.selectCount + 1
            itemInfo.selectBoxGreen = true
        else
            if itemInfo.selectedMarkGreen == true then
                return
            end
            itemInfo.selectedMarkGreen = true
            itemInfo.selectBoxGreen = true
        end
    ...
    if itemInfo.selectedMarkGreen then
        itemInfo.reduceFunc = reduceFunc
    end
end

function ItemSelectPanelV2:BatchSelection(quality, isSelect)
    local curData = self.curBagData
    self.onlyChangeData = true
    for i = 1, #curData, 1 do
        if self.pauseSelect and isSelect then
            break
        end
        ...
        if itemConfig.quality == quality then
            self:SelectData(itemInfo, itemType, isSelect)
