LevelBehavior10011007 = BaseClass("LevelBehavior10011007",LevelBehaviorBase)
--肉鸽测试关卡:浮空岛基础战斗01
function LevelBehavior10011007:__init(fight)
	self.fight = fight
end


function LevelBehavior10011007.GetGenerates()
	local generates = {900040,910040,900080,2030502,203051113,20305080002,20305023,20305080101,203050804,900070,900071}
	return generates
end


function LevelBehavior10011007:Init()
...
-----关卡配置参数---------------------------------------------------------------------------------------------------------------------------------------
	...
	self.monsterList =
	{
		[1] = {state = self.monsterStateEnum.Default,wave = 1,entityId = 900080},
	}
	...
	self.gravityList =
	{
		[1] = {state = self.monsterStateEnum.Default,wave =1,distance = 0,pos=0,radius = 40,inRadius = false,round = false, roundDeep = 5},
	}
...
function LevelBehavior10011007:LateInit()
	----设置任务开始位置
...
function LevelBehavior10011007:Update()
...
--检查玩家角色是否处于关卡范围内
function LevelBehavior10011007:CheckPlayerInRange(distance)
...
--肉鸽追踪
function LevelBehavior10011007:RogueGuidePointer(guidePos,guideDistance,guideType)
	local playerPos = BehaviorFunctions.GetPositionP(self.role)
...
--进入实体范围
function LevelBehavior10011007:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--退出实体范围
function LevelBehavior10011007:ExitTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
	if roleInstanceId == self.role then
		if triggerInstanceId == self.levelFlag then
...
--实体按钮交互
function LevelBehavior10011007:WorldInteractClick(uniqueId,instanceId)
...
--召唤敌人
function LevelBehavior10011007:SummonMonster(monsterList,monsterDotList,monsterNumLimit)
...
--召唤重力装置
function LevelBehavior10011007:SummonGravity(gravityList,gravityDotList)
...
--解决全部敌人
function LevelBehavior10011007:KillAllMonsterChallenge(monsterList,gravityList)
	for i,v in ipairs(monsterList) do
...
function LevelBehavior10011007:Die(attackInstanceId,instanceId)
	--怪物死亡计数
...
function LevelBehavior10011007:Death(instanceId,isFormationRevive)
	--角色死亡判负
...
--检查坐标点是否处于画面中
function LevelBehavior10011007:CheckPosInCam(Position)
...
function LevelBehavior10011007:OnSwitchPlayerCtrl(oldInstanceId, instanceId)
