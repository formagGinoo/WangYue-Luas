...
local sortTypeName = {
    [BagEnum.SortType.Quality] = { name = TI18N("品质排序") },
    [BagEnum.SortType.Lvl] = { name = TI18N("等级排序") },
}
...
function BagWindow:__BindListener()
    ...
    self:BindCloseBtn(self.CommonBack1_btn,self:ToFunc("HideWindow"))
    ...
    EventMgr.Instance:AddListener(EventName.BagItemChange, self:ToFunc("BagItemChange"))
end
...
function BagWindow:__Create()
    ...
    self.tabPanel = self:OpenPanel(CommonLeftTabPanelV2,
    { 
        name = TI18N("背包"), 
        name2 = "beibao",
        tabList = self.BagMainToggleInfo, 
        defaultSelect = tagId
    })
    --self.playableDirector = self.BagWindowShowNode.gameObject:GetComponent(CS.UnityEngine.Playables.PlayableDirector)
end
...
function BagWindow:__delete()
	...
	self.coinBarClass:OnCache()
	self.coinBarClass = nil
    ...
    for k, v in pairs(self.itemObjList) do
        PoolManager.Instance:Push(PoolType.class, "CommonItem", v.commonItem)
    end

    EventMgr.Instance:RemoveListener(EventName.ItemUpdate, self:ToFunc("ItemUpdate"))
    EventMgr.Instance:RemoveListener(EventName.BagItemChange, self:ToFunc("BagItemChange"))
end
...
function BagWindow:InitCurrencyBar()
	self.currencyBarClass = Fight.Instance.objectPool:Get(CurrencyBar)
	self.currencyBarClass:init(self.CurrencyBar, 3)

	self.coinBarClass = Fight.Instance.objectPool:Get(CurrencyBar)
	self.coinBarClass:init(self.CoinBar, 2)
end
...
function BagWindow:__AfterExitAnim()
...
function BagWindow:CreateBagTypeList()
    ...
    local bagTypeList = {}
    local priorityList = {}
    for k, v in ipairs(Config.DataBagTag.Find) do
...
function BagWindow:RefreshItemList()
    self.ItemScroll_recyceList.inertia = false
     LuaTimerManager.Instance:AddTimer(1,0.5, function()
        if self.ItemScroll_recyceList then
            self.ItemScroll_recyceList.inertia = true
        end
    end)
    self.curBagData = {}
    local bagData
    if self.curBagType == BagEnum.BagType.Weapon then
        bagData = mod.BagCtrl:SortBag(nil, self.sortType, self.isAscending, BagEnum.BagType.Weapon) or {}
        self.BagCount_txt.text = string.format(TI18N("%s/%s"), mod.BagCtrl:GetCapacity(BagEnum.BagType.Weapon))
    else
        bagData = mod.BagCtrl:SortBag(self.curBagType, self.sortType, self.isAscending) or {}
        self.BagCount_txt.text = string.format(TI18N("%s/%s"), mod.BagCtrl:GetCapacity(BagEnum.BagType.Item))
    end

    self.SmallTitleTxt_txt.text = self.BagMainToggleInfo[self.curBagType].name
    ...
    if self.ItemScroll_recyceList then
        ...
        self.ItemScroll_recyceList:SetCellNum(0)
        ...
        self.ItemScroll_recyceList:SetCellNum(listNum, true)
...
function BagWindow:RefreshItemCell(index, go)
    ...
    local commonItem
    local itemObj = go.transform:Find("SingleItem").gameObject
    if self.itemObjList[index] then
        commonItem = self.itemObjList[index].commonItem
    else
        commonItem = PoolManager.Instance:Pop(PoolType.class, "CommonItem") --CommonItem.New()
        if not commonItem then
            commonItem = CommonItem.New()
        end
        self.itemObjList[index] = { commonItem = commonItem, isSelect = false, itemObj = nil }
    end
...
function BagWindow:__TempShow()
    self.tabPanel:ReSelectType()
     ...
     -- 无限列表回到顶部
     UnityUtils.SetAnchoredPosition(self.ItemScrollList.transform, 0, 0)
     self.ItemScroll_recyceList.inertia = false
     LuaTimerManager.Instance:AddTimer(1,0.5, function()
        if self.ItemScroll_recyceList then
            self.ItemScroll_recyceList.inertia = true
...
function BagWindow:SetTipsBaseInfo()
    ...
    self.LevelInfo:SetActive(itemType == BagEnum.BagType.Weapon)
...
function BagWindow:OnClick_OperaBtn()
    ...
    if ItemConfig.GetItemType(itemInfo.template_id) == BagEnum.BagType.Item then
        ...
        elseif operaType == BagEnum.OperaType.Use then
			if BagEnum.UseItemPanelType[itemTypeConfig.type] then
				PanelManager.Instance:OpenPanel(UseItemPanel, self.curSelectItem.commonItem.itemInfo)
			else
        ...
        elseif operaType == BagEnum.OperaType.Formula then
            ...
            MsgBoxManager.Instance:ShowTips(string.format(TI18N("已学习配方%s"), itemCfg.name))
...
function BagWindow:UpdateDelMode()
    ...
    self.SelectedDesc_txt.text = string.format(TI18N("已选<color=#ff4c34>%s</color>/%s"), #self.delList, MAX_DEL_VOLUME)
...
function BagWindow:OnClick_ChangeAscending()
    ...
    local trans = self.SortBtn.transform
    trans.eulerAngles = Vec3.New(trans.eulerAngles.x,trans.eulerAngles.y,(trans.eulerAngles.z + 180)%360)
    self:RefreshItemList()
end
...
function BagWindow:BagItemChange(iteminfo)
    for k, v in pairs(self.itemObjList) do
        if v.commonItem.itemConfig and v.commonItem.itemInfo then
            if v.commonItem.itemInfo.template_id == iteminfo.template_id and 
            v.commonItem.itemInfo.unique_id == iteminfo.unique_id then
                v.commonItem:SetItem(iteminfo)
                v.commonItem:Show()
                local onClickFunc = function()
                    self:OnClick_SingleItem(v)
                end
                v.commonItem:SetBtnEvent(false, onClickFunc)
                if self.curSelectItem == v then
                    self:OnClick_SingleItem(v)
                end
            end
        end
