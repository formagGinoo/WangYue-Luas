...
LevelBehavior104030301 = BaseClass("LevelBehavior104030301",LevelBehaviorBase)
--刷吉禄那里的怪

function LevelBehavior104030301:__init(fight)
	self.fight = fight
end

function LevelBehavior104030301.GetGenerates()
	local generates = {
        900040,   --木从士
        900041,   --火从士
        900042,   --冰从士
...
function LevelBehavior104030301:Init()
	self.missionState = 0
    self.role = nil
    self.posName = "SaveNpc01"
	self.deathCount = 0
	self.monsterList = {
        900040,910040,900051
    }
    self.npcEntityId = 8013001
end

function LevelBehavior104030301:Update()
	self.role = BehaviorFunctions.GetCtrlEntity()
    self:CreateMonster()
end

function LevelBehavior104030301:__delete()

end

--在某个点附近创建怪
function LevelBehavior104030301:CreateMonster()
    if self.missionState == 0 then
        ...
        if distanceXZ < 30 and distanceY < 20 then
            ...
            self.monster3 = BehaviorFunctions.CreateEntity(self.monsterList[2], nil, pos.x, pos.y, pos.z-6, nil, nil, nil, self.levelId)   --特殊处理！
...
--死亡事件
function LevelBehavior104030301:Death(instanceId,isFormationRevive)
...
function LevelBehavior104030301:LevelFinish()
