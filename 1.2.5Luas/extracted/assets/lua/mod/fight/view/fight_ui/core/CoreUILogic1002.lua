...
function CoreUILogic1002:UpdatePercent(value, maxValue)
	...
	if not self.coreActive and oldPercent < percent then
		self:UpdateEffect("22091", true)
	end
end
...
function CoreUILogic1002:SwitchCoreUIType(type)
	...
	self.ui.ActiveIcon:SetActive(active)
	self.ui.NormalIcon:SetActive(not active)
