PurchaseCommonGoods = BaseClass("PurchaseCommonGoods", Module)

function PurchaseCommonGoods:__init()
	self.parent = nil
	self.goodInfo = nil
	self.object = nil
	self.itemObj = nil
...
function PurchaseCommonGoods:InitGoods(object, goodInfo, disCount, defaultShow)
	-- 获取对应的组件
	self.object = object
	...
	self:SetGoods(goodInfo)

	self.object:SetActive(false)
	self:Show()
end

function PurchaseCommonGoods:SetGoods(itemInfo)
...
function PurchaseCommonGoods:Show()
...
function PurchaseCommonGoods:SetName()
	local goodName = self.itemInfo.name .. "x" .. self.goodInfo.item_count
	self.node.ItemName_txt.text = self.itemInfo.name
end

local colorCanNotBuy = Color(1,0.36,0.36)
local colorCanBuy = Color(1,1,1)
function PurchaseCommonGoods:SetCurrency()
	local currency = self.goodInfo.consume[1]
	local originPrice = self.goodInfo.show_inti_price
	local discount = self.goodInfo.show_discount~= "" and 1 or tonumber(self.goodInfo.show_discount)
	...
	self.node.NewPriceText_txt.text = currency.value
	...
	if discount ~= 1 then 
		...
		self.node.NewPriceText_txt.text = originPrice
	else
		UtilsUI.SetActive(self.node.PriceLine,false)
		UtilsUI.SetActive(self.node.PriceText,false)
		UtilsUI.SetActive(self.node.Currency,true)
		UtilsUI.SetActive(self.node.NewPriceText,true)
	end
	LuaTimerManager.Instance:RemoveTimer(self.timer)
	self.timer = LuaTimerManager.Instance:AddTimerByNextFrame(1,0.1,function()
		if UtilsBase.IsNull(self.node.Price) then
			return
		end
...
function PurchaseCommonGoods:RemoveTimer()
	LuaTimerManager.Instance:RemoveTimer(self.timer)
end

function PurchaseCommonGoods:SetBuyLimit()
...
function PurchaseCommonGoods:SetCheckBox()
	if not self.isSelect then 
		self.node.ExchangeScollItem_click_fanhui:SetActive(true)
	else
		self.node.ExchangeScollItem_Eixt:SetActive(true)
	end
	self.node.CheckBox:SetActive(self.isSelect)
end


function PurchaseCommonGoods:SetReSetTime()
...
function PurchaseCommonGoods:SetCommonItemCount(count)
	self.commonItem:SetNum(count)
end

function PurchaseCommonGoods:SetBtnEvent(noShowPanel, btnFunc, onClickRefresh)
...
function PurchaseCommonGoods:OnReset()
