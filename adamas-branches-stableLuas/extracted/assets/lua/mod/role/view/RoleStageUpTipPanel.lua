...
function RoleStageUpTipPanel:__delete()
end
...
function RoleStageUpTipPanel:showAttributeChange()
    ...
    for i = 1, self.newStage do
        self["Stage" .. (i)]:SetActive(i == self.newStage)
    end
    ...
    for index, attr_id in pairs(showAttrList.ui_show_attr) do
        ...
        item.Name_txt.text,item.OldValue_txt.text = RoleConfig.GetShowAttr(attr_id, curValue)
        item.Name_txt.text,item.NewValue_txt.text = RoleConfig.GetShowAttr(attr_id, nextValue)
