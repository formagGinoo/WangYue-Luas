PurchasePackageSingleReward = BaseClass("PurchasePackageSingleReward", Module)

function PurchasePackageSingleReward:__init()
    self.object = nil
	self.node = {}
    self.curItemData = {}
end

function PurchasePackageSingleReward:Destroy()
	PoolManager.Instance:Push(PoolType.class, "CommonItem", self.commonItem)
end

function PurchasePackageSingleReward:InitItem(object, template_id, itemCount, unique_id)
	-- 获取对应的组件
...
function PurchasePackageSingleReward:SetItem(itemId, itemCount, unique_id)
	self.itemId = itemId
...
function PurchasePackageSingleReward:Show()
	self.node.Name_txt.text = self.itemData.name
	self.node.Count_txt.text = "x" ..self.itemCount
...
function PurchasePackageSingleReward:OnReset()
