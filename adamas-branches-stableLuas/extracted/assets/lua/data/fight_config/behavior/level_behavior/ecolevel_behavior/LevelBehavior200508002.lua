LevelBehavior200508002 = BaseClass("LevelBehavior200508002",LevelBehaviorBase)
--屋顶解谜战斗关卡-屋顶飞跃

function LevelBehavior200508002.GetGenerates()
	local generates = {
		790012000, --噬脉从侍
		790013000, --噬脉猎手
		790008000, --箴石之劣_木
...
function LevelBehavior200508002:__init(fight)
	...
	self.missionCreateDis = 80
	self.missionUnloadDis = 190
	self.missionPos = "type_higher_1"
	self.currentTip = nil
	...
	self.monsterList =
	{
		...
		[2] = {entityId = 790012000 ,posName = "enemyPos2" ,wave = 1, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
		[3] = {entityId = 790012000 ,posName = "enemyPos4" ,wave = 2, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
		[4] = {entityId = 790013000 ,posName = "enemyPos5" ,wave = 2, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
		[5] = {entityId = 790013000 ,posName = "enemyPos1" ,wave = 3, id = nil, state = self.monsterStateEnum.Default, tag = "enemy"},
...
function LevelBehavior200508002:Init()
...
function LevelBehavior200508002:Update()
...
--区域判断
function LevelBehavior200508002:EnterArea(triggerInstanceId, areaName, logicName)
	--判断玩家是否进入区域内
...
--区域退出判断
function LevelBehavior200508002:ExitArea(triggerInstanceId, areaName, logicName)
	----判断玩家是否抵达最大关卡卸载范围
...
--创建看向相机
function LevelBehavior200508002:WaveSpawnCamera(waveGuideList)
...
--检查死亡
function LevelBehavior200508002:Die(attackInstanceId,dieInstanceId)
	for i,v in ipairs(self.monsterList) do
...
--关卡追踪
function LevelBehavior200508002:GuidePointer(posName,radius)
...
--关卡移除追踪
function LevelBehavior200508002:RemoveGuidePointer(guide)
	if guide then
		BehaviorFunctions.RemoveEntity(self.guide)
		self.guide = nil
	end
...
--判断是否隐藏tips
function LevelBehavior200508002:TipDisCheck(posName)
	--判断距离玩法的位置
	local playerPos = BehaviorFunctions.GetPositionP(self.role)
	local guidePos = BehaviorFunctions.GetTerrainPositionP(posName, self.levelId)
	local distance = BehaviorFunctions.GetDistanceFromPos(playerPos,guidePos)
...
--世界交互事件检测
function LevelBehavior200508002:WorldInteractClick(uniqueId,instanceId)
...
--判断关卡是否被移除
function LevelBehavior200508002:RemoveLevel(levelId)
...
--赋值
function LevelBehavior200508002:Assignment(variable,value)
	self[variable] = value
end

--禁用角色移动
function LevelBehavior200508002:DisablePlayerInput(isOpen,closeUI)
	--取消摇杆移动
...
--保证关卡仅有一个tipsid
function LevelBehavior200508002:AddlevelTipSingle(tipsId)
	if self.currentTip then
		BehaviorFunctions.RemoveLevelTips(self.currentTip)
	end
...
--------------关卡创建--------------------------------------------------------------------------------
--基于波次生成敌人
function LevelBehavior200508002:SpawnEnemy(wave)
	for i,v in ipairs(self.monsterList) do
...
--控制敌人战斗目标
function LevelBehavior200508002:SetEnemyFightTarget()
	for i,v in ipairs(self.monsterList) do
...
--检测此关卡总敌人数量
function LevelBehavior200508002:TotalEnemyCount()
	for i,v in ipairs(self.monsterList) do
		if v.tag == "enemy" then
...
--对比获取最大wave值
function LevelBehavior200508002:WaveLimitCount()
	self.waveLimit = 1
	for i,v in ipairs(self.monsterList) do
...
--------------关卡移除处理--------------------------------------------------------------------------------
function LevelBehavior200508002:RemoveTips()
...
--批次设置预设物件显示
function LevelBehavior200508002:setSceneObjDeActive(List,index)
...
--批次设置预设物件显示
function LevelBehavior200508002:setSceneObjActive(List,index)
	local count = List[index].count
	for i = count,1,-1 do
		local objName = List[index].objName..i
...
function LevelBehavior200508002:FinishLevel(levelId)
	if levelId == self.levelId then
