LevelBehavior30000001 = BaseClass("LevelBehavior30000001",LevelBehaviorBase)
--限时击杀怪物玩法
function LevelBehavior30000001:__init(fight)
	self.fight = fight
end


function LevelBehavior30000001.GetGenerates()
	local generates = {900040,910040}
	return generates
end


function LevelBehavior30000001:Init()
...
function LevelBehavior30000001:Update()
	self.time = BehaviorFunctions.GetFightFrame()
	self.role = BehaviorFunctions.GetCtrlEntity()

	if self.missionState == 0  then
		--创建玩家出生点
		local pos = BehaviorFunctions.GetTerrainPositionP("characterBorn",30000001)
		BehaviorFunctions.DoSetPosition(self.role,pos.x,pos.y,pos.z)
		
		--关卡玩法引导
		BehaviorFunctions.ShowGuideImageTips(30001)
		self.missionState = 1
		
	elseif self.missionState == 1 then
		--创建挑战指引实体
		local pos2 = BehaviorFunctions.GetTerrainPositionP("Center",30000001)
...
--清理战场
function LevelBehavior30000001:ResetBattle(monsterList)
...
--召唤敌人
function LevelBehavior30000001:SummonMonster(monsterList,monsterNumLimit)
...
--击杀全部怪物挑战检查
function LevelBehavior30000001:KillAllMonsterChallenge(monsterList)
	for i,v in ipairs(monsterList) do
...
function LevelBehavior30000001:TimeLimitChallange(startTime,timeLimit,condiTion,showTimeRemain)
...
--额外时长奖励
function LevelBehavior30000001:BounsTime(bonusTime)
	self.bonusTime = self.bonusTime + bonusTime
end

--挑战开始
function LevelBehavior30000001:ChallengeStart(challangeName)
...
--挑战结束
function LevelBehavior30000001:ChallengeFinish(challangeName,isSuccese)
	if isSuccese then
...
function LevelBehavior30000001:Die(attackInstanceId,instanceId)
...
function LevelBehavior30000001:Death(instanceId,isFormationRevive)
	--角色死亡判负
...
function LevelBehavior30000001:BeDodge(attackInstanceId,hitInstanceId,limit)
	if hitInstanceId == self.role and limit == true then
		self:BounsTime(3)
	end
end

function LevelBehavior30000001:WorldInteractClick(uniqueId)
	if self.interactUniqueId and self.interactUniqueId == uniqueId then
		...
		self:ChallengeStart("限时击杀怪物")
		BehaviorFunctions.WorldInteractRemove(self.me,self.interactUniqueId) --移除
	end
end


function LevelBehavior30000001:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
function LevelBehavior30000001:ExitTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
function LevelBehavior30000001:LevelLookAtPos(pos,type,frame,bindTransform)
	local fp1 = BehaviorFunctions.GetTerrainPositionP(pos,30000001)
...
--死亡事件
function LevelBehavior30000001:RemoveEntity(instanceId)

end


function LevelBehavior30000001:__delete()
