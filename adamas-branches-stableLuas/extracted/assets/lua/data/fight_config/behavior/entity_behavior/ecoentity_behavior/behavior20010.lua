Behavior20010 = BaseClass("Behavior20010",EntityBehaviorBase)
--竞速挑战
function Behavior20010.GetGenerates()
	local generates = {900040,910040,2030401}
	return generates
end

function Behavior20010:Init()
	...
	self.monsterList =
	{
		...
		[3] = {state = self.monsterStateEnum.Default,wave = 1,bp = "c4_mb1_3",entityId = 900040},
		[4] = {state = self.monsterStateEnum.Default,wave = 1,bp = "c4_mb2_1",entityId = 900040},
		[5] = {state = self.monsterStateEnum.Default,wave = 1,bp = "c4_mb2_2",entityId = 900040},
		[6] = {state = self.monsterStateEnum.Default,wave = 1,bp = "c4_mb2_3",entityId = 900040},
		[7] = {state = self.monsterStateEnum.Default,wave = 1,bp = "c4_mb3_2",entityId = 910040},
	}
...
function Behavior20010:WorldInteractClick(uniqueId, instanceId)
...
function Behavior20010:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--击杀全部怪物挑战检查
function Behavior20010:ProtectChallenge(instanceId)
...
function Behavior20010:ExitTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
	if self.showSelf then
...
--清理战场
function Behavior20010:ResetBattle(monsterList)
...
--召唤敌人
function Behavior20010:SummonMonster(monsterList,monsterNumLimit)
...
function Behavior20010:ChangeTarget(monsterList)
	for i,v in ipairs(monsterList) do
		if v.state == self.monsterStateEnum.Live then
...
--击杀全部怪物挑战检查
function Behavior20010:KillAllMonsterChallenge(monsterList)
	for i,v in ipairs(monsterList) do
...
function Behavior20010:TimeLimitChallange(startTime,timeLimit,condiTion)
...
--额外时长奖励
function Behavior20010:BounsTime(bonusTime)
	self.bonusTime = self.bonusTime + bonusTime
end

--挑战开始
function Behavior20010:ChallengeStart(challangeName)
	local pos2 = BehaviorFunctions.GetTerrainPositionP("c4_Center",self.levelId,"Logic10020001_Eco1")
...
--挑战结束
function Behavior20010:ChallengeFinish(challangeName,isSuccese)
	BehaviorFunctions.HideTip()
	if BehaviorFunctions.CheckEntity(self.robot) then
...
function Behavior20010:Die(attackInstanceId,instanceId)
...
function Behavior20010:Death(instanceId,isFormationRevive)
	--角色死亡判负
