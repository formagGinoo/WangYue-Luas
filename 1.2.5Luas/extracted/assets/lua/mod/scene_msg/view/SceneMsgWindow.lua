...
function SceneMsgWindow:UpdateData(msgData)
    if msgData and msgData.id == self.msgId then
        if msgData.like then
            self.LikeCount_txt.text = string.format("（%s人）", msgData.like + MsgConfig[self.msgId].initial_like)
        end
        if msgData.dislike then
            self.DislikeCount_txt.text = string.format("（%s人）", msgData.dislike + MsgConfig[self.msgId].initial_dislike)
        end
