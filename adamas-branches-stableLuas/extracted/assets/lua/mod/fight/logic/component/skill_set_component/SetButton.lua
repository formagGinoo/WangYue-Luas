...
function SetButton:ChangeTemplate(config)
    ...
    if config.SkillCostPart then
        self.costSource = SetButtonEnum.CostSource.Entity
        local entity = self.entity
        if config.UseParentCost and entity.parent then
            self.parentEntity = entity.parent
            self.costSource = SetButtonEnum.CostSource.ParentEntity
        end
        if config.UseCostMode == SetButtonEnum.UseCostMode.EntityCost then
            if not AttrType[self.config.UseCostType] then
                self.costSource = SetButtonEnum.CostSource.Player
            end
        end
    end
end
...
function SetButton:RemoveEvent()
    ...
    EventMgr.Instance:RemoveListener(EventName.PlayerAttrChange, self:ToFunc("PlayerAttrChange"))
...
function SetButton:SetSkill()
    ...
    -- 限时触发CD
    skill.curTriggerCD = 0
    self.skill = skill
end

function SetButton:UpdateIgnoreTimeScale()
    ...
    self.isChanged = self.isChanged or false
    if config.SkillCdPart then
        self.isChanged = self:SkillCdPart(skill, config) or self.isChanged
    end

    if config.SkillCdChargePart then
        self.isChanged = self:SkillCdChargePart(skill, config) or self.isChanged
    end

    self.isChanged = self:SkillTriggerLimitTime(skill) or self.isChanged
...
function SetButton:SkillTriggerLimitTime(skill)
    if skill.curTriggerCD > 0 then
        local deltaTime = FightUtil.deltaTime
        skill.curTriggerCD = skill.curTriggerCD - deltaTime
        if skill.curTriggerCD <= 0 then
            BehaviorFunctions.fight.entityManager:CallBehaviorFun("SkillTriggerLimitTimeOver", self.entity.instanceId, self.entity.skillSetComponent:GetKeyEventBySkillId(self.skillId))
        end
        return true
    end
end

function SetButton:PlayerAttrChange(costType)
    if self.config.UseCostType == costType then
        self:UpdateShow()
    end
end

function SetButton:EntityAttrChange(costType, entity, oldValue, curValue)
    local tempEntity = self.entity
    if self.costSource == SetButtonEnum.CostSource.Entity then
        tempEntity = self.entity
    elseif self.costSource == SetButtonEnum.CostSource.ParentEntity then
        tempEntity = self.parentEntity
    end

    if entity.instanceId == tempEntity.instanceId then
...
function SetButton:SkillPointChangeAfter(instanceId, type, oldValue, newValue)
    ...
    if instanceId == tempEntity.instanceId then
        local config = self.config
        if config.SkillCostPart then
            ...
            elseif config.UseCostMode == SetButtonEnum.UseCostMode.NormalSkillPoint then
                if type == FightEnum.RoleSkillPoint.Normal then
                    self:UpdateShow()
                end
            elseif config.UseCostMode == SetButtonEnum.UseCostMode.ExSkillPoint then
...
function SetButton:ApplyChargeChange()
    ...
    if config.SkillCdChargePart and value then
        ...
        elseif config.RecoverType == SetButtonEnum.AssetType.EntityCost then
            self:SetValue(config.RecoverCostType, value)
        end
        self:UpdateShow()
    end
...
function SetButton:GetCostValue()
    ...
    if config.SkillCostPart then
        if config.UseCostMode == SetButtonEnum.UseCostMode.EntityCost then
            local curValue = self:GetValue(config.UseCostType)
        ...
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.NormalSkillPoint then
            local allPoint, ex, normal = self:GetSkillPointCost()
            return normal, config.UseCostValue, config.MaxUseCostValue
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.ExSkillPoint then
            local allPoint, ex, normal = self:GetSkillPointCost()
            return ex, config.UseCostValue, config.MaxUseCostValue
        end
    end
end
...
function SetButton:GetSkillCostType()
    ...
    if config.SkillCostPart then
        ...
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.NormalSkillPoint then
            return FightEnum.RoleSkillPoint.Normal
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.ExSkillPoint then
            return FightEnum.RoleSkillPoint.Ex
        end
    end
end
...
function SetButton:GetChargePoint()
    ...
    if config.SkillCdChargePart then
        ...
        elseif config.RecoverType == SetButtonEnum.AssetType.EntityCost then
            local curValue, maxValue = self:GetValueAndMaxValue(config.RecoverCostType)
...
function SetButton:GetTriggerLimitTime()
    local skill = self.skill
    return skill.curTriggerCD or 0, skill.maxTriggerCD or 0
end
...
function SetButton:CheckUseSkill(showEffect, isIgnoreCostCheck)
    ...
    if config.SkillCostPart and (not isIgnoreCostCheck) then
        local keyEvent = self.entity.skillSetComponent:GetKeyEventBySkillId(self.skillId)
        ...
        if config.UseCostMode == SetButtonEnum.UseCostMode.EntityCost then
            if not self:CheckCost(config.UseCostType, config.UseCostValue) then
                if showEffect and btnName then
                    BehaviorFunctions.StopSkillUIEffect("UI_SkillPanel_buzu", btnName)
                    BehaviorFunctions.PlaySkillUIEffect("UI_SkillPanel_buzu", btnName, false)
                end
                return false
            end
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.ChargePoint then
        ...
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.SkillPoint then
            if self:GetSkillPointCost() < config.UseCostValue then
                if showEffect then
                    ...
                    BehaviorFunctions.StopFightUIEffect("UI_Power_buzu", "Yellow")
                    BehaviorFunctions.PlayFightUIEffect("UI_Power_buzu", "Yellow")
                end
                return false
            end
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.NormalSkillPoint then
            local all, ex, normal =  self:GetSkillPointCost()
            if normal < config.UseCostValue then
                if showEffect then
                    if btnName then
                        BehaviorFunctions.StopSkillUIEffect("UI_SkillPanel_buzu", btnName)
                        BehaviorFunctions.PlaySkillUIEffect("UI_SkillPanel_buzu", btnName, false)
        ...
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.ExSkillPoint then
            local all, ex, normal =  self:GetSkillPointCost()
            if ex < config.UseCostValue then
                if showEffect then
                    if btnName then
                        BehaviorFunctions.StopSkillUIEffect("UI_SkillPanel_buzu", btnName)
                        BehaviorFunctions.PlaySkillUIEffect("UI_SkillPanel_buzu", btnName, false)
        ...
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.StaminaValue then
            local cur, max = BehaviorFunctions.GetPlayerAttrValueAndMaxValue(FightEnum.PlayerAttr.CurStaminaValue)
            if cur <= config.UseCostValue then
                if showEffect then
                    if btnName then
                        BehaviorFunctions.StopSkillUIEffect("UI_SkillPanel_buzu", btnName)
                        BehaviorFunctions.PlaySkillUIEffect("UI_SkillPanel_buzu", btnName, false)
...
-- 有没有足够资源
function SetButton:CheckSkillResources()
    local config = self.config
    return self:CheckCost(config.UseCostType, config.UseCostValue)
end
...
function SetButton:GetBtnSkillCD()
    local config = self.config
    local skill = self.skill
    if config.SkillCdPart and not config.IgnoreSkillCD then
        return skill.curCDtime
    end
end
...
function SetButton:CastSkillCost(isIgnoreCost)
    local config = self.config
    local skill = self.skill
    if config.SkillCostPart and (not isIgnoreCost) then
        if config.UseCostMode == SetButtonEnum.UseCostMode.EntityCost then
            local curAttrValue = self:GetValue(config.UseCostType)
            curAttrValue = math.min(curAttrValue or 0, config.MaxUseCostValue or 0)
            self:AddValue(config.UseCostType, - curAttrValue)
        elseif config.UseCostMode == SetButtonEnum.UseCostMode.ChargePoint then
            self:ChangeChargePoint(- config.MaxUseCostValue or 0)
        else
            local allPoint, curExPoint, curNormalPoint, exPointValue, normalPointValue = self:GetSkillPointCost()
            local needNormal, needEx = 0,0
            if config.UseCostMode == SetButtonEnum.UseCostMode.NormalSkillPoint then
                needNormal = math.min(config.MaxUseCostValue, curNormalPoint)
            elseif  config.UseCostMode == SetButtonEnum.UseCostMode.ExSkillPoint then
                needEx =  math.min(config.MaxUseCostValue, curExPoint)
            elseif config.UseCostMode == SetButtonEnum.UseCostMode.SkillPoint then
                needNormal = math.min(config.MaxUseCostValue - curExPoint, curNormalPoint)
                if needNormal > 0 then
                    needEx = curExPoint
                else
                    needEx = config.MaxUseCostValue
                end
            end
            local exValue = needEx * exPointValue
            local normalValue = needNormal * normalPointValue
            local tempInstanceId = self.entity.instanceId
            if self.costSource == SetButtonEnum.CostSource.ParentEntity then
                tempInstanceId = self.parentEntity.instanceId
            end

            BehaviorFunctions.fight.entityManager:CallBehaviorFun("CastSkillCostBefore", tempInstanceId, 
            self.skillId, needEx, needNormal, curExPoint, curNormalPoint, needEx + needNormal)

            self:AddValue(FightEnum.RoleSkillPoint.Ex, - exValue)
            self:AddValue(FightEnum.RoleSkillPoint.Normal, - normalValue)

            EventMgr.Instance:Fire(EventName.UseSkillDecPoint, exValue, normalValue)
            local all, ex, normal = self:GetSkillPointCost()
            BehaviorFunctions.fight.entityManager:CallBehaviorFun("CastSkillCostAfter", tempInstanceId,
             self.skillId, curExPoint - ex, curNormalPoint - normal, ex, normal, allPoint)  
        end
    end
    if config.SkillCdPart and not config.IgnoreSkillCD then
        skill.curCDtime = skill.maxCDtime
...
function SetButton:CDReadyFinsh()
    ...
    if config.SkillCostPart and config.CoolingNotGetCost then
        if config.UseCostMode == SetButtonEnum.UseCostMode.EntityCost then
            ...
            elseif self.costSource == SetButtonEnum.CostSource.ParentEntity  then
                self.parentEntity.attrComponent:LockAttr(costType, false)
            end
...
function SetButton:EnterCooling()
    ...
    if config.SkillCostPart and config.CoolingNotGetCost then
        if config.UseCostMode == SetButtonEnum.UseCostMode.EntityCost then
            ...
            elseif self.costSource == SetButtonEnum.CostSource.ParentEntity  then
                self.parentEntity.attrComponent:LockAttr(costType, true)
            end
        end
    end
end
...
function SetButton:GetSkillPointCost()
    ...
    elseif self.costSource == SetButtonEnum.CostSource.ParentEntity then
        return self.parentEntity.skillSetComponent:GetSkillPointCost()
    end
end
...
-- 限时触发CD
function SetButton:SetSkillTriggerLimitTime(cdTime)
    local config = self.config
    local skill = self.skill

    cdTime = cdTime * 10000
    skill.curTriggerCD = cdTime
    skill.maxTriggerCD = skill.curTriggerCD
    if skill.curTriggerCD < 0 then
        skill.curTriggerCD = 0
    end
end

function SetButton:SetSkillCoolTime(cdTime)
    local config = self.config
    local skill = self.skill

    local res
    if (not config.SkillCDMask) or (not config.ShowCD) then
        config.SkillCdPart = true
        config.ShowCD = true
        config.SkillCDMask = true
        config.SkillCostPart = true
        config.AutoReduceSkillCD = true
        res = true
    end

    cdTime = cdTime * 10000
    skill.curCDtime = cdTime
    skill.maxCDtime = skill.curCDtime
    if skill.curCDtime < 0 then
        skill.curCDtime = 0
    end
    return res
...
function SetButton:GetValue(attrType)
    if self.costSource == SetButtonEnum.CostSource.Entity then
        return self.entity.attrComponent:GetValue(attrType)
    elseif self.costSource == SetButtonEnum.CostSource.ParentEntity then
        return self.parentEntity.attrComponent:GetValue(attrType)
    else
        return BehaviorFunctions.GetPlayerAttrVal(attrType)
    end
end

function SetButton:SetValue(attrType, value)
    if self.costSource == SetButtonEnum.CostSource.Entity then
        return self.entity.attrComponent:SetValue(attrType, value)
    elseif self.costSource == SetButtonEnum.CostSource.ParentEntity then
        return self.parentEntity.attrComponent:SetValue(attrType, value)
    else
        return BehaviorFunctions.SetPlayerAttr(attrType, value)
    end
end

function SetButton:GetValueAndMaxValue(attrType)
    if self.costSource == SetButtonEnum.CostSource.Entity then
        return self.entity.attrComponent:GetValueAndMaxValue(attrType)
    elseif self.costSource == SetButtonEnum.CostSource.ParentEntity then
        return self.parentEntity.attrComponent:GetPlayerValueAndMaxValue(attrType)
    else
        return BehaviorFunctions.GetPlayerValueAndMaxValue(attrType)
    end
end

function SetButton:CheckCost(attrType, value)
    if self.costSource == SetButtonEnum.CostSource.Entity then
        return self.entity.attrComponent:CheckCost(attrType, value)
    elseif self.costSource == SetButtonEnum.CostSource.ParentEntity then
        return self.parentEntity.attrComponent:CheckCost(attrType, value)
    else
        local player = BehaviorFunctions.fight.playerManager:GetPlayer()
        return player.fightPlayer:CheckCost(attrType, value)
    end
end

function SetButton:AddValue(attrType, value)
    if self.costSource == SetButtonEnum.CostSource.Entity then
        return self.entity.attrComponent:AddValue(attrType, value)
    elseif self.costSource == SetButtonEnum.CostSource.ParentEntity then
        return self.parentEntity.attrComponent:AddValue(attrType, value)
    else
        return BehaviorFunctions.ChangePlayerAttr(attrType, value)
    end
end