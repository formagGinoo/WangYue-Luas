Behavior2030409 = BaseClass("Behavior2030409",EntityBehaviorBase)
--生态玩法战斗交互机关
function Behavior2030409.GetGenerates()
	local generates = {2030409}
	return generates
end


function Behavior2030409:Init()
...
function Behavior2030409:LateInit()
	-- 获取生态状态
...
function Behavior2030409:Update()
	...
	if self.missionState == 0 then
		...
		self.effect = BehaviorFunctions.CreateEntityByEntity(self.me,203040901,myPos.x,myPos.y,myPos.z)
		self.missionState = 1
	end
	...
	if self.myState == self.stateEnum.finished then
		...
		--关闭激活特效
		BehaviorFunctions.SetEntityBineVisible(self.me,"FxChallenge_fight",false)
		--关闭待机特效
		BehaviorFunctions.SetEntityBineVisible(self.me,"FxChallenge_fight_Stand",false)
		--关闭过渡特效
	...
	if self.effectTurn == true then
		...
		if self.distance <= self.effectDistance then
			...
			if self.effectState ~= self.effectStateEnum.Near then
				--隐藏待机特效
				BehaviorFunctions.AddDelayCallByFrame(7,BehaviorFunctions,BehaviorFunctions.SetEntityBineVisible,self.me,"FxChallenge_fight_Stand",false)
				--播放过渡特效
				BehaviorFunctions.SetEntityBineVisible(self.me,"FxChallenge_parkour01",true)
				--关闭时过渡特效
				BehaviorFunctions.SetEntityBineVisible(self.me,"FxChallenge_parkour02",false)
				--播放激活特效
				BehaviorFunctions.AddDelayCallByFrame(7,BehaviorFunctions,BehaviorFunctions.SetEntityBineVisible,self.me,"FxChallenge_fight",true)
				--播放激活音效
			...
			if self.effectState ~= self.effectStateEnum.Far then
				...
				BehaviorFunctions.SetEntityBineVisible(self.me,"FxChallenge_fight",false)
				--播放待机特效
				BehaviorFunctions.SetEntityBineVisible(self.me,"FxChallenge_fight_Stand",true)
				--关闭过渡特效
				BehaviorFunctions.SetEntityBineVisible(self.me,"FxChallenge_parkour01",false)
				--关闭时过渡特效
...
--点击交互
function Behavior2030409:WorldInteractClick(uniqueId,instanceId)
...
function Behavior2030409:RemoveLevel(LevelId)
	--通过remove来检测挑战是否失败
...
function Behavior2030409:FinishLevel(levelId)
	if levelId == self.bindLevelId then
