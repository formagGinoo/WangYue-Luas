...
function ByteArray:getPack(__offset, __length)
    ...
    --print("fmt:", __fmt)
    local __s = spack(__fmt, table.unpack(__t))
    return __s
end
...
function ByteArray:readULong()
    local __, __v = sunpack(self:readBuf(4), self:_getLC("L"))
    return __v
end
...
function ByteArray:readUByte()
    local __, __val = sunpack(self:readRawByte(), "b")
    return __val
end
...
function ByteArray:_checkAvailable()
	if not (#self._buf >= self._pos) then
		LogError(string_format("End of file was encountered. pos: %d, len: %d.", self._pos, #self._buf))
	end
