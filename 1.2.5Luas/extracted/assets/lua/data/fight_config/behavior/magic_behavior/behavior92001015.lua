...
Behavior92001015 = BaseClass("Behavior92001015",EntityBehaviorBase)
function Behavior92001015.GetGenerates()


end

function Behavior92001015.GetMagics()

end

function Behavior92001015:Init()
...
function Behavior92001015:Update()
	...
	if self.hited >= 10 or self.DamageCount >= self.hp * 0.2 then
		BehaviorFunctions.AddBuff(self.me,self.me,92001015,1)
		self.DamageCount = 0
		self.hited = 0
	end
...
--受击计数
function Behavior92001015:Collide(attackInstanceId,hitInstanceId)
	if hitInstanceId == self.me then
		if BehaviorFunctions.HasBuffKind(hitInstanceId,92001015) ~= true then
...
--伤害计数
function Behavior92001015:AfterDamage(attackInstanceId,hitInstanceId,damageType,magicId,damageElementType,damageVal)
	if hitInstanceId == self.me and BehaviorFunctions.HasBuffKind(hitInstanceId,92001015) ~= true then
