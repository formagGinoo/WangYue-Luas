LevelBehavior32000103 = BaseClass("LevelBehavior32000103",LevelBehaviorBase)
--无buff神荼挑战

function LevelBehavior32000103.GetGenerates()
	local generates = {709200300}
	return generates
end


function LevelBehavior32000103:__init(taskInfo)
	self.enemyList = {
	...
	self.monsterLevelBias = {
		...
		[FightEnum.EntityNpcTag.Boss] = 21,
	}
...
function LevelBehavior32000103:Update()
...
function LevelBehavior32000103:summonMonster()
	for i,v in ipairs(self.enemyList) do
		...
		if pos and rot then
			v.id = BehaviorFunctions.CreateEntity(v.enemyId, nil, pos.x, pos.y, pos.z, nil, nil, nil, self.levelId,26)
...
function LevelBehavior32000103:Death(instanceId,isFormationRevive)
	for i,v in ipairs(self.enemyList) do
...
function LevelBehavior32000103:AddEntitySign(instanceId,sign)
	--进入神荼2阶段处理
...
function LevelBehavior32000103:RemoveEntitySign(instanceId,sign)
	if instanceId == self.enemyList[1].id and sign == 92003018 then
...
function LevelBehavior32000103:LevelLookAtPos(pos,type,frame,bindTransform)
