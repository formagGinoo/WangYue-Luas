...
function Behavior2020701:Update()
	...
	if self.interactSwitch == false and self.breakTimeInterface == true then
		BehaviorFunctions.WorldInteractRemove(self.breakUntillMorningInteractId)
		BehaviorFunctions.WorldInteractRemove(self.breakUntillAfternonInteractId)
		BehaviorFunctions.WorldInteractRemove(self.breakUntillNightInteractId)
		self.breakTimeInterface = false
	end
end

function Behavior2020701:WorldInteractClick(uniqueId)
	--点击休息按钮
	if uniqueId == self.breakInteractId then
		BehaviorFunctions.WorldInteractRemove(self.breakInteractId)
		
		self.breakTimeInterface = true
		self.breakUntillMorningInteractId = BehaviorFunctions.WorldInteractActive(WorldEnum.InteractType.Talk,nil,"休息至早上",1)
		self.breakUntillAfternonInteractId = BehaviorFunctions.WorldInteractActive(WorldEnum.InteractType.Talk,nil,"休息至中午",1)
		self.breakUntillNightInteractId = BehaviorFunctions.WorldInteractActive(WorldEnum.InteractType.Talk,nil,"休息至晚上",1)
...
function Behavior2020701:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
	if triggerInstanceId == self.instanceId and self.interactSwitch == false then
		self.breakInteractId = BehaviorFunctions.WorldInteractActive(WorldEnum.InteractType.Talk,nil,"休息",1)
