...
function TalentInfoTips:SetConditionTittle()
	if self.upgradeConfig.lv == 1 then
		self.UnlockTittle_txt.text = TalentConfig.LvUpType.unLock[1] .. "条件:"
	else
		self.UnlockTittle_txt.text = TalentConfig.LvUpType.lvUp[1] .. "条件:"
	end
end
...
function TalentInfoTips:OnClick_LvUpBtn()
	if self.canLvUp and (BagCtrl:GetItemCountById(self.upgradeConfig.consume_id) >= self.upgradeConfig.consume_count) then
		...
		if not self.canLvUp then
			MsgBoxManager.Instance:ShowTips("未满足所有条件")
		else
			MsgBoxManager.Instance:ShowTips(ItemConfig.GetItemConfig(self.upgradeConfig.consume_id).name.."不足")
		end
	end
end
