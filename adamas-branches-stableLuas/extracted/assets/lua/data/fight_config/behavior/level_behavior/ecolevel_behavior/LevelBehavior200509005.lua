LevelBehavior200509005 = BaseClass("LevelBehavior200509005",LevelBehaviorBase)
--关卡测试关
function LevelBehavior200509005:__init(fight)
	self.fight = fight
end


function LevelBehavior200509005.GetGenerates()
	local generates = {8011001,8011002,8011003,8011004,8011009,8011010,8011011,8011012,
						...
						900070,900071,203041301}
	return generates
end

function LevelBehavior200509005.GetMagics()
	local generates = {200000017}
	return generates
end

function LevelBehavior200509005:Init()
	--通用参数---------------------------------------------------------------------------------------------------------------------
	...
	--需要配置的内容---------------------------------------------------------------------
	self.closeDialog = 602200301 --靠近区域播放的对话
	self.startDialog = 602200401 --开始玩法的对话
	self.battleDialog = 602200601 --开始战斗的对话
	self.endDialog = 602200701 --结束玩法的对话
	self.policeAskDialog = 602200403 --npc表示请求的对话
	self.monsterShowDialog = 602200602 --怪物不演了的对话
	self.policedialog = 602200501 --和司机的对话
	
	self.carEcoId = {id = nil,ecoId = 2003002080001} --车的生态ID
	self.lookatPos = {} --碰瓷对话注视的位置
	self.lookatIns = nil --碰瓷对话注视的实体
	
	self.ecoMe = 2003002080107 --自己生态id
	self.police = {id = nil, pos = nil,ecoId = nil, ecoPos = "npcPos8010008_2" , entity = 8010008,bubble1 = "这可怎么办啊...", bubble2 = "就没什么证据吗..."} --警察参数
	--怪物参数，只需要填怪物实体id即可
	self.monster = {id = nil, pos = nil, rot = nil, entity = 900071, npcEntity = nil, ecoId = nil,ecoPos = "npcPos8010004_1", henshin = false} --怪物参数
	self.npc = {
		[1] = {id = nil, pos = "npcPos8010004_1", rot = nil, entity = 8010004, isEnemy = true,ecoId = 5003002080024,ecoPos = "npcPos8010004_1"},
		--[2] = {id = nil, pos = nil, rot = nil, entity = 8011002, isEnemy = false,ecoId = 5003001092012,ecoPos = "npc2"},
		--[3] = {id = nil, pos = nil, rot = nil, entity = 8011003, isEnemy = false,ecoId = 5003001092013,ecoPos = "npc3"},
		--[4] = {id = nil, pos = nil, rot = nil, entity = 8010004, isEnemy = true, ecoId = 5003001092015,ecoPos = "npc5"},
		--[5] = {id = nil, pos = nil, rot = nil, entity = 8010004, isEnemy = false,ecoId = 5003001092016,ecoPos = "npc6"},
	}
	...
	--开始玩法前，npc对玩家招手
	self.npcShowState = {aniIn = "Fue_in",aniLoop = "Fue_loop",aniEnd = "Fue_end"}
	self.npcShowInFrameCD = 60 --npc举手in的动作帧数
	self.npcShowLoopFrameCD = 300 --npc举手in的动作帧数
...
function LevelBehavior200509005:Update()
	...
	if self.missionState == 0 then--------------------------------------------------------------------------------------------------------------------------------------
		...
		if self.police.id == nil then
			self.police.pos = BehaviorFunctions.GetTerrainPositionP(self.police.ecoPos,10020005,"TYC00208") --获得位置
			--创建警察
			if self.police.pos then
				self.police.id = BehaviorFunctions.CreateEntityByPosition(self.police.entity, nil,self.police.ecoPos ,"TYC00208", 10020005,self.levelId)
				self.posPoliceDefault = BehaviorFunctions.GetPositionP(self.police.id)
				BehaviorFunctions.SetEntityHackInformation(self.police.id,"张弛","Textures/Icon/Single/HeadIcon/SquNHuiguangqingtingnan.png","梦想是成为拉力赛赛车手") --修改駭入信息
				--气泡
		...
		if not self.carEcoId.id then
			self.carEcoId.id = BehaviorFunctions.GetEcoEntityByEcoId(self.carEcoId.ecoId)
			if self.carEcoId.id then
				BehaviorFunctions.SetEntityWorldInteractState(self.carEcoId.id,false)
			end
		end
		
		--获得奖励空实体的ecoId
		if self.groupMe then
			...
			for i = 1, #self.npc do
				...
				if self.npc[i].isEnemy == false then
					if self.npc[i].id == nil then
						self.npc.pos = BehaviorFunctions.GetTerrainPositionP(self.npc[i].ecoPos,10020005,"TYC00208") --获得位置
						--local npcEntity = BehaviorFunctions.GetNpcEntity(self.npc[i].ecoId)
						local npcEntity = BehaviorFunctions.CreateEntityByPosition(self.npc.entity, nil,self.npc.ecoPos ,"TYC00208", 10020005,self.levelId)
				...
				--获得npc的怪物数据
					...
					--self.npc[i].pos = BehaviorFunctions.GetTerrainPositionP(self.npc[i].ecoPos,10020005,"TYC00208") --获得位置
					--获取npc的instance实体
					local monsterEntity = BehaviorFunctions.GetNpcEntity(self.npc[i].ecoId)
					--local monsterEntity = BehaviorFunctions.CreateEntityByPosition(self.npc[i].entity, nil,self.npc[i].ecoPos ,"TYC00208", 10020005,self.levelId)
					...
					if self.monster.id then
						...
						BehaviorFunctions.SetEntityHackInformation(self.monster.id,"彭辞","Textures/Icon/Single/HeadIcon/SquRWeizhirenyuan.png","访问指令被拒绝") --修改駭入信息
						BehaviorFunctions.PlayAnimation(self.monster.id,"Lie",FightEnum.AnimationLayer.BaseLayer)
						BehaviorFunctions.ShowCharacterHeadTips(self.monster.id,true)
						--BehaviorFunctions.ChangeNpcBubbleContent(self.monster.id,"哎哟，我的腰啊...")
						--BehaviorFunctions.AddDelayCallByFrame(1,BehaviorFunctions,BehaviorFunctions.ChangeNpcBubbleContent,self.monster.id,"哎哟，疼死我了！",self.bubbleFrameCD)
						--BehaviorFunctions.AddDelayCallByFrame(1,BehaviorFunctions,BehaviorFunctions.SetNonNpcBubbleVisible,self.monster.id,true)
		...
		--if not self.lookatIns then
			--local pos = BehaviorFunctions.GetTerrainPositionP("npcPos8010004_1",10020005,"TYC00208")
			--self.lookatPos = {pos.x,pos.y+1,pos.z}
			--self.lookatIns = BehaviorFunctions.CreateEntity(2001,nil,self.lookatPos.x,self.lookatPos.y,self.lookatPos.z)
		--end
		
		----怪物加交互组件
		--if self.monster.id and self.role then
			--if BehaviorFunctions.CheckEntity(self.police.id) then
				--self.disMonster = BehaviorFunctions.GetDistanceFromTargetWithY(self.role, self.monster.id) --获得角色与怪物npc之间的距离
				----根据距离增加交互对话
				--if self.disMonster < self.interactRadius and self.levelStart == false then
					--if self.interactMonster == nil then
						----增加交互组件
						--self.interactMonster = BehaviorFunctions.WorldInteractActive(self.role,WorldEnum.InteractType.Talk,nil,"询问",1)
					--end
				--elseif self.interactMonster then
					--BehaviorFunctions.WorldInteractRemove(self.role,self.interactMonster)
					--self.interactMonster = nil
				--end
			--end
		--end
		...
		if self.police.id and self.role then
			if BehaviorFunctions.CheckEntity(self.police.id) then
				...
				--根据距离增加交互对话
				if self.disPolice < self.interactRadius and self.levelStart == true then
					if self.interactPolice == nil then
						--增加交互组件
						self.interactPolice = BehaviorFunctions.WorldInteractActive(self.role,WorldEnum.InteractType.Talk,nil,"询问司机",1)
					end
		...
		local dis = BehaviorFunctions.GetDistanceFromTarget(self.monster.id,self.role)
		if not self.startDialogStart2 then
			local dis = BehaviorFunctions.GetDistanceFromTarget(self.monster.id,self.role)
			if dis ~= nil then
				if dis <= 3 then
					BehaviorFunctions.StartStoryDialog(self.startDialog)
					self.startDialogStart2 = true
					self.startCloseDialog = true
				end
			end
			
		end
		if not self.startCloseDialog then
			if dis ~= nil then
				if dis and dis <= 20 then
					--靠近时的对话
					BehaviorFunctions.StartStoryDialog(self.closeDialog)
					self.closeDialogCam = self.LevelCommon:LevelCameraLookAtInstance(22002,-1,nil,self.monster.id,"CameraTarget",0)
					self.startCloseDialog = true
				end
			end
		...
		if self.levelStart then
			...
			if self.frame > self.startTipShowFrameNext and self.startTipShow == false then
				...
				self.startTip = BehaviorFunctions.AddLevelTips(200509008,self.levelId)
			...
			if self.frame > self.startTip1ShowFrameNext and self.startTip1Show == false then
				...
				BehaviorFunctions.ShowCommonTitle(10,"通过骇入寻找证据")
			end
		end
		...
		if self.battleDialogStart == false then
			if self.levelSearch == true then
				if self.frame > self.searchTitleShowFrameNext and self.searchTitleShow == false then
					...
					BehaviorFunctions.ShowCommonTitle(10,"和躺在地上的人交谈")
				end
				...
				if self.frame > self.searchTipShowFrameNext and self.searchTipShow == false then
					...
					self.searchTip = BehaviorFunctions.AddLevelTips(200509009,self.levelId)
		...
		if self.battleDialogStart == false then
			if self.dis < 3 then
				if self.interact == nil then
					--修改npc的交互组件——敌人可以被交互
					if self.monster.id then
						self.interact = BehaviorFunctions.WorldInteractActive(self.role,WorldEnum.InteractType.Talk,nil,"和躺地上的人交谈",1)
	...
	elseif self.missionState == 2 then
		...
		if self.henshinFinish == false then
			...
			if self.monster.henshin == false and self.monster.id then
				...
				--BehaviorFunctions.AddBuff(self.monster.id,self.monster.id,200000101)
				--BehaviorFunctions.AddBuff(self.monster.id,self.monster.id,200000100)
				BehaviorFunctions.SetEntityShowState(self.monster.id,false)
				--BehaviorFunctions.SetNpcHeadInfoVisible(self.monster.id,false)
				self.monster.id = nil
				--让警察摆出蹲下的动作
				if self.police.id then
					if BehaviorFunctions.CheckEntity(self.police.id) then
			...
			elseif self.monster.henshin == true and self.monster.id == nil then
				...
				if self.monsterListNow == nil then
					self.monsterListNow = true
					self.monster.id = BehaviorFunctions.CreateEntityByPosition(self.monster.entity, nil,self.monster.ecoPos ,"TYC00208", 10020005,self.levelId)
		...
		if self.levelBattle == true then
			...
			if self.frame > self.battleTipShowFrameNext and self.battleTipShow == false then
				...
				self.tip = BehaviorFunctions.AddLevelTips(200509010,self.levelId) --增加tips
		...
		if self.henshinFinish == true and self.monster.id and self.battleTipShow and self.battleTitleShow and self.henshinWait then
			if self.monster.id then
				...
				--开始战斗前的表现制作
				BehaviorFunctions.AddDelayCallByFrame(80,BehaviorFunctions,BehaviorFunctions.ChangeNpcBubbleContent,self.monster.id,"你这家伙！坏我好事是吧！",5)
	...
	elseif self.missionState == 4 then
		if self.frame > self.levelEndFrameNext then
			...
			if self.police.id and self.endDialogStart == false then
				...
				BehaviorFunctions.PlayAnimation(self.police.id,"Stanshou_in",FightEnum.AnimationLayer.BaseLayer)
				BehaviorFunctions.AddDelayCallByFrame(30,BehaviorFunctions,BehaviorFunctions.PlayAnimation,self.police.id,"Stanshou_loop",FightEnum.AnimationLayer.BaseLayer)
			end
	...
	elseif self.missionState == 5 then
		...
		self.carEntity = BehaviorFunctions.GetEcoEntityByEcoId(self.carEcoId.ecoId)
		BehaviorFunctions.InteractEntityHit(self.carEntity,false)
...
function LevelBehavior200509005:Die(attackInstanceId,dieInstanceId)
...
----死亡
--function LevelBehavior200509005:Death(instanceId,isFormationRevive)
...
--与NPC互动
function LevelBehavior200509005:WorldInteractClick(uniqueId,instanceId)
	...
	if uniqueId == self.interactPolice then
		if self.startDialog then
			BehaviorFunctions.StartStoryDialog(self.policedialog)
			--BehaviorFunctions.DoLookAtTargetImmediately(self.police.id,self.role)
		end
	end
end
...
--对话开始
function LevelBehavior200509005:StoryStartEvent(dialogId)
	--开始战斗的对话
	if dialogId == self.battleDialog then
		...
		if self.monster.id then
			...
			--播放npc的表现——切换成战斗待机动作
			if self.monster.id then
				--BehaviorFunctions.PlayAnimation(self.monster.id,"StandUp",FightEnum.AnimationLayer.BaseLayer)
				--BehaviorFunctions.SetHitType(self.monster.id,FightEnum.EntityHitState.StandUp)
	...
	elseif dialogId == self.startDialog then
		...
		--警察npc的特殊镜头——审视嫌疑犯们
		self.cameraPolice = self.LevelCommon:LevelCameraLookAtInstance(22002,-1,nil,self.monster.id,nil,0.3)
...
--对话结束
function LevelBehavior200509005:StoryEndEvent(dialogId)
	if dialogId == self.battleDialog then
	...
	elseif dialogId == self.closeDialog then
		self.LevelCommon:RemoveLevelCamera(self.closeDialogCam)
	end
end

--播放特定对话
function LevelBehavior200509005:StoryPassEvent(dialogId)
	--如果播放警察的请求对话
	...
	elseif dialogId == self.monsterShowDialog then
		BehaviorFunctions.PlayAnimation(self.monster.id,"StandUp",FightEnum.AnimationLayer.BaseLayer)
		BehaviorFunctions.DoLookAtTargetImmediately(self.monster.id,self.role)
		BehaviorFunctions.AddDelayCallByFrame(45,BehaviorFunctions,BehaviorFunctions.PlayAnimation,self.monster.id,"Stand2",FightEnum.AnimationLayer.BaseLayer)
		--BehaviorFunctions.PlayAnimation(self.monster.id,"Stand2",FightEnum.AnimationLayer.BaseLayer)
		----隐藏手机
		--BehaviorFunctions.SetEntityBineVisible(self.monster.id,"Phone",false)
		self.phoneInvisible = true
	elseif dialogId == 602200702 then
...
--拦截短信
function LevelBehavior200509005:HackingClickUp(instanceId)
	if instanceId == self.monster.id then
...
--输入按键
function LevelBehavior200509005:KeyInput(key, status)
...
--检查npc生成状态
function LevelBehavior200509005:CheckNPC(npcList)
	if self.monster.id then
...
--播放动作
function LevelBehavior200509005:PlayAnimationHuishou(instanceId,ani,Loop,aniFrameCD,aniInFrameCD,aniLoopFrameCD,aniEndFrameCD)
...
function LevelBehavior200509005:OnAttackNpc(attackInstanceId,hitInstanceId,instanceId,attackType, skillType)
