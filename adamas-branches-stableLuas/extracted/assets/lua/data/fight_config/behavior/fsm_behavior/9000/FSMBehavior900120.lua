FSMBehavior900120 = BaseClass("FSMBehavior900120",FSMBehaviorBase)
--资源预加载
function FSMBehavior900120.GetGenerates()
	local generates = {}
	return generates
end
--mgaic预加载
function FSMBehavior900120.GetMagics()
	local generates = {900000024,900000025,900000107}
	return generates
end

function FSMBehavior900120:Init()
	...
	self.warnSkillId = 90012004            --警告技能Id
	self.warnSitSkillId = 90012005         --坐下后起立警告技能Id
	self.tauntSkillId = 90012006           --嘲讽技能
	...
	self.LRWalkSwitchTime = 1.667		--左右走切换时间
	self.switchDelayTime = 1.333		--延迟切换时间(前后走)
	self.walkDazeTime = 1.333			--移动发呆时间
	self.canRun = true                  --跑步开关
	self.haveRunAndHit = true           --是否有追杀模式(↓↓跑到脸上放defaultSkill↓↓)
	self.defaultSkillId = 90012001		--默认技能id，追杀模式使用
...
function FSMBehavior900120:LateInit()
...
function FSMBehavior900120:Update()
...
function FSMBehavior900120:CastSkill(instanceId,skillId,SkillConfigSign,skillType)
...
function FSMBehavior900120:SetSkillFrame(skillId)
	for k = 1, #self.currentSkillList do
		if self.currentSkillList[k].id == skillId then
			--修改frame值
			self.currentSkillList[k].frame = self.myFrame + self.currentSkillList[k].cd * 21
		end
	end
end

--检测暗杀目标移除
function FSMBehavior900120:RemoveBuff(entityInstanceId, buffInstanceId,buffId)
...
function FSMBehavior900120:BeforeDie(instanceId)
...
function FSMBehavior900120:DeathEnter(instanceId,isFormationRevive)
	if instanceId==self.me then
...
function FSMBehavior900120:Die(attackInstanceId,dieInstanceId)
