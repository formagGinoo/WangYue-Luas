...
function CommonItemTip:SetItemInfo(itemInfo,callback,type)
    self.itemInfo = itemInfo
    self.itemConfig = ItemConfig.GetItemConfig(itemInfo.template_id)
    self.tipsType=type or CommonItemTip.CommonItemType.Normal
    self:UpdateBaseInfo()
...
function CommonItemTip:UpdateBaseInfo()
    ...
    for i = 1, 5, 1 do
        if self.tipsType == CommonItemTip.CommonItemType.bag then
            UtilsUI.SetActive(self["QualityBagBack".. i], i == itemConfig.quality)
        else
...
function CommonItemTip:UpdateDetailInfo()
    ...
    if showLock then
        UtilsUI.SetActive(self.HaveLock, self.itemInfo.is_locked)
        UtilsUI.SetActive(self.Unlock, not self.itemInfo.is_locked)
    end
    ...
    self.Node_Lock_btn.onClick:AddListener(function()
        ...
        mod.BagCtrl:SetItemLockState(self.itemInfo.unique_id, not self.itemInfo.is_locked, itemType)
    end)
    ...
    if itemType == BagEnum.BagType.Partner then
        ...
        self.TypeName_txt.text = talentConfig.name
    ...
    if itemType == BagEnum.BagType.Item or itemType == BagEnum.BagType.Currency then
        ...
        if self.tipsType == CommonItemTip.CommonItemType.bag then
            self.MainDesc_txt.color=Color.white
        end
        --TODO 暂时没有内容
