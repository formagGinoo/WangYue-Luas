...
function Network:SendPack()
    ...
    for i,v in ipairs(self.sendList) do
        while v.sendLen < v.maxLen and lenCount < self.maxSendLen do
            ...
            -- local protoName = protoIdToName[v.protoId]
            -- Log(currentTime, "发送协议 = ", protoName)
...
function Network:HandlerPack()
    ...
    while not self.isStopRecv and (self.maxRecvNum == 0 or recvCount < self.maxRecvNum) do
        ...
        if protoId == NetworkDefine.tickProtoId then 
            self:ResetTickTime(data)
        end

        local func = self.handlers[protoName]
        if func then xpcall(func,Network.PackError, data, protoName) end

        if mod.GmCtrl.protocolDebug and protoName and protoName ~= "resp_heartbeat" then
            local currentTime = os.date("%c")
            LogTable(currentTime .. "  协议回包信息打印 = "..protoName, data)
        end
...
function Network:SendTick(ignoreTimeout)
    ignoreTimeout = ignoreTimeout or self.isStopRecv
    local time = os.time()
    if not ignoreTimeout and self:IsTimeout(time) then return end
    self:Send(NetworkDefine.tickProtoName,{ client_time = time })
    self.curTickCount = self.curTickCount + 1
    self.lastTickTime = time
    --ping值计算
    mod.LoginCtrl.pingStart = os.clock()
end

function Network:IsTimeout(time)
    if ctx.Editor then return false end
    if self.curTickCount < 1 then return false end
...
function Network:ResetTickTime(data)
    if self.curTickCount > 0 then self.curTickCount = self.curTickCount - 1 end
    if data.client_time ~= self.lastTickTime then return LogError("心跳包时间不一致") end
    self:RestartTick()
end
