...
function BaseWindow:OpenPanel(panel, args, cacheMode)
    if not self.panelList[panel.__className] then
        self.panelList[panel.__className] = panel.New(self)
        self.panelList[panel.__className].parentWindow = self
...
function BaseWindow:TempShowWindow()
    ...
    -- 修复动效有一帧的画面闪现
    self:SetActive(true)
    UnityUtils.SetLocalScale(self.transform, 0,0,0)
...
function BaseWindow:__ShowComplete()
    ...
    if next(self.panelList) and next(self.panelsActive) then
        for k, v in pairs(self.panelsActive) do
            if v and self.panelList[k] then
