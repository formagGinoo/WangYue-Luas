...
function BasePool:Pop(poolKey)
    if not self.poolDict[poolKey] then return end

    local poolLength = self.poolDict[poolKey].length
    if poolLength <=0 then return nil end
    ...
    if poolLength >= 500 then
        LogError("too many active pool objs, count : " .. poolLength, debug.traceback())
    end
...
function BasePool:ExistCount(poolKey)
