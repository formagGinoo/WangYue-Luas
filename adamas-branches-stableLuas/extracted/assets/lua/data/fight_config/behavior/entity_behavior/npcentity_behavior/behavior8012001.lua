Behavior8012001 = BaseClass("Behavior8012001",EntityBehaviorBase)
--脉灵
function Behavior8012001.GetGenerates()
	 local generates = {}
	 return generates
end
function Behavior8012001:Init()
...
function Behavior8012001:LateInit()	
...
function Behavior8012001:Update()
...
--点击交互
function Behavior8012001:WorldInteractClick(uniqueId, instanceId)
...
--脉灵激活函数
function Behavior8012001:MailingJihuo()
...
--进入交互范围，添加交互列表
function Behavior8012001:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--退出交互范围，移除交互列表
function Behavior8012001:ExitTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--脉灵激活:显示模型，播放动作，开启镜头
function Behavior8012001:MailingActive(npcId, mailingId)
	if npcId == self.npcId then
		...
		if self.HideEffect and not self.ShowEffect then
			...
			--关卡相机
			local pos = BehaviorFunctions.GetPositionOffsetBySelf(self.me,5,0)
			self.levelCam = BehaviorFunctions.CreateEntity(80120011,nil,pos.x,pos.y,pos.z)
...
--脉灵交易完成：关闭界面
function Behavior8012001:MailingExchangeBeforeFinish(npcId,mailingId)
	if npcId == self.npcId then
		if self.isTrigger then
			BehaviorFunctions.WorldInteractRemove(self.me,self.interactUniqueId)
...
--脉灵交易完成：遁地
function Behavior8012001:MailingExchangeFinish(npcId,mailingId)
	if npcId == self.npcId then
...
--赋值
function Behavior8012001:Assignment(variable,value)
	self[variable] = value
...
--移除实体
function Behavior8012001:RemoveEntity(instanceId)
	if instanceId == self.me then
