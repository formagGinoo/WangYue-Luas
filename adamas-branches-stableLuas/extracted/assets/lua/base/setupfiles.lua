...
Global.IsLocalLuaPath = false
Global.Editor = false
if ctx then
    Global.Editor = ctx.Editor   
    if ctx.Editor then  
       Global.IsLocalLuaPath = true
    else
       Global.IsLocalLuaPath = ctx.IsLocalLuaPath     
    end
end
...
if not Global.IsLocalLuaPath then
    -- 初始化映射表
...
--编辑器模式下，动态扫描lua目录、配置目录的文件，并建立文件映射
    ...
    if ctx.Editor and CS.UnityEngine.PlayerPrefs.GetInt("LuaDebuggee") > 0 then
...
if Global.IsLocalLuaPath then
    ...
    local relactivePath = "/../../"
    if type(LuaSvrManager.Instance.RelactivePath) == "string" then
        print("relactivePath is string" .. LuaSvrManager.Instance.RelactivePath)
        relactivePath = "/.." .. LuaSvrManager.Instance.RelactivePath
    end

    local luaPath = IOUtils.GetAbsPath(Application.dataPath .. relactivePath .."lua/")
    local dataPath = IOUtils.GetAbsPath(Application.dataPath .. relactivePath .."data/lua")
