...
function SwimComponent:TryPlayEffect(pos)
	...
	--跳入水中，播水花特效
	if ctx and self.moveComponent.isAloft and not self.splashId then
...
function SwimComponent:Update()
	...
	if self.stateComponent:IsState(FightEnum.EntityState.StrideOver) or self.stateComponent:IsState(FightEnum.EntityState.Death) 
		or (self.stateComponent:IsState(FightEnum.EntityState.Climb) and self.stateComponent.stateFSM:GetSubClimbState() == FightEnum.EntityClimbState.StartClimb) then
		return
	end
	...
	if self.stateComponent:IsState(FightEnum.EntityState.Swim) then
		...
		if self.ySpeed >= 0 and self.surfaceOfWater - pos.y <= self.config.SwimHeightInterval + 0.1 then
			...
			if check and terrainHeight - pos.y > self.stopSwimOffset then
				self:Reset(FightEnum.EntityState.Move)
				return
			end
		--水下
		else
