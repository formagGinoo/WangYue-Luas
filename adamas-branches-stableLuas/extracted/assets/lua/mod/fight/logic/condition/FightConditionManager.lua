...
local HitState = FightEnum.EntityHitState
...
local CheckHitFlyState = {
	HitState.HitFly,
	HitState.HitFlyFall,
	HitState.HitFlyFallLoop,
	HitState.HitFlyHover,
	HitState.HitFlyLand,
	HitState.HitFlyUp,
	HitState.HitFlyUpLoop,
}
...
function FightConditionManager:HitCount(data, record, onlyCheck, instanceId)
	...
	if not onlyCheck then
		...
		local inHitFly = false
		if entity and entity.buffComponent and entity.buffComponent:CheckState(FightEnum.EntityBuffState.ImmuneHit) then
			immuneHit = true
		end

		for i = 1, #CheckHitFlyState do
			if BehaviorFunctions.CheckHitType(instanceId, CheckHitFlyState[i]) then
				inHitFly = true
			end
		end

		if not inHitFly and not entity.stateComponent:IsState(FightEnum.EntityState.Hit) then
			local incomeHitType = entity.stateComponent:GetIncomingHitType()
			inHitFly = incomeHitType and incomeHitType == HitState.HitFly
		end

		if not immuneHit and not inHitFly then
			record.minusCount = false
			record.count = record.count + 1
		elseif (not inHitFly and data.CountWhenSuperArmor) or (not immuneHit and data.CountWhenHitFly) then
			record.count = record.count + 1
		end
