...
function FontAnimObj:LoadAsset()
    ...
    self.gameObject = self.clientFight.assetsPool:Get(self.path,true)
    ...
    self.fontTrailGameObject = self.gameObjectTrans:Find("weak_trail")
    self.fontNessGameObject = self.gameObjectTrans:Find("weak_trail/weak_ness")
...
function FontAnimObj:PlayAnimation(npc, fontType, text, isCirt, isRestriction, attachPoint, flyPosition)
    ...
    local animName
    if isRestriction == true then
        text = "k" .. text
        animName = "pz_01"
    elseif isRestriction == false then
        text = "n" .. text
        animName = "pz_03"
    elseif isRestriction == nil then
        animName = "pz_02"
    end
    -- if isCirt then
    --     text = "b"..text
...
function FontAnimObj:PlayTipsAnimationEnd()
	self.fontAnimManager:PlayTipsAnimationEnd()
end


function FontAnimObj:Update()
