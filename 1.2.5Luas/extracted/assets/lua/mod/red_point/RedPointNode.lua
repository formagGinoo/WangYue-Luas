...
function RedPointNode:CreateNode(key, parent)
    self.key = key
    self.nodeData = RedPointConfig.GetNodeData(key)
    self.parent = parent
...
function RedPointNode:AddChild(key, state)
    if self.childMap[key] then
        LogError("添加重复结点", key)
    end
    local point = RedPointNode.New()
    point:CreateNode(key, self)
