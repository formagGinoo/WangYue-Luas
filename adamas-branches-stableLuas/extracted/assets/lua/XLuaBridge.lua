...
function XLuaBridge.EntityTriggerEnter(triggerType, objInsId, enterObjInsId, layer, name, param0, param1, param2)
	if Fight.Instance then
		local entityMgr = Fight.Instance.entityManager
		if entityMgr then
			entityMgr:OnTriggerColliderEntityEnter(triggerType, objInsId, enterObjInsId, layer, name, param0, param1, param2)
		end
	end
end
...
function XLuaBridge.RecvCallQueryEntry(data)
	local curWindow = WindowManager.Instance:GetCurWindow()
	if curWindow ~= "LoginWindow" then
		return
	end
	...
	local window = WindowManager.Instance:GetWindow("LoginWindow")
	if not window then return end
	window:RecvCallQueryEntry(data)
end