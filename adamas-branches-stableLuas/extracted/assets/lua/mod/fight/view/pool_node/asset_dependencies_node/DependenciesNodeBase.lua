...
function DependenciesNodeBase:StartLoad(loadtype)
	self.isLoading = true
	for k, v in pairs(self.nodes) do
		v:StartLoad(loadtype)
	end
	
	self.resList = self.resList or {}
	self.loadtype = loadtype or AssetLoadType.BothAsync
	...
	self.loader = AssetMgrProxy.Instance:GetLoader("DependenciesNodeBase")
	...
	self.loader:SetLoadType(loadtype)
...
function DependenciesNodeBase:LoadDone()
	...
	--Log("LoadDone", self.name)
	self.isDone = true
	self.isPoping = true

	local notWaitPop = false
	if self.loadtype == AssetLoadType.BothSync 
	or LoadPanelManager.Instance:IsLoading() 
	or CurtainManager.Instance:IsCurtain() then
		notWaitPop = true
	end

	if self.loader then
		if notWaitPop and not WorldSwitchTimeLine.Instance.switchStart then
			for k, v in pairs(self.fullResList) do
				if self.resMap[v.path] then
					local asset = self.loader:Pop(v.path)
					if asset then
						if v.type ~= AssetType.Prefab then
							AssetMgrProxy.Instance:IncreaseReferenceCount(v.path)
						end
						self.totalAssetPool:Cache(v.path,asset,v.type == AssetType.Prefab,true)
					end
					self.totalAssetPool:AddReferenceCount(v.path, true)
					self.totalAssetPool:RemoveLoadingReferenceCount(v.path)
				end
			end
			self:PopDone()
		else
...
function DependenciesNodeBase:PopDone()
	...
	if self.loader then
		AssetMgrProxy.Instance:CacheLoader(self.loader)
		self.loader = nil
	end
end
