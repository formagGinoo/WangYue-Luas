...
function GuideManager:CheckUIOpen(window, panel, nullPanel)
	...
	if window == "CanvasContainer" then
		return self.curPanel == panel
	end
	
	return self.curWindow == window and self.curPanel == panel
end
...
function GuideManager:ShowGuide(data)
	...
	local func = function()
		...
		self.clientFight.fight.taskManager:CallBehaviorFun("OnGuideStart", self.guidingData.id, data.stage)
...
function GuideManager:OnFinishGuideStage(groupId, stage)
	...
	self.clientFight.fight.taskManager:CallBehaviorFun("OnGuideFinish", groupId, stage)
	
	local groupData = self.guidingData.groupData
	if groupData.finish_stage == stage then
		self:SendFinishGuideId(groupId)
	end
	
	if stage + 1 > self.guidingData.stageAmount then
		self:ClearGuidingData()
...
function GuideManager:OnWindowOpen(name)
	...
	if self.guidingType == GuidingType.WaitForUIOpen then
		...
		if self:CheckUIOpen(stageData.window_name, stageData.panel_name, true) then
			self:DoGuideStage(self.guidingData.playStage)
			return
		end
	end
