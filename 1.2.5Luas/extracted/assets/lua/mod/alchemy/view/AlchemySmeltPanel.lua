...
function AlchemySmeltPanel:SetTotalSum(keyDir)
    ...
    if numDir == 1 then
        ...
        self.LeftSumText_txt.text = string.format("阳元素总量：%s", totalSum)
    end
    if numDir == 2 then
        totalSum = totalSum + self.addNum[2]
        self.RightSumText_txt.text = string.format("阴元素总量：%s", totalSum)
    end
...
function AlchemySmeltPanel:SetLitmitText()
    ...
    self.HasItemRuleText_txt.text = string.format("可放置材料上限：<color=%s>%s</color>/%s", AlchemyConfig.TextColor.Yellow, nowNum, self.limitNum)
end
...
function AlchemySmeltPanel:OnClick_MixItem()
    if self:CanMix() == false then
        if PanelManager.Instance:GetPanel(AlchemySmeltSelectItemPanel) then
            MsgBoxManager.Instance:ShowTips("每种材料至少放入一种才可以进行冶炼")
        else
