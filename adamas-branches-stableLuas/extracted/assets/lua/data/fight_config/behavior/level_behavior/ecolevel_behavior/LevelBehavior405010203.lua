LevelBehavior405010203 = BaseClass("LevelBehavior405010203",LevelBehaviorBase)


function LevelBehavior405010203.GetGenerates()
	local generates = {2030407,2030408,22002}
	return generates
end

function LevelBehavior405010203:__init(fight)
	self.fight = fight
end


function LevelBehavior405010203:Init()
...
function LevelBehavior405010203:Update()
	...
	if self.missionState == 0 then
		...
		--checkPos10
		local checkPos11 = BehaviorFunctions.GetTerrainPositionP("checkP11",self.levelId)
		local checkPosRot11 = BehaviorFunctions.GetTerrainRotationP("checkP11",self.levelId)
		self.checkEntity11 = BehaviorFunctions.CreateEntity(self.checkWhite, nil, checkPos11.x, checkPos11.y, checkPos11.z,checkPosRot11.x,checkPosRot11.y,checkPosRot11.z,self.levelId)
		...
		BehaviorFunctions.ShowTip(405010203)
		--设置tips参数为0
		BehaviorFunctions.ChangeSubTipsDesc(1, 405010203,self.pointNum)
		BehaviorFunctions.ChangeSubTipsDesc(2, 405010203,self.goldNum)
		self.missionState = 1
		--摄像头调用
	...
	if self.missionState == 13 and self.time - self.creatFrame > 30 then
		--创建宝箱
		BehaviorFunctions.ChangeEcoEntityCreateState(2005001119003, true)
	...
	--失败条件：超出时间限制
	local result2 = self.LevelCommon:FailCondition_TimeLimit(50)
	--将上述条件作为失败条件
	self.LevelCommon:LevelFailCondition(result2)
end

--自定义胜利条件
function LevelBehavior405010203:CustomSuccessCondition1()
...
function LevelBehavior405010203:RemoveEntity(instanceId)
...
--进入区域，并激活下一个区域
function LevelBehavior405010203:EnterArea(triggerInstanceId, areaName, logicName)
	...
	if triggerInstanceId == self.role  and areaName == "Area09" and self.missionState == 9 then
		...
		self.checkPointer = BehaviorFunctions.AddEntityGuidePointer(self.checkEntity10,FightEnum.GuideType.Transport,0,false)
		self.pointNum = self.pointNum + 1
		BehaviorFunctions.ChangeSubTipsDesc(1, 405010203,self.pointNum)
	...
	if triggerInstanceId == self.role  and areaName == "Area10" and self.missionState == 10 then
		...
		self.checkPointer = BehaviorFunctions.AddEntityGuidePointer(self.checkEntity11,FightEnum.GuideType.Transport,0,false)
		self.pointNum = self.pointNum + 1
		BehaviorFunctions.ChangeSubTipsDesc(1, 405010203,self.pointNum)
		self.missionState = 11
	end
	if triggerInstanceId == self.role  and areaName == "Area11" and self.missionState == 11 then
		BehaviorFunctions.RemoveEntity(self.checkEntity11)
		self.pointNum = self.pointNum + 1
		BehaviorFunctions.ChangeSubTipsDesc(1, 405010203,self.pointNum)
...
function LevelBehavior405010203:Remove()
