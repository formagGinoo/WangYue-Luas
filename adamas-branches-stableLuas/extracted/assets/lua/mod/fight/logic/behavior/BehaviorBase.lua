...
function BehaviorBase:__init()
	self.childBehaviors = nil
end

function BehaviorBase:SetInstanceId(instanceId, sInstanceId, behaviorMgr)
	self.instanceId = instanceId
	self.sInstanceId = sInstanceId
	self.behaviorMgr = behaviorMgr
end

function BehaviorBase:Init()
...
function BehaviorBase:SetCustomParam(list, level)
	if list and next(list) then
		if not self.customParam then
			self.customParam = {}
		end
	
		for k, v in pairs(list) do
			if not v.sValue or v.sValue == "" and v.type == FightEnum.CustomBehaviorParamType.Number then
				goto continue
			end
	
			local value = v.type == FightEnum.CustomBehaviorParamType.Bool and v.bValue or ((v.type == FightEnum.CustomBehaviorParamType.Number) and tonumber(v.sValue) or v.sValue)
			self.customParam[k] = value
...
function BehaviorBase:__delete()
	self.entity = nil
	if self.behaviorMgr then
		self.behaviorMgr:RemoveBehavior(self)
	end
end
...
function BehaviorBase:BeforeCollide(attackInstanceId,hitInstanceId,instanceId,attackType)

end

function BehaviorBase:FirstCollide(attackInstanceId,hitInstanceId,instanceId,attackType, skillType, atkElement)

end

function BehaviorBase:HitGround(attackInstanceId,instanceId,positionX, positionY, positionZ, rotationX, rotationY, rotationZ, rotationW)
...
function BehaviorBase:BeforeGetDamageParam(ownerInstanceId,hitInstanceId, attackType, damageType, atkElementType, magicId, attackInstanceId)
end

function BehaviorBase:BeforeCalculateDamage(ownerInstanceId,hitInstanceId, attackType, damageType, atkElementType, magicId, attackInstanceId)
end
...
functions BehaviorBase:BeforeCalculateCure(healer,treatee,magicId)
end
...
function BehaviorBase:Die(attackInstanceId,dieInstanceId, deathReason)

end

function BehaviorBase:DeathEnter(instanceId, deathReason)
...
function BehaviorBase:ClueUnlock(dialogId, clueId)
	
end

function BehaviorBase:ClueFullUnlock(dialogId)
	
end

function BehaviorBase:ExploreStartEvent(dialogId)
	
end

function BehaviorBase:ExploreEndEvent(dialogId, fullUnlock)
...
function BehaviorBase:PVSetAttackTarget(instanceId)
end

function BehaviorBase:TrafficLightChange(lightId, lightType)
end

function BehaviorBase:OnExitShop(shop_id)
end

function BehaviorBase:OnStealEntityGoods(goods_id,instanceId)
end

function BehaviorBase:QTEChangeRole(curInstanceId, targetInstanceId)
end

function BehaviorBase:ExitHacking()
end

function BehaviorBase:FirstTrigger(onwnerInstanceId, entityInstanceId)
end

function BehaviorBase:SkillTriggerLimitTimeOver(instanceId, keyEvent)
end

function BehaviorBase:BaraginOnExit(instanceId)
end

function BehaviorBase:CoreUI1007OnOut()
end

function BehaviorBase:BountyStarChange(start)
end

functions BehaviorBase:InformCameraBlendState(isBlend, formCamera, toCamera)
end

functions BehaviorBase:EntryPhotoFrame(instanceId)
end

functions BehaviorBase:ExitPhotoFrame(instanceId)
end

functions BehaviorBase:CheckEntityOnBuildControl(instanceId)
end

functions BehaviorBase:OnEnemyFirstInFight(instanceId)
end

functions BehaviorBase:OnBuildStateChange(instanceId, newState)
end

functions BehaviorBase:FinishLevel(levelId)
end

functions BehaviorBase:OnAttackNpc(attackInstanceId,hitInstanceId,instanceId,attackType, skillType)
end

functions BehaviorBase:OnBuildConnect(instanceId_1, instanceId_2, point1, point2)
end

functions BehaviorBase:OnBuildDisconnect(instanceId_1, instanceId_2, point1, point2, disconnectType)
end
]]