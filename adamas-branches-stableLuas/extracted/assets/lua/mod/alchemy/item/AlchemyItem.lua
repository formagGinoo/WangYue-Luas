...
function AlchemyItem:UpdateImageState()
	if self.alreadyNum > 0 then
		...
		self.commonItem.node.Num_txt.text = string.format(TI18N("已放：<color=%s>%s</color>"), AlchemyConfig.TextColor.Yellow, self.alreadyNum)
	else
		UtilsUI.SetActive(self.node.HasItem, true)
		local itemCount = mod.BagCtrl:GetItemCountById(self.AlchemyInfo.id)
		if itemCount > 0 then
			self.commonItem.node.Num_txt.text = string.format(TI18N("拥有：<color=%s>%s</color>"), AlchemyConfig.TextColor.Yellow, itemCount)
		else
			self.commonItem.node.Num_txt.text = string.format(TI18N("拥有：<color=%s>%s</color>"), AlchemyConfig.TextColor.Red, itemCount)
		end
	end
end
