Behavior2020704 = BaseClass("Behavior2020704",EntityBehaviorBase)
--NPC对话空实体
local DataItem = Config.DataItem.data_item

--预加载
function Behavior2020704.GetGenerates()
	-- local generates = {}
	-- return generates
end

function Behavior2020704:Init()
...
function Behavior2020704:LateInit()
	if not self.extraParam then
		...
		if self.extraParam and self.extraParam[1] then
			--获取额外参数的对话ID
			...
			--获取出现交互标志距离参数
			if self.extraParam[1].showDistance then
				self.showDistance = tonumber(self.extraParam[1].showDistance)
			end
		else
...
function Behavior2020704:Update()
	self.role = BehaviorFunctions.GetCtrlEntity()
	
	if self.defaultDialogId > 0 then
		--根据距离显示实体标记
		if BehaviorFunctions.GetDistanceFromTarget(self.me,self.role) < self.showDistance then
...
--进入交互范围，添加交互列表
function Behavior2020704:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
	if triggerInstanceId == self.me then
		if self.defaultDialogId	> 0 then
			
			if not BehaviorFunctions.CheckPlayerInFight() and not BehaviorFunctions.GetNowPlayingId() then
				local myPos = BehaviorFunctions.GetPositionP(self.me)
				local isInsign = BehaviorFunctions.CheckPosIsInScreen(myPos)
...
--退出交互范围，移除交互列表
function Behavior2020704:ExitTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--点击交互:播放默认对话
function Behavior2020704:WorldInteractClick(uniqueId, instanceId)
...
function Behavior2020704:StoryDialogEnd(dialogId)
