...
function AdvMainWindowV2:Update()
...
function AdvMainWindowV2:BlurShowCb()
    self:SetEffectLayer()
...
function AdvMainWindowV2:GetSubSelectTagIdx()
    if not self.args or not self.args._jump then
        return
    end
    local selectIdx = tonumber(self.args._jump[2])
    return selectIdx
end
...
function AdvMainWindowV2:DefaultSelectType()
	...
	if not defaultSelect then
        ...
        local mercenaryReward = mod.MercenaryHuntCtrl:CheckCanGetRankReward()

		if worldLevelState then
			defaultSelect = AdvType.WorldLevel
        elseif mercenaryReward then
...
function AdvMainWindowV2:__AfterExitAnim()
