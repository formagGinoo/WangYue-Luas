...
function RoleAttributePanel:__ShowComplete()
    ...
    if self.uid then
        ...
        UtilsUI.SetActive(self.PreviewButton,false)
    end
    self.parentWindow:ShieldDrag(true)
    self:UpdateData(mod.RoleCtrl:GetCurUISelectRole(),true)
    self:OnRoleModelLoad(self.heroId)

    if self.args.isOpenUpgradeWindow then
        self:OnClick_OpenRoleUpgradeWindow()
    end
end
...
function RoleAttributePanel:UpdateData(heroId, isInit)
    local hero_id = mod.RoleCtrl:GetRealRoleId(heroId)
    self.heroId = heroId
    self.curRoleInfo = mod.RoleCtrl:GetRoleData(heroId,self.uid)
    if self.uid and not self.curRoleInfo then
        return
    end
    self.curLevelLimit = Config.DataHeroStageUpgrade.Find[UtilsBase.GetStringKeys(hero_id, self.curRoleInfo.stage)].limit_hero_lev
    self:UpdateShow()
end
...
function RoleAttributePanel:GetRolePartner()
...
function RoleAttributePanel:UpdateShow()
    ...
    local hero_id = mod.RoleCtrl:GetRealRoleId(self.heroId)
    ---角色名和元素
    self.RoleName_txt.text = DataHeroMain[hero_id].name
    self.ADAMASName_txt.text = DataHeroMain[hero_id].pinyin
    UtilsUI.SetActive(self.ElementIcon, false)
    --LogError(DataHeroMain[self.heroId].name)
    SingleIconLoader.Load(self.ElementIcon, DataHeroElement[DataHeroMain[hero_id].element].element_icon_big,function()
        UtilsUI.SetActive(self.ElementIcon, true)
    end)
    ---突破和经验
    ...
    self.curLimitLevel = Config.DataHeroStageUpgrade.Find[UtilsBase.GetStringKeys(hero_id, self.curRoleInfo.stage)].limit_hero_lev
    ...
    local finalAttrMap, finalBaseAttrMap = mod.RoleCtrl:GetstaticAttr(self.heroId,self.uid)
    ...
    for i, attrInfo in ipairs(DataHeroAttrShow) do
        if attrInfo.is_show_out == 1 then
            ...
            if roleEntity and not self.uid then
                ...
                value = finalAttrMap[attrInfo.attr_id] or 0
            end
            if DataAttrsDefine[attrInfo.attr_id] and DataAttrsDefine[attrInfo.attr_id].value_type == FightEnum.AttrValueType.Percent then
                ...
                nodes.Value_txt.text = math.floor(value)
            end
    ...
    ---好感度和介绍
    self.WorldDescText_txt.text = DataHeroMain[hero_id].world_desc

    self.LevelUpButton:SetActive(not self.uid and true)
    ---升级/突破按钮
    ---机器人不显示升级和突破按钮
    if self.curRoleInfo.isRobot or self.uid then
        UtilsUI.SetActive(self.LevelUpButton,false)
        UtilsUI.SetActive(self.PreviewButton,false)
        return
    else
        UtilsUI.SetActive(self.PreviewButton,true)
    end
    ...
    if self.curRoleInfo.lev < self.curLimitLevel then
        ...
        if stageUpInfo == nil and lv > self.curRoleInfo.lev then
        ...
        self.LevelUpButton:SetActive(not self.uid and stage)
        local isPass, desc
        ...
        if stageUpInfo.need_gold > haveGold then isPass = false end
        self.LvRedPoint:SetActive(isPass)
    end
end
...
function RoleAttributePanel:OnClick_OpenRoleAttributeDetailPanel()
    self.parentWindow:OpenPanel(RoleAttributeDetailPanel, { heroId = self.heroId ,uid = self.uid})
end
...
function RoleAttributePanel:__AfterExitAnim()
