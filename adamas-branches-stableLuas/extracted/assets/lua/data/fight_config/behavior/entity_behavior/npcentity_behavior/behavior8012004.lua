...
function Behavior8012004:Update()
	...
	if self.myState == self.myStateEnum.Default then
		...
		BehaviorFunctions.DoMagic(1,self.me,900000022) --强制左倾受击
		self.myState = self.myStateEnum.Initial
	
	--初始化进入休闲状态
	elseif self.myState == self.myStateEnum.Initial then
	...
	--如果处于休闲状态下
	elseif self.myState == self.myStateEnum.Leisure then
		self:DefaultAct()
	...
	elseif self.myState == self.myStateEnum.Escape then
		...
		if result then
			self:Vanish()
		elseif self.escapeFrame < self.myFrame then
		...
		if self.skillState == self.SkillStateEnum.Default then
			...
			elseif self.battleTargetDistance >= self.skillList[1].minDistance and
				self.battleTargetDistance < self.skillList[1].maxDistance then
					self.skillState = self.SkillStateEnum.Ready
			end
		elseif self.skillState == self.SkillStateEnum.Ready then
...
function Behavior8012004:LieAct()
	if self.myState == self.myStateEnum.Initial and self.isBornLie == true then
...
function Behavior8012004:ShouldEnterFight()
	if self.myState == self.myStateEnum.Leisure then
		...
		if distance < 20 and self.canAttack == true and self.skillState == self.SkillStateEnum.Default then
...
function Behavior8012004:Hit(attackInstanceId,hitInstanceId,hitType)
	if hitInstanceId == self.me and attackInstanceId == self.role and self.skillState == self.SkillStateEnum.Default then
		self.myState = self.myStateEnum.Fight
		local fxEntityDanger = BehaviorFunctions.CreateEntity(80120040103, self.me)
	end
end
...
function Behavior8012004:FinishSkill(instanceId,skillId,SkillConfigSign,skillType)
	if instanceId == self.me and skillId == 801200401 then	
		self.myState = self.myStateEnum.Escape
	end
end

function Behavior8012004:BreakSkill(instanceId,skillId,SkillConfigSign,skillType)
	if instanceId == self.me and skillId == 801200401 then
		self.myState = self.myStateEnum.Escape
	end
end