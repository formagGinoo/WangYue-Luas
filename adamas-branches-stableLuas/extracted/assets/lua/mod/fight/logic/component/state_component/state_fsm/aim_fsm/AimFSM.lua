...
local AimTargets = {FightEnum.EntityNpcTag.Monster}
...
function AimFSM:Init(fight, entity)
    ...
    self.aimAnimEndMouseInput = true
    self:InitStates()
end
...
function AimFSM:OnEnter()
    ...
    self.entity.clientIkComponent:SetLookFinalIKTarget(self.aimTargetTrans, self.aimTargetTrans)
...
function AimFSM:EnableWeakEffect(enable)
    if self.enableWeakEffect == enable then
        return
    end

    self.enableWeakEffect = enable
    local entites = self.fight.entityManager:GetEntitysByTags(AimTargets)
    if entites then
        for k, v in pairs(entites) do
            if v.tagComponent.camp == FightEnum.EntityCamp.Camp2 then
                if enable then
                    v.partComponent:CreateWeakEffect()
                else
                    v.partComponent:DestroyWeakEffect()
                end
            end 
        end
    end
...
function AimFSM:LateInit()
	...
	if self.entity.attrComponent then
        self.walkSpeed = self.entity.attrComponent:GetValue(EntityAttrsConfig.AttrType.WalkSpeed)
	else
		self.walkSpeed = 0
	end
end
...
function AimFSM:Update()
    ...
    if self.canMove and moveEvent and self.walkSpeed > 0 then
        ...
        local speed = self.walkSpeed * FightUtil.deltaTimeSecond * timeScale
...
function AimFSM:OnLeave()
    ...
    self.entity.clientIkComponent:StopHandAim()
...
function AimFSM:CameraAimEnd(time)
    ...
    self.targetDotween = DOTween.Sequence()
    self.targetDotween:Append(self.cameraTarget:DOLocalMove(self.cameraTargetOrginPos, time))
    if self.aimAnimEndMouseInput then
        self.targetDotween:AppendCallback(endCallBack)
    else
       InputManager.Instance:SetCameraMouseInput(false)     
    end
    CustomUnityUtils.SetAimTarget(self.transform, nil)  
    self.entity.clientIkComponent:SetLookFinalIKTarget()
end


function AimFSM:ShootMissile(missleMoveEntityId, transformName)
    missleMoveEntityId = missleMoveEntityId
    if not missleMoveEntityId then
        LogError("missleMoveEntityId is nil")
        return
    end
    ...
    missileMoveEntity.rotateComponent:Async()
...
function AimFSM:UpdateSetAimUITarget()
    ...
    local screenWidth = GameSetCtrl.ScreenW or Screen.width
    local screenHeight = GameSetCtrl.ScreenH or Screen.height
    local hitList = CustomUnityUtils.GetScreenRayWorldHits(CameraManager.Instance.mainCameraComponent, 
       screenWidth * 0.5, screenHeight * 0.5, AimParam.AimMaxDistance, FightEnum.LayerBit.Entity)
    ...
    if hitList and hitList.Length > 0 then
        for i = 0, hitList.Length - 1 do
            ...
            if entity and entity.tagComponent
                and entity.tagComponent.camp ~= self.entity.tagComponent.camp and entity.partComponent
                and entity.tagComponent and entity.tagComponent.tag ~= FightEnum.EntityTag.Bullet then
...
function AimFSM:RotateToLockTarget(targetTransform, speedX, speedY)
    ...
    local y1 = self.entity.clientTransformComponent.eulerAngles.y
    if speedX then
    ...
    self.entity.clientTransformComponent:UpdateRotate(1)
    self.lockTargetMoveY = self.entity.clientTransformComponent.eulerAngles.y - y1
