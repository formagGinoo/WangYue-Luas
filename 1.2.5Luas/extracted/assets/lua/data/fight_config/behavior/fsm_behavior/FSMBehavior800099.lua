...
--点击交互:播放默认对话
function FSMBehavior800099:WorldInteractClick(uniqueId)
...
--进入交互范围，添加交互列表
function FSMBehavior800099:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--退出交互范围，移除交互列表
function FSMBehavior800099:ExitTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
...
--对话结束
function FSMBehavior800099:StoryEndEvent(dialogId)
...
--巡逻
function FSMBehavior800099:Patrol()
...
function FSMBehavior800099:PathFindingBegin()
	local nextPosition = BehaviorFunctions.GetTerrainPositionP(self.patrolList[self.patrolNum].posName,10020001,self.logicName)
...
function FSMBehavior800099:NextPatrolNum()
	--获取下一个目标点
	--不会折返
...
--受击
function FSMBehavior800099:FirstCollide(attackInstanceId,hitInstanceId,instanceId)
...
--移动碰撞
function FSMBehavior800099:OnEntityCollision(instanceId,collisionInstanceId)
...
--移动碰撞条件
function FSMBehavior800099:BeCollideRule()
...
--位移转身
function FSMBehavior800099:Turn(pos,lookPos)
...
--默认动作轮播
function FSMBehavior800099:DefaultAct()
...
--对话动作轮播
function FSMBehavior800099:TalkAct()
...
--气泡对话
function FSMBehavior800099:Bubble()
	--设置气泡对话内容
...
--注视IK
function FSMBehavior800099:CanLook(Can)

end

--解析patrolList
function FSMBehavior800099:GetPatrolList(list)
...
--解析AniList
function FSMBehavior800099:GetAniList(list)
	local beforeList = list
	local afterList = {}
	for i = 1, #beforeList do
		local stepList = self:SplitParam(beforeList[i],"-")
...
--拆分字符串
function FSMBehavior800099:SplitParam(string,sep)
	local list = {}
	if string == nil then
		return nil
	end
...
--赋值
function FSMBehavior800099:Assignment(variable,value)
	self[variable] = value
