LevelBehavior200503012 = BaseClass("LevelBehavior200503012",LevelBehaviorBase)
--NPC搬运玩法1

function LevelBehavior200503012:__init(fight)
	self.fight = fight
end

function LevelBehavior200503012.GetGenerates()
	local generates = {808012004, 8011003, 2080109, 2080102, 2041201, 2041202, 2080103, 2080106, 2080107}
	return generates
end

function LevelBehavior200503012:Init()
	...
	self.lookAtList = {
		...
		[5] = {entityId = 2001, bornPos = "tishi1",  id = nil},
	}
...
function LevelBehavior200503012:Update()
...
--创建实体
function LevelBehavior200503012:CreateActor(list)
...
--空实体创建
function LevelBehavior200503012:LookAtInstance()
...
--移除收集物函数
function LevelBehavior200503012:ResetLevel()
...
--黑幕移除物品
function LevelBehavior200503012:Black()
...
--黑幕移除物品
function LevelBehavior200503012:BlackStart()
...
--失败传送
function LevelBehavior200503012:FailTransfer(position,lookAtPos)
...
--镜头调用
function LevelBehavior200503012:levelCamera(type,cameraInstanceId,target)
...
--引导管理
function LevelBehavior200503012:guide()
...
--关卡开始
function LevelBehavior200503012:LevelStart()
...
--关卡胜利判断
function LevelBehavior200503012:LevelSuccessCondition(...)
	local condition = {...}
...
--自定义关卡胜利条件1：收集物全收集
function LevelBehavior200503012:SuccessCondition_Collection(num)
	--区域内收集物数量重置
	self.inareaNum = 0
	self.collectionCtrlState = false
	if self.collectionList then
	...
	if self.collectionList then
		for k,v in ipairs(self.collectionList) do
			local distance = BehaviorFunctions.GetDistanceFromTargetWithY(self.lookAtList[5].id,v.id)
			if distance then
				if distance <= 4 then
					v.state = self.collectionStateEnum.inarea
				end
				if distance > 4 and distance <= 80 then
...
--关卡失败判断
function LevelBehavior200503012:LevelFailCondition(...)
	local condition = {...}
	--计算完成条件的数量
	for i,v in ipairs(condition) do
		if v == true then
...
function LevelBehavior200503012:FailCondition_Distance1(pos,warndistance,faildistance,timeLimit)
	local characterPos = BehaviorFunctions.GetPositionP(self.role)
...
function LevelBehavior200503012:FailCondition_Distance2(timeLimit)
	--重置状态收集物计数
	self.resetNPCNum = 0
	self.collectionCtrlState = false
	for k,v in ipairs(self.collectionList) do
...
--function LevelBehavior200503012:OnBuildDisconnect(instanceId_1, instanceId_2, point1, point2, disconnectType)
...
--关卡状态机
function LevelBehavior200503012:ChallengeLevelFSM()
	--关卡处于默认状态时
	if self.levelState == self.levelStateEnum.Default then
...
--赋值
function LevelBehavior200503012:Assignment(variable,value)
	self[variable] = value
end
...
--提示光圈回调
function LevelBehavior200503012:EnterArea(triggerInstanceId, areaName)
	for k,v in ipairs(self.collectionList) do
...
--交互实体回调
function LevelBehavior200503012:WorldInteractClick(uniqueId,instanceId)
	--初次对话阶段按钮
...
--教学关闭回调
function LevelBehavior200503012:LevelInstructionComplete(tipld)
...
--对话节点
function LevelBehavior200503012:StoryPassEvent(dialogId)
		--关卡加载后初次交互
...
--节奏控制
function LevelBehavior200503012:StoryEndEvent(dialogId)
	--自定义失败条件2：NPC超出距离限制
