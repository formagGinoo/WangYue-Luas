Behavior900130 = BaseClass("Behavior900130",EntityBehaviorBase)
--资源预加载
function Behavior900130.GetGenerates()
	local generates = {}
	return generates
end

function Behavior900130.GetMagics()
	local generates = {900000024,900000025}
	return generates
end


function Behavior900130:Init()
	self.mission = 0
	self.beAssassin = 90013005
	self.backHited = 90013006
	...
	self.MonsterCommonParam.warnLimitRange = 5           --直接警告距离
	...
	self.MonsterCommonParam.warnSkillId = 90013003            --警告技能Id
	self.MonsterCommonParam.warnSitSkillId = 90013004            --坐下后起立警告技能Id
	self.MonsterCommonParam.tauntSkillId = nil            --嘲讽技能
	self.MonsterCommonParam.noWarnInFightRange = 40        --自如其名
	--MonsterCastSkill
	...
	--MonsterWander
	self.MonsterCommonParam.shortRange = 8                 --游荡近距离边界值（小于此距离往后走）
	self.MonsterCommonParam.longRange = 15                 --游荡远距离边界值（小于此距离前走或左右走）
	...
	self.MonsterCommonParam.LRWalkSwitchTime = 3.334        --左右走切换时间
	self.MonsterCommonParam.switchDelayTime = 1.333         --延迟切换时间(前后走)
	self.MonsterCommonParam.walkDazeTime = 1.333               --移动发呆时间（配stand2时间）
	...
	self.MonsterCommonParam.monsterRangeRadius = 3							   --以自身为圆心的一个范围，其他怪物无法wander进来
...
function Behavior900130:Update()
	--可暗杀
...
function Behavior900130:DeathEnter(instanceId,isFormationRevive)
...
function Behavior900130:Die(attackInstanceId,dieInstanceId)
...
--function Behavior900130:WalkBack()
...
--function Behavior900130:FightIdle()
	--if  self.MonsterCommonParam.inFight == true and BehaviorFunctions.CanCtrl(self.MonsterCommonParam.me) then
		--if self.MonsterCommonParam.skillState == self.MonsterCommonParam.SkillStateEnum.Default
...
--function Behavior900130:RunBack()
	--if  self.MonsterCommonParam.inFight == true and BehaviorFunctions.CanCtrl(self.MonsterCommonParam.me) then
