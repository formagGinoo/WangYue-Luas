...
function DebugSettings:__CacheObject()
	...
	self.BlockLodDistConfirm_btn.onClick:AddListener(self:ToFunc("BlockLodDistConfirmFun"))
...
function DebugSettings:TranportConfirmFun()
	...
	if idx == 10 then
        ...
        WorldSwitchTimeLine.Instance:OnEnter(10020001)
        return
	end

	if idx then
		local posInfo = transposList[idx]
		local mapId = transposMapId[idx] or 10020001
		BehaviorFunctions.Transport(mapId,posInfo[1], posInfo[2], posInfo[3])
...
function DebugSettings:BlockLodDistConfirmFun()
	local txt = self.BlockLodDistInput:GetComponent(TMP_InputField).text
	local list = self:SplitParam(txt, ",")
	if list then
		if #list == 1 then
			CS.SceneBlockManger.Instance:SetLodShowSize(10000, 0, 0, 0)
		elseif #list == 4 then
			CS.SceneBlockManger.Instance:SetLodShowSize(list[1], list[2], list[3], list[4])
		end
	end
end
