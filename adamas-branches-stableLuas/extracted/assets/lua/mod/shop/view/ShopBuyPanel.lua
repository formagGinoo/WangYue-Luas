...
function ShopBuyPanel:__BindEvent()
    ...
    EventMgr.Instance:AddListener(EventName.ChangeLanguage,self:ToFunc("ChangeLanguage"))
end
...
function ShopBuyPanel:__delete()
    ...
    EventMgr.Instance:RemoveListener(EventName.ChangeLanguage,self:ToFunc("ChangeLanguage"))
...
function ShopBuyPanel:__Show(args)
    self.TitleText_txt.text = TI18N("购买物品")
    ...
    for key, value in ipairs(self.goodsList) do
...
function ShopBuyPanel:UpdateLimitBuy(goodsItem)
    ...
    if not (goodsItem.buy_limit == 0)  then
        self.LimitTxt_txt.text =  string.format(TI18N("限购%s/%s"),goodsItem.buy_limit - goodsItem.buy_count,goodsItem.buy_limit)
    else
...
function ShopBuyPanel:UpDateTime(goodsItem)
    ...
    if goodsItem.refresh_id ~= 0 then
        ...
        --     self.TimeLeftTxt_txt.text = string.format(TI18N("%s天%s时"), resettime.days, resettime.hours)
        -- else
        --     self.TimeLeftTxt_txt.text = string.format(TI18N("%s时%s分"), resettime.hours, resettime.minutes)
        -- end
...
function ShopBuyPanel:UpdataBuyInfo(goodsItem, itemConfig)
    ...
    self.itemPrice = math.floor(goodsItem.consume[1].value * self.disCount / 10000)
...
function ShopBuyPanel:OnClick_ConfirmBuyItem()
    ...
    if self.args.shopId == 1006 then 
        EventMgr.Instance:Fire(EventName.CloseEntityShop)
    end
...
function ShopBuyPanel:SetText()
    if self.resettime == nil then
        self.TimeLeftTxt_txt.text = ""
        self.TimeLeftIcon:SetActive(false)
        return 
    end
    if self.resettime.days > 0 then
        self.TimeLeftTxt_txt.text = string.format(TI18N("%s天%s时"), self.resettime.days, self.resettime.hours)
    else
        self.TimeLeftTxt_txt.text = string.format(TI18N("%s时%s分"), self.resettime.hours, self.resettime.minutes)
    end
    self.TimeLeftIcon:SetActive(true)
end

function ShopBuyPanel:SetArtText()
    
end

function ShopBuyPanel:ChangeLanguage()
    self:SetText()
    self:SetArtText()  -- 艺术字
end
