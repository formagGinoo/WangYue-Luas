LevelBehavior404010602 = BaseClass("LevelBehavior404010602",LevelBehaviorBase)

function LevelBehavior404010602:__init(fight)
	self.fight = fight
...
function LevelBehavior404010602.GetGenerates()
	local generates = {790007100,808011001}
	return generates
end

function LevelBehavior404010602.GetMagics()
	local generates = {900000001,900000007,900000013,900000020,900000022,900000023}
	return generates
end

function LevelBehavior404010602:Init()
	--肉鸽事件信息获取
	...
	self.monsterentityid = 790007100
	self.mapname = Logic404010602
	self.mappositionid = 404010602
	self.wave = 1                        --总波次
	----可修改参数 ↑-----
...
function LevelBehavior404010602:Update()
	self.role = BehaviorFunctions.GetCtrlEntity()
	self.time = BehaviorFunctions.GetFightFrame()
	self.realtime = BehaviorFunctions.GetFightFrame()/30
	-- print("LevelBehavior404010602初始化")
...
function LevelBehavior404010602:__delete()
end

-------------------------函数--------------------------------

-- -- 更新距离函数，该函数直接对比 self.range， self.range<=30 , 30 < self.range <= 50, self.range > 50
-- function LevelBehavior404010602:UpdateRange()
...
-- --开场自定义功能函数
-- function LevelBehavior404010602:CustomLevelFunctions()
--     --如果需要开场图文教学
...
--创怪函数
function LevelBehavior404010602:CreateMonster(wave)
	--print("创怪物函数")
...
----npc受伤函数
function LevelBehavior404010602:FirstCollide(attackInstanceId,hitInstanceId,instanceId)
	--print("进入受伤函数")
...
--卸载内容
function LevelBehavior404010602:Leave()
	--移除相机
...
-- --关卡结束函数
-- function LevelBehavior404010602:LevelFinish()
--     BehaviorFunctions.Victory()
-- end

--设置攻击目标
function LevelBehavior404010602:ChangeTarget(monsterList)
...
--关卡胜利表现
function LevelBehavior404010602:Victory()
...
--关卡失败表现
function LevelBehavior404010602:Fail()
	BehaviorFunctions.HideTip(32000007) --隐藏敌人数量的tips
...
--相机函数
function LevelBehavior404010602:LevelLookAtPos(pos,logic,type,bindTransform)
...
---------------------回调----------------------------------
--结尾对话播放完毕
function LevelBehavior404010602:StoryEndEvent(dialogId)
...
--死亡回调
function LevelBehavior404010602:Death(instanceId,isFormationRevive)
...
--任务前置肉鸽追踪指标，以封装函数，不管
function LevelBehavior404010602:RogueGuidePointer(guidePos,guideDistance,guideType)
...
----随机获取怪物，并播放攻击动画--函数
function LevelBehavior404010602:GetRandomAliveMonster()
    local aliveMonsters = {}
    -- 筛选出活着的怪物
    for i, v in pairs(self.monsterList) do
...
function LevelBehavior404010602:MakeRandomMonsterAttack()
    local monster = self:GetRandomAliveMonster()
    if monster then
        BehaviorFunctions.PlayAnimation(monster.instanceId, "Attack003")
    end
end