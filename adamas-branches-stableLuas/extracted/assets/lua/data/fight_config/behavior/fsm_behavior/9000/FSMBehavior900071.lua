FSMBehavior900071 = BaseClass("FSMBehavior900071",FSMBehaviorBase)
--资源预加载
function FSMBehavior900071.GetGenerates()
	local generates = {}
	return generates
end
--mgaic预加载
function FSMBehavior900071.GetMagics()
	local generates = {900000024,900000025,900000107}
	return generates
end

function FSMBehavior900071:Init()
	...
	self.warnSkillId = 900071901            --警告技能Id
	...
	--MonsterWander
	self.shortRange = 2.5				--游荡近距离边界值
	self.longRange = 5                 --游荡远距离边界值
	self.maxRange = 20                  --游荡超远距离边界值
	self.minRange = 2.2                   --极限近身距离，追杀模式最小追踪距离
	self.canLRWalk = true               --左右走开关
	self.LRWalkSwitchTime = 1.2		--左右走切换时间
	self.switchDelayTime = 1.2		--延迟切换时间(前后走)
	self.walkDazeTime = 0			--移动发呆时间
	self.canRun = true                  --跑步开关
	self.haveRunAndHit = true           --是否有追杀模式(↓↓跑到脸上放defaultSkill↓↓)
	self.defaultSkillId = 90007101		--默认技能id，追杀模式使用
...
function FSMBehavior900071:LateInit()
...
function FSMBehavior900071:Update()
...
function FSMBehavior900071:DeathEnter(instanceId,isFormationRevive)
...
function FSMBehavior900071:Die(attackInstanceId,dieInstanceId)
...
--检测暗杀目标移除
function FSMBehavior900071:RemoveBuff(entityInstanceId, buffInstanceId,buffId)
...
function FSMBehavior900071:BeforeDie(instanceId)
	if instanceId == self.me then
...
--检测实体标记
function FSMBehavior900071:HasEntitySign()
	if self.battleTarget and self.me then
		--抛沙后撤派生
		if BF.HasEntitySign(self.me,900071041) then
			local L = BF.GetDistanceFromTarget(self.me,self.battleTarget)
			--近距离派生
			if L <= 5 then
				--判断派生技能表是否为空
				if next(self.ComboSkillList) then
					--释放表里的第一个技能
					BF.CastSkillByTarget(self.me,self.ComboSkillList[1],self.battleTarget)
					--第一个技能加CD
				...
				BF.CastSkillByTarget(self.me,90007105,self.battleTarget)
		...
		--self.x,self.y,self.z = BehaviorFunctions.GetEntityTransformPos(self.battleTarget)
...
function FSMBehavior900071:SerchSkillList(skillid,table)
	for i = 1,#table do
...
--修改技能frame值
function FSMBehavior900071:SetSkillFrame(skillId)
	--找到这个技能
...
----命中时判断
--function FSMBehavior900071:Collide(attackInstanceId,hitInstanceId,instanceId,shakeStrenRatio,attackType,camp)
...
function FSMBehavior900071:StunAvoid()
	local S = BF.GetEntityState(self.me)
	local i = self:SerchSkillList(90007104,self.initialSkillList)

	if S == 14 and self.state == 0 then
		self.state = 1
		self.initialSkillList[i].priority = 2
	elseif S ~= 14 and self.state == 1 then
		if BF.GetSkill(self.me) == 90007105 then
			self.initialSkillList[i].priority = 1
			self.state = 0
		end
	end
end