...
function PasvComponent:Update()
    ...
    for iter in self.execPasvs:Items() do
        ...
        if pasvRunData.frame > pasvRunData.totalFrame then
            ...
            local skillLevel = self.skillComponent:GetSkillLevel(pasvRunData.pasvId)
            skillLevel = skillLevel or self.entity:GetDefaultSkillLevel()
            pasvRunData.frameEvent:DoFrameEvent(pasvRunData.frame, skillLevel)
        end
    end
end
...
function PasvComponent:CastPasv(pasvId,args)
    ...
    local skillLevel = self.skillComponent:GetSkillLevel(pasvRunData.pasvId)
    skillLevel = skillLevel or self.entity:GetDefaultSkillLevel()
	pasvRunData.frameEvent:DoFrameEvent(0,skillLevel)
end
