...
function TabGroup:Clean()
    EventMgr.Instance:RemoveListener(EventName.role_level_change, self.listener)
    self.gameObject = nil
    self.transform = nil
    self.rect = nil
    self.callback = nil
...
function TabGroup:Init()
    ...
    for i = 1, childcount do
        ...
        if text_transform ~= nil then tab["text"] = UtilsBase.GetText(text_transform.gameObject) end
        table.insert(self.buttonTab, tab)
    end

    if #self.openLevel > 0 then
        self:Layout()
        EventMgr.Instance:AddListener(EventName.role_level_change, self.listener)
    end
