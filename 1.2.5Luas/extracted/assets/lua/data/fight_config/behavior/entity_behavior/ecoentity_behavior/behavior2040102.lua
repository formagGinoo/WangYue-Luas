...
function Behavior2040102:WorldInteractClick(uniqueId)
	for index,value in ipairs(self.objectIdentity) do
		if uniqueId == self.objectIdentity[index].uniqueID then
			...
			BehaviorFunctions.WorldInteractRemove(self.objectIdentity[index].uniqueID)
		...
		if ecoID ~= nil then
			for index,value in ipairs(self.objectIdentity) do
				if self.objectIdentity[index].ecoID == ecoID and self.objectIdentity[index].interactState == false then
					--self.objectIdentity[index].uniqueID = BehaviorFunctions.WorldInteractActive(WorldEnum.InteractType.Item,200001,self.objectIdentity[index].name, 1)
