TaskBehavior101020102 = BaseClass("TaskBehavior101020102")
--传送至仿古街

function TaskBehavior101020102.GetGenerates()
	local generates = {}
	return generates
end

function TaskBehavior101020102:__init(taskInfo)
	...
	self.dianxiang =
	{
		[1] = {insId = nil , ecoId = 2003001000015 , beenHit = nil ,inArea = true},
	}
...
function TaskBehavior101020102:Update()
	self.role = BehaviorFunctions.GetCtrlEntity()

	--任务开始时检查玩家是否处于Area内
	--if self.inArea == nil then
	--if BehaviorFunctions.CheckEntityInArea(self.role,"Task_main_01","Task101020102Area01")
	--or	BehaviorFunctions.CheckEntityInArea(self.role,"Task_main_01","Task101020102Area02") then
	--if BehaviorFunctions.CheckEntityInArea(self.role,"Task_main_01","Task101020102Area02")then
		--self.inArea = true
	--else
		--self.inArea = false
	--end
	--end
	if self.taskFinish == false and BehaviorFunctions.GetEcoEntityState(2003001000015) == 1 then
		BehaviorFunctions.AddDelayCallByFrame(60,BehaviorFunctions,BehaviorFunctions.SendTaskProgress,self.taskId,self.taskStepId,1)
	...
	----BehaviorFunctions.SendTaskProgress(101020102,1,1)
	--elseif v.ecoId == 2003001010003 then
	----BehaviorFunctions.SendTaskProgress(101020102,2,1)
	--BehaviorFunctions.SendTaskProgress(101020102,1,1)
	--end
	--v.beenHit = true
	--end
...
--进入区域
function TaskBehavior101020102:EnterArea(triggerInstanceId, areaName, logicName)

end

--退出区域
function TaskBehavior101020102:ExitArea(triggerInstanceId, areaName, logicName)

end

--进入骇入模式
function TaskBehavior101020102:Hacking(instanceId)
...
--退出骇入模式
function TaskBehavior101020102:StopHacking(instanceId)
	if instanceId == self.dianxiang[1].insId then
		self:RemoveWeakGuide()
...
function TaskBehavior101020102:HackTutorial()
	----先获取当前角色的对应仲魔
	...
	--if partnerIns then
		...
		--else
			--if self.weakGuide[4].state == false then
				----当前未装备箴石之劣，请装备箴石之劣
				--BehaviorFunctions.ShowTip(101020102)
...
--开启弱引导，并且关闭其他弱引导
function TaskBehavior101020102:WeakGuide(guideId)
	local result = false
...
--关闭所有弱引导
function TaskBehavior101020102:RemoveWeakGuide()
	for i,v in ipairs(self.weakGuide) do
		BehaviorFunctions.FinishGuide(v.Id,1)
	end
end
...
--关卡看向实体相机
function TaskBehavior101020102:LevelLookAtInstance(instanceId,type,frame,bindTransform,targetbindTransform)
...
--赋值
function TaskBehavior101020102:Assignment(variable,value)
	self[variable] = value
end

function TaskBehavior101020102:HackingClickUp(instanceId)
	if instanceId == self.dianxiang[1].insId then
		self:RemoveWeakGuide()
	end
end
