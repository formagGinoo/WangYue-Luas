...
function RoleUpgradeTipPanel:__BindListener()
    --UtilsUI.SetHideCallBack(self.RoleUpgradeTipPanel_out, self:ToFunc("OnCloseHideCallBack"))
    self:BindCloseBtn(self.Close_btn, self:ToFunc("OnClickCloseButton"))
end
...
function RoleUpgradeTipPanel:showAttributeChange()
    --目前养成系统只有升级，所以属性值仅统计等级和突破带来的基础属性，后续增加武器和月灵的统计
...
function RoleUpgradeTipPanel:OnClickCloseButton()
