...
function Behavior2020301:WorldInteractClick(uniqueId,instanceId)--点击交互列表
	local id = BehaviorFunctions.GetEntityTemplateId(instanceId)
	if id == 2020301 then
		BehaviorFunctions.OpenSceneMsgWindow(1,self.dropentity) --交互事件
		
		--BehaviorFunctions.WorldInteractRemove(self.me,self.interactUniqueId)--移除交互按钮
	end
end
--function Behavior2020301:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)--进入交互范围
	...
	----if not self.opentime or (self.opentime and self.time> self.opentime + 0.5) then--没打开过或者打开过了0.5秒
		----self.interactUniqueId = BehaviorFunctions.WorldInteractActive(self.me,WorldEnum.InteractType.Talk,200001,"查看", 1)--物品类型，图标，文字，品质
	----end
