...
function WeakLockingCamera:AddFollowTarget(target)
	self.targetGroup:AddMember(target,1,1)
	--self.groupCount = self.groupCount + 1
end

function WeakLockingCamera:RemoveAllLookAtTarget()
	self.lookAtTargetGroup:RemoveAllMember()
end

function WeakLockingCamera:RemoveAllFollowTarget()
	self.targetGroup:RemoveAllMember()
end

function WeakLockingCamera:RemoveFollowTarget(target)
	self.targetGroup:RemoveMember(target)
	--self.groupCount = self.groupCount - 1
end

function WeakLockingCamera:SetFollowTargetWeight(target,weight)
	self.targetGroup:SetMemberWeight(target,weight)
end

function WeakLockingCamera:SetGroupPositionMode(mode)
	self.targetGroup.m_PositionMode = mode
end

function WeakLockingCamera:AddLookAtTarget(target)
	self.lookAtTargetGroup:AddMember(target,1,1)
end

function WeakLockingCamera:RemoveLookAtTarget(target)
	self.lookAtTargetGroup:RemoveMember(target)
end

function WeakLockingCamera:SetLookAtTargetWeight(target,weight)
...
function WeakLockingCamera:UpdateVAxisTemp()
	--do
		--return
	--end
	local entity = BehaviorFunctions.GetEntity(BehaviorFunctions.GetCtrlEntity())
	if not BehaviorFunctions.HasEntitySign(entity.instanceId,10000033) then
		return
	end
	...
	local curValue = axisV.Value
	local force = false
	--self.lockingPOV.m_VerticalAxis = axisV
	if targetPoint.y < 0.4 then
		curValue = curValue + 10 * (0.4 - targetPoint.y)
		force = true
		--Log("1111111")
	elseif targetPoint.y > 0.8  then
		curValue = curValue - 10 * (targetPoint.y - 0.8)
		force = true
		--Log("2222222")
	else
		curValue = curValue - (curValue - 8) * 0.1
		--Log("33333")
	end
...
function WeakLockingCamera:Update(lerpTime)
	if UtilsBase.IsNull(self.mainTarget) then
		return
	end
	...
	local force = self:UpdateVAxisTemp()
	...
	if not force and self.cameraManager.cinemachineBrain.IsBlending then
...
function WeakLockingCamera:AfterUpdate(isNotState)
	if UtilsBase.IsNull(self.target) or UtilsBase.IsNull(self.mainTarget) then
		return
	end
	self.targetGroup.transform.position = self.mainTarget.position
...
function WeakLockingCamera:SetGlideCameraParams(id)
...
function WeakLockingCamera:SetInheritPosition(isInherit)
...
function WeakLockingCamera:SetCameraDistance(distance)
	self.framingTransposer.m_CameraDistance = distance
end

function WeakLockingCamera:GetCurCameraDistance()
