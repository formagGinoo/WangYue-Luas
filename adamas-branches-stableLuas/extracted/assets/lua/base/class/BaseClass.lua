...
function BaseClass(className,...)
	...
	class_type.New = function(...)
		...
		obj.DeleteMe = function(self)
			...
			local now_supers = {self._class_type}
			local _delete
			_delete = function(supers, obj)
				for i, v in ipairs(supers) do
					if v.__delete then
						v.__delete(obj)
					end
					_delete(v.supers, obj) 
				end
			end
			_delete(now_supers, self)
		...
		--
		local _superFunc
		_superFunc = function(c,fn,flag, dirtyMap, ...)
			--TODO 曲线救国 dirtyMap防止方法被多次调用
			if flag then
				for i, v in ipairs(c.supers) do _superFunc(v, fn, flag, dirtyMap, ...) end
				if c[fn] and not dirtyMap[c[fn]] then
					dirtyMap[c[fn]] = true
					c[fn](obj, ...) 
				end
			else
		...
		obj.SuperFunc = function(self,fn,flag,...)
			local dirtyMap = {}
			_superFunc(class_type,fn,flag, dirtyMap, ...)
		end

		local _baseFunc
		_baseFunc = function(c, fn, ...)
			for i, v in ipairs(c.supers) do
				if v[fn] then 
					v[fn](obj, ...)
				else
					_baseFunc(v, fn, ...)
