LevelBehavior405010202 = BaseClass("LevelBehavior405010202",LevelBehaviorBase)


function LevelBehavior405010202.GetGenerates()
	local generates = {2030407,2030408,22002}
	return generates
end

function LevelBehavior405010202:__init(fight)
	self.fight = fight
end


function LevelBehavior405010202:Init()
	self.logicId = "OpenWorldParkour02"
	self.bornPos = "born"
	self.role = nil
	self.currentWave = 0 --当前波次
	self.missionState = 0
	self.goldCount = 0  --拾取的道具数量
	self.posAmount = 31   --金币总数
...
function LevelBehavior405010202:Update()
	...
	if self.missionState == 0 then
		...
		BehaviorFunctions.ShowTip(405010202)
		--设置tips参数为0
		BehaviorFunctions.ChangeSubTipsDesc(1, 405010202,self.pointNum)
		BehaviorFunctions.ChangeSubTipsDesc(2, 405010202,self.goldNum)
		self.missionState = 1
		--摄像头调用
		if self.camera == false then
			self.LevelCommon:LevelCameraLookAtPos(22002,40,nil,"Collect1",0.5,0.5)
			self.camera = true
		end
	end
	...
	if self.missionState == 12 and self.time - self.creatFrame > 30 then
		--创建宝箱
		BehaviorFunctions.ChangeEcoEntityCreateState(2005001119002, true)
...
function LevelBehavior405010202:RemoveEntity(instanceId)
...
--进入区域，并激活下一个区域
function LevelBehavior405010202:EnterArea(triggerInstanceId, areaName, logicName)
...
--自定义胜利条件
function LevelBehavior405010202:CustomSuccessCondition1()
...
function LevelBehavior405010202:Remove()
