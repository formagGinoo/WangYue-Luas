LevelBehavior3011801 = BaseClass("LevelBehavior3011801",LevelBehaviorBase)
--委托任务追车
function LevelBehavior3011801.GetGenerates()
	local generates = {2040832,790014001,203020601,600080,2040831,2040811}
	return generates
end


function LevelBehavior3011801:Init()
	self.role = 0
	self.initState = 0
	self.bgmState = 0
	self.missionState = 0
...
function LevelBehavior3011801:Update()
	...
	if self.initState == 0 then
		--BehaviorFunctions.SetTrafficMode(2)  --修改驾驶模式（2）为透明可穿透模式
		...
		--创建汽车
		local Car1 = BehaviorFunctions.GetTerrainPositionP("Car1",self.levelId)
		local rotate1 = BehaviorFunctions.GetTerrainRotationP("Car1",self.levelId)
		local Car2 = BehaviorFunctions.GetTerrainPositionP("Car2",self.levelId)
		local rotate2 = BehaviorFunctions.GetTerrainPositionP("Car2",self.levelId)
		--self.Car1 = BehaviorFunctions.CreateEntityByPosition(2040831,nil,"Car1",self.logicId,self.levelId,self.levelId,nil)
		self.Car2 = BehaviorFunctions.CreateEntityByPosition(2040811,nil,"Car2",self.logicId,self.levelId,self.levelId,nil)
	...
	if self.initState == 4 then
		...
		if distance < 5 then
			...
			BehaviorFunctions.SetTaskGuideEntity(nil,self.aiCar,nil,0,false,true,-10) --给车加追踪
			

			local speedUpMotion = CarMotionFactory.CreateMaxSpeedMultMotion(1.4,2,12,7,80)
			local waitplayer = CarMotionFactory.ChangeSpeedFromDistance(0.005,1,20,180,1)
			local changeLaneMotion = CarMotionFactory.CreateObstacleMotion(6,true)
			local unStopMotion = CarMotionFactory.CreateUnStopMotion(6)
			...
			BehaviorFunctions.AddCarMotion(self.aiCar,waitplayer)  --给车加距离较远后等待玩家的功能
			
			--BehaviorFunctions.ShowTip(40000000)
			self.tips1 = BehaviorFunctions.AddLevelTips(40000000,self.levelId)
	...
	if self.missionState == 1 then
		if self.onCar == true then
			...
			self.tips2 = BehaviorFunctions.AddLevelTips(40000001,self.levelId)
			BehaviorFunctions.ChangeLevelSubTips(self.tips2,1,self.progress2 .. "%")
		...
		if self.onCar == true then --在车上
			...
			if self.dis ~= 0 and self.dis < 30 then		
				...
				if self.inArea == 1 then
					...
					self.progress1 = math.floor(timechange/15)
					--骇入表现
					...
					elseif self.progress2 >= 100 then
						...
						self.tips3 = BehaviorFunctions.AddLevelTips(40000002,self.levelId)
						...
						BehaviorFunctions.SetTaskGuideEntity(nil,self.monster2,nil,30)  --切换追踪目标为怪
						...
						self.tips4 = BehaviorFunctions.AddLevelTips(40000003,self.levelId)
						--BehaviorFunctions.ChangeSubTipsDesc(1,40000003,self.monsterNum)
						BehaviorFunctions.ChangeLevelSubTips(self.tips4,1,self.monsterNum)
...
function LevelBehavior3011801:Death(instanceId, isFormationRevive)
	if instanceId == self.monster2 then
		self.monsterNum = self.monsterNum - 1
		--BehaviorFunctions.ChangeSubTipsDesc(1,40000003,self.monsterNum)
		BehaviorFunctions.ChangeLevelSubTips(self.tips4,1,self.monsterNum)
		--BehaviorFunctions.HideTip()
		BehaviorFunctions.FinishLevel(self.levelId)--击杀敌人后游戏关卡完成
		--BehaviorFunctions.StartStoryDialog(self.dialogList[2].id) --击败敌人后掉落任务物品
...
--function LevelBehavior3011801:WorldInteractClick(uniqueId, instanceId)
...
--function LevelBehavior3011801:StoryEndEvent(dialogId)
...
--function LevelBehavior3011801:LevelFinish()
	--BehaviorFunctions.HideTip()
	--BehaviorFunctions.FinishLevel(self.levelId)
--end

--赋值
function LevelBehavior3011801:Assignment(variable,value)
	self[variable] = value
end

function LevelBehavior3011801:GetCarPath(posNameList)
	local posListCache = {}
...
--驾驶骇入
function LevelBehavior3011801:DriveHack(bindTarget, hackTarget)
		...
		if BehaviorFunctions.GetEntityTemplateId(self.partner) and BehaviorFunctions.GetEntityTemplateId(self.partner) == 600080 then --装备箴石之劣的角色是否出战
			if self.driveHack == false then
				...
				if self.fxhackDriveLine then
					...
					self.fxhackDriveLine = BehaviorFunctions.CreateEntity(6000800306,self.partner,0,0,0)
	...
	elseif self.zszlOn == 0 then
		if self.driveHack == false then
			...
			if self.fxhackDriveLine then
				...
				self.fxhackDriveLine = BehaviorFunctions.CreateEntity(6000800306,self.car,0,0,0) --需在car预设中新增一个与特效出生点匹配的挂点
				BehaviorFunctions.ClientEffectRelation(self.fxhackDriveLine,self.car,"WeaponCaseLeft",self.car,"WeaponCaseLeft",0)
			end
		end
	end
...
--使用骇入连线
function LevelBehavior3011801:DriveHackLine(target,targetDot)
	if self.zszlOn == 1 then
		--出战角色装备箴石之劣
...
--退出驾驶骇入
function LevelBehavior3011801:QuitDriveHack()
	if self.zszlOn == 1 then
