...
function AnimBaseTween:__init()
    ...
    self.ease = Tweening.Ease.Linear

    self.delay = 0

    self.loopNum = 1
    self.loopType = Tweening.LoopType.Restart
...
function AnimBaseTween:OnComplete()
    self.tween:Kill()
    self.tween = nil
    LuaTimerManager.Instance:AddTimerByNextFrame(1,0,self:ToFunc("BaseComplete"))
end
...
function AnimBaseTween:BaseCreate(animData)
    self:SetEase(animData.ease or Tweening.Ease.Linear)
    self:SetDelay(animData.delay or 0)
    self:SetLoop(animData.loopNum or 1,animData.loopType or Tweening.LoopType.Restart)
