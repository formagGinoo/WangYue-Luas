LevelBehavior200503001 = BaseClass("LevelBehavior200503001",LevelBehaviorBase)
--限时收集玩法2

function LevelBehavior200503001:__init(fight)
	self.fight = fight
end

function LevelBehavior200503001.GetGenerates()
	local generates = {8010008, 2080102, 2041201, 2041202, 2041203, 2041204, 2041205, 2080107}
	return generates
end

function LevelBehavior200503001:Init()
	...
	self.logic = 200503001
	self.positionLevel = 200503001
...
function LevelBehavior200503001:Update()
	...
	if self.missionState == 2 then
		...
		self.inareaNum = 0
		--关卡提示
		self.levelTips = BehaviorFunctions.AddLevelTips(200503001,self.levelId)
		--剩余数量提示
	...
	if self.missionState == 3 then
		...
		if self.levelState == self.levelStateEnum.Ongoing then
			...
			--自定义失败条件1：倒计时结束
			local result1 = self:FailCondition_TimeLimit(40)
			--自定义失败条件2：角色超出距离限制
			...
			--自定义成功条件1：完成5个收集物收集
			local result1 = self:SuccessCondition_Collection(3)
...
--创建实体
function LevelBehavior200503001:CreateActor(list)
...
--空实体创建
function LevelBehavior200503001:LookAtInstance()
...
--移除收集物函数
function LevelBehavior200503001:ResetLevel()
...
--黑幕移除物品
function LevelBehavior200503001:Black()
...
----重置关卡时间
--function LevelBehavior200503001:ResetTime(timeLimit)
----修改动作
...
--失败传送
function LevelBehavior200503001:FailTransfer(position,lookAtPos)
...
--关卡自动开启
function LevelBehavior200503001:autoStart()
	self.collectionCtrlState = false
	if self.collectionList then
		for k,v in ipairs(self.collectionList) do
...
--镜头调用
function LevelBehavior200503001:levelCamera(type,cameraInstanceId,target)
...
--引导管理
function LevelBehavior200503001:guide()
...
--关卡开始
function LevelBehavior200503001:LevelStart()
...
--关卡胜利判断
function LevelBehavior200503001:LevelSuccessCondition(...)
	local condition = {...}
...
--自定义关卡胜利条件1：收集物全收集
function LevelBehavior200503001:SuccessCondition_Collection(num)
	--区域内收集物数量重置
	self.inareaNum = 0
	self.collectionCtrlState = false
	if self.collectionList then
...
--关卡失败判断
function LevelBehavior200503001:LevelFailCondition(...)
	local condition = {...}
	--计算完成条件的数量
	for i,v in ipairs(condition) do
		if v == true then
...
--自定义关卡失败条件1：时间限制
function LevelBehavior200503001:FailCondition_TimeLimit(timeLimit)
...
function LevelBehavior200503001:FailCondition_Distance(pos,warndistance,faildistance,timeLimit)
	local characterPos = BehaviorFunctions.GetPositionP(self.role)
...
function LevelBehavior200503001:FailCondition_AutoReset(timeLimit)
	--重置状态收集物计数
	self.resetNum = 0
	self.collectionCtrlState = false
	for k,v in ipairs(self.collectionList) do
...
--关卡状态机
function LevelBehavior200503001:ChallengeLevelFSM()
	--关卡处于默认状态时
	if self.levelState == self.levelStateEnum.Default then
...
--赋值
function LevelBehavior200503001:Assignment(variable,value)
	self[variable] = value
end
...
--计时器结束回调
function LevelBehavior200503001:TimerCountFinish(timerId)
	--失败条件时间限制返回失败
...
--教学关闭回调
function LevelBehavior200503001:LevelInstructionComplete(tipld)
...
--开场倒计时结束回调
function LevelBehavior200503001:OnCountDownFinishEvent(levelId)
...
--提示光圈回调
function LevelBehavior200503001:EnterArea(triggerInstanceId, areaName)
	for k,v in ipairs(self.collectionList) do
...
--交互实体回调
function LevelBehavior200503001:WorldInteractClick(uniqueId,instanceId)
	--初次对话阶段按钮
...
--对话镜头及事件
function LevelBehavior200503001:StoryPassEvent(dialogId)
	--关卡加载后初次交互
...
--节奏控制
function LevelBehavior200503001:StoryEndEvent(dialogId)
	--倒计时开始节点
