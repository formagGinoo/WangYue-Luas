...
function Behavior92003:Init()
	...
	self.skillBreak = false
	
	--被暗杀动作
	self.beAssassin = 92003909
	self.backHited = 92003062
end
...
function Behavior92003:Update()
	...
	self:ChangePhase(5000)		--转阶段血量
	BehaviorFunctions.SetEntityValue(self.me,"ChangeState",self.ChangeState)
	--开放参数
	BehaviorFunctions.SetEntityValue(self.me,"beAssassin",self.beAssassin)
	BehaviorFunctions.SetEntityValue(self.me,"backHited",self.backHited)
	...
	if self.initBuff ~= true then
		...
		if not BehaviorFunctions.HasEntitySign(1,10000007) then
			...
			BehaviorFunctions.SetCameraParams(FightEnum.CameraState.ForceLocking,9200301,true)
		end
	...
	if self.skillBreak and not BehaviorFunctions.HasEntitySign(self.role,10000019) then
	...
	if self.test == 0 then
		...
		--else
			--if self.dis < 30 then
				--if self.MoveState ~= FightEnum.EntityMoveSubState.Walk and not BehaviorFunctions.HasEntitySign(self.role,62001001) then
					--BehaviorFunctions.DoSetMoveType(self.me,FightEnum.EntityMoveSubState.Walk)
				--end
			--else
				--if self.myState ~= FightEnum.EntityState.Idle and not BehaviorFunctions.HasEntitySign(self.role,62001001) then
		...
		if self.phase == 1 then
			if self:CheckSkillCanUse(1,6,2) then
				self.skillList[self:GetSkillIndex(92003003)].isAuto = true
			else	
				self.skillList[self:GetSkillIndex(92003003)].isAuto = false
			end
		end
	...
	--根据不同阶段释放不同技能
		--self.phase = 2
		if self.inFight == true then
			if not BehaviorFunctions.HasEntitySign(self.me,92003018) then
					...
					elseif self.useMove == true and self.phase == 1 and self.skillList[self:GetSkillIndex(92003003)].frame < self.time and self.dis <= 6 then
						...
						if self.ChangeState == 0 or self.ChangeState == 3 then
							...
							elseif self.phase == 2 then
								BehaviorFunctions.SetAnimatorLayer(self.me, "AdvanceLayer")
	...
	--调试模式
		...
		if BehaviorFunctions.CheckKeyDown(FightEnum.KeyEvent.Attack) then
			BehaviorFunctions.CastSkillByTarget(self.me,92003226,self.role)
		end
	end
...
function Behavior92003:Wander()
	...
	if self.ignoreWalk == false then
		...
		elseif self.battleRange == self.BattleRangeEnum.Mid then
			...
			if self:CheckSkillCanUse(self.phase,4,1) and self.confrontation ~= true then
				if self.MoveState ~= FightEnum.EntityMoveSubState.Walk then
					BehaviorFunctions.DoSetMoveType(self.me,FightEnum.EntityMoveSubState.Walk)
					self.walking = 1
				end
			...
			--self:WanderLR()
		elseif self.battleRange == self.BattleRangeEnum.Short and not self:CheckSkillCanUse(self.phase,4,1) then
...
--返回没cd但不满足距离条件的技能:阶段，距离，检查模式：1.小于；2.大于
function Behavior92003:CheckSkillCanUse(phase,distance,checkType)
	if BehaviorFunctions.CanCtrl(self.me) then
		local dir = self:ReturnDirection()
		self.skillList = self:InitSkillList(self.skillList)
		for index,skill in pairs(self.skillList) do
			if self.time >= skill.frame then
				if skill.isAuto == true and skill.phase == phase then
					if (self.skillAngle > skill.minAngle and self.skillAngle < skill.maxAngle) or skill.dir == dir then
						if (distance <= skill.maxDistance and checkType == 1)
							or (distance >= skill.minDistance and checkType == 2) then
...
function Behavior92003:AddSkillSign(instanceId,sign)
	if instanceId == self.me then
		...
		--if sign == 92003211 and self.dis < 6 then
		...
		--长矛投掷
		if sign == 92003210 then
			self.shootX,self.shootY,self.shootZ = BehaviorFunctions.GetEntityTransformPos(self.me,"Bip001 L Hand")
			local rolePos = BehaviorFunctions.GetPositionP(self.role)
			local Angle = BehaviorFunctions.GetEntityAngle(self.me,self.role)
			local targetPos = BehaviorFunctions.GetPositionOffsetBySelf(self.role,1,Angle)
			self.bullet1 = BehaviorFunctions.CreateEntity(9200331102,self.me,self.shootX,self.shootY,self.shootZ,targetPos.x,targetPos.y,targetPos.z)
		...
		if sign == 92003020 then
			...
			BehaviorFunctions.RemoveEntitySign(self.me,92003019)	--移除不进入元素破防标记
		end
...
--function Behavior92003:ChangePhase(hp)
	...
	--if BehaviorFunctions.CanCtrl(self.me) and self.phase == 1 then
		----触发timeline
		--if self.hp < hp and self.hp > 0 then
			--if  self.ChangeState == 0 then
				--BehaviorFunctions.AddEntitySign(self.me,92003019,-1,false)	--标记为转阶段不进入元素破防状态
				--BehaviorFunctions.DoMagic(self.me,self.me,92003010)	--转阶段无敌
				--BehaviorFunctions.DoMagic(self.me,self.me,92003018)	--播放timeline
				--BehaviorFunctions.DoMagic(self.me,self.me,92003020)	--timeline期间隐藏神荼模型
				--BehaviorFunctions.SetJoyMoveEnable(self.me, false)	--设置摇杆接收状态
				----BehaviorFunctions.DoMagic(self.me,self.role,92003020)	--timeline期间隐藏角色模型
				...
				--BehaviorFunctions.DoEntityAudioPlay(self.me,"ShentuMb1_Timeline1",true)
		...
		--if self.storyFrame < self.time and self.ChangeState == 1 then
			...
			--BehaviorFunctions.SetJoyMoveEnable(self.me, true)	--设置摇杆接收状态
			----BehaviorFunctions.RemoveBuff(self.me,92003019)	--移除timeline时停
			--BehaviorFunctions.RemoveBuff(self.me,92003018)
			--BehaviorFunctions.RemoveBuff(self.role,92003020)
			----BehaviorFunctions.DoSetPosition(self.me,self.bornPos.x,self.bornPos.y,self.bornPos.z)	--设置自己位置
...
function Behavior92003:ChangePhase(hp)
	if BehaviorFunctions.CanCtrl(self.me) then
		--血量小于50%转阶段,如果不处于timeline中
		if self.hp < hp and self.hp > 0 and not BehaviorFunctions.HasEntitySign(self.me,92003018) and self.ChangeState == 0 then
			BehaviorFunctions.ShowBlackCurtain(true,0.7)	--黑幕
			BehaviorFunctions.BreakSkill(self.role)	--把角色技能状态重置
			BehaviorFunctions.BreakSkill(self.me)	--打断当前技能
			BehaviorFunctions.AddBuff(self.me,self.role,92003020)	--timeline隐藏模型
			BehaviorFunctions.AddEntitySign(self.me,92003018,-1,false)	--给关卡发信号挪位置
			BehaviorFunctions.AddDelayCallByTime(0.6,self,self.PlayChangeTimeline)	--揭开黑幕后开始播timeline
		end
	end
	...
	if self.ChangeState == 2 and self.phase == 1 then
		if BehaviorFunctions.CheckEntityElementState(self.me, 2, -1) then
			BehaviorFunctions.BreakSkill(self.me)	--打断当前技能
			BehaviorFunctions.CastSkillBySelfPosition(self.me,92003225)	--释放虚弱技能
			BehaviorFunctions.RemoveEntitySign(self.me,92003019)	--移除不进入元素破防标记
			BehaviorFunctions.RemoveBuff(self.me,92003010)	--移除无敌
			self.ChangeState = 3
			self.phase = 2
		end
	end
	...
	--转阶段timeline结束
	if self.storyFrame < self.time and self.ChangeState == 1 then
		BehaviorFunctions.ShowBlackCurtain(true,0.2)	--黑幕
		--延迟揭开黑幕
		BehaviorFunctions.RemoveBuff(self.role,92003020)	--移除timeline隐藏模型
		BehaviorFunctions.AddDelayCallByTime(1,self,self.BlackCurtainEnd)
	end
end

--结束黑幕
function Behavior92003:BlackCurtainEnd()
	BehaviorFunctions.ShowBlackCurtain(false,0.6)
	BehaviorFunctions.RemoveEntitySign(self.me,92003018)	--移除转阶段标记
	BehaviorFunctions.SetJoyMoveEnable(self.role, true)	--设置摇杆可用
	BehaviorFunctions.SetFightMainNodeVisible(1,"PanelParent",true,1)--隐藏所有战斗UI
	--BehaviorFunctions.RemoveBuff(self.me,92003019)
	BehaviorFunctions.RemoveBuff(self.me,92003018)
	BehaviorFunctions.RemoveBuff(self.role,92003020)
	BehaviorFunctions.RemoveBuff(self.me,92003020)
	BehaviorFunctions.SetEntityElementStateAccumulation(self.me, self.me, -1, 0)	--重置元素值
	BehaviorFunctions.CastSkillBySelfPosition(self.me,92003019)	--释放转阶段技能
	BehaviorFunctions.DoLookAtTargetByLerp(self.role,self.me,false,180,180,-2)
	BehaviorFunctions.RemoveBuff(self.me,92003003)
...
--播放转阶段timeline
function Behavior92003:PlayChangeTimeline()

	if self.ChangeState == 0 then
		--BehaviorFunctions.CastSkillBySelfPosition(self.me,92003018)
		BehaviorFunctions.ShowBlackCurtain(false,0.6)
		BehaviorFunctions.DoMagic(self.me,self.me,92003018)	--播放timeline
		BehaviorFunctions.DoMagic(self.me,self.me,92003020)	--timeline期间隐藏神荼模型
		BehaviorFunctions.AddEntitySign(self.me,92003019,-1,false)
		BehaviorFunctions.DoEntityAudioPlay(self.me,"ShentuMb1_Timeline1",true)	--播放声音
		--BehaviorFunctions.DoMagic(self.me,self.role,92003019)	--全局时停
		BehaviorFunctions.SetJoyMoveEnable(self.role, false)
		--BehaviorFunctions.DoMagic(self.me,self.role,92003020)	--隐藏角色模型
		BehaviorFunctions.DoMagic(self.me,self.me,92003010)	--转阶段无敌
		BehaviorFunctions.SetFightMainNodeVisible(1,"PanelParent",false,1)--隐藏所有战斗UI
		BehaviorFunctions.SetJoyMoveEnable(self.me, false)	--设置摇杆接收状态
		self.ChangeState = 1
		--self.changStateStory = false	--标记为正在播timeline
		self.storyFrame = self.time + 654	--进入timeline播放时间计时
	end
...
function Behavior92003:BreakSkill(instanceId,skillId,skillSign,skillType)
	if instanceId == self.me then
		self.skillFinishFrame = self.time
		if skillId == 92003321 and BehaviorFunctions.HasEntitySign(self.role,10000019) then
			self.skillBreak = true	--标记为被跳反打断了大招
		end
	end
end

function Behavior92003:FinishSkill(instanceId,skillId,skillSign,skillType)
	if instanceId == self.me then
		self.skillFinishFrame = self.time
	end
end
...
function Behavior92003:Death(instanceId,isFormationRevive)
	if instanceId == self.me then
		...
		if self.sInstanceId then
			BehaviorFunctions.CheckCreatResDupBoosLinkEntity(self.sInstanceId)
		end
	end
end

function Behavior92003:DeathEnter(instanceId,isFormationRevive)
	if instanceId == self.me then
...
function Behavior92003:Die(attackInstanceId,dieInstanceId)
...
function Behavior92003:Collide(attackInstanceId,hitInstanceId,instanceId,shakeStrenRatio,attackType)
	...
	if attackInstanceId == self.me and hitInstanceId == self.role then
		...
		if BehaviorFunctions.GetEntityTemplateId(instanceId) == 9200320501 then
			self.useCombo = false
			self.comboCheckFrame = self.time + 60
		end
	end
...
--子弹触地回调
--function Behavior92003:HitGround(attackInstanceId,instanceId,positionX, positionY, positionZ)
	--if BehaviorFunctions.GetEntityTemplateId(instanceId) == 9200331102 then
		--BehaviorFunctions.CreateEntity()
	--end
--end


function Behavior92003:LateInit()
	self.bornPos = BehaviorFunctions.GetPositionP(self.me)
end

function Behavior92003:HitGround(attackInstanceId,instanceId,positionX, positionY, positionZ, rotationX, rotationY, rotationZ, rotationW)
	if attackInstanceId == self.me and BehaviorFunctions.GetEntityTemplateId(instanceId) == 9200331102 then
		BehaviorFunctions.CreateEntity(92003311006,self.me,positionX, positionY, positionZ,rotationX, rotationY, rotationZ, rotationW)
