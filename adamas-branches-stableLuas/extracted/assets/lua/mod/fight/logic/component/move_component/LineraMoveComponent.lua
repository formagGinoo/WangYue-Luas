...
function LineraMoveComponent:Init(moveComponent, moveConfig)
	...
	if self.entity.parent then
		self.transformComponent:SetRotation(self.entity.parent.transformComponent.rotation)
	end
...
function LineraMoveComponent:SetEnable(enable, delayFrame)
...
function LineraMoveComponent:TryEarlyWarning()
	...
	if self.warning and self.warningDurationFrame > 0 then
		...
		if self.warningDurationFrame <= 0 then
			local instancId = self.entity.parent and self.entity.parent.instanceId or self.entity.instanceId
