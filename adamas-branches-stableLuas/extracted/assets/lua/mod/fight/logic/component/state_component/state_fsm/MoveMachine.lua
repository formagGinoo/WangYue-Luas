...
function MoveMachine:Init(fight,entity)
    ...
    self.moveFSM.parentFSM = self.parentFSM
    self.moveFSM.stateId = self.stateId
end
...
function MoveMachine:OnEnter(dontStart)
	if not dontStart then
		self.moveFSM:StartMove(moveType)
	end
end
...
function MoveMachine:GetSubState()
	return self.moveFSM:GetState()
end
...
function MoveMachine:CanFall()
	return (not self.entity.tagComponent) or not self.entity.tagComponent:IsMonster()
end
