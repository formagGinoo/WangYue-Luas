...
function Behavior2010500.GetGenerates()
	local generates = {2010511,2010512,2010513,2010514,2010515}
	return generates
end
...
function Behavior2010500:LateInit()
	if BehaviorFunctions.CheckTaskStepIsFinish(1980101,2) and not BehaviorFunctions.CheckTaskStepIsFinish(1980101,3) then
		BehaviorFunctions.AddEntityGuidePointer(self.me,4,1.6)
	end
end

--物品交互
function Behavior2010500:WorldInteractClick(uniqueId, instanceId)
	if instanceId ~= self.me then
		return
	end

	if not BehaviorFunctions.CheckPartnerObtain() then
		return 
	end
	...
	if self.interactUniqueId and self.interactUniqueId == uniqueId then
		...
		--非无色玉佩
			...
			--五月版本演示流程特殊处理
			if BehaviorFunctions.CheckTaskStepIsFinish(1980101,2) and not BehaviorFunctions.CheckTaskStepIsFinish(1980101,3) then
				BehaviorFunctions.SendTaskProgress(1980101,3,1)
			end
		end
...
function Behavior2010500:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
	...
	self.interactUniqueId = BehaviorFunctions.WorldInteractActive(self.me,WorldEnum.InteractType.Jade,nil,"凝聚佩玉",1)
end
...
function Behavior2010500:CreateRemoveEffect(quality)
	for i,v in ipairs(self.removeEffect) do
		if v.quality == quality then
			...
			BehaviorFunctions.CreateEntityByEntity(self.me,v.entityId,myPos.x,myPos.y,myPos.z)
		end
	end
end