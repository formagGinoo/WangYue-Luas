Behavior2030804 = BaseClass("Behavior2030804",EntityBehaviorBase)
--蓄电装置
function Behavior2030804.GetGenerates()
	-- local generates = {}
	-- return generates
end

function Behavior2030804:Init()
...
function Behavior2030804:LateInit()
	--创建能量显示特效
	self.energyEffect1 = BehaviorFunctions.CreateEntity(203080401,self.me)
...
function Behavior2030804:Update()
	if BehaviorFunctions.GetEntityEcoId(self.me) then
		...
		-- --获取实体属性值
		-- local A = BehaviorFunctions.GetEntityAttrValueRatio(self.me,1086)
		-- print("weakpoint Rota 1086_",A)
		-- local B = BehaviorFunctions.GetEntityAttrValueRatio(self.me,86)
		-- print("weakpoint Rota 86_",B)

		-- local C = BehaviorFunctions.GetEntityAttrVal(self.me,1086)
		-- print("weakpoint 1086_",C)
		-- local D = BehaviorFunctions.GetEntityAttrVal(self.me,86)
		-- print("weakpoint 86_",D)
...
--修改能量值百分比
function Behavior2030804:ChangeEnergyPercentage(value)
	if value > 1 then
...
--增减能量百分比
function Behavior2030804:IncreaseEnergyPercentage(value)
	--修改总能量百分比
...
function Behavior2030804:FirstCollide(attackInstanceId,hitInstanceId,instanceId,attackType,skillType, atkElement)
...
function Behavior2030804:Damage(InstanceId,hitInstanceId,damageType,magicId,damageElementType,damageVal,attackType,partType,damageInfo,attackInstanceId,isCirt,camp)
	if hitInstanceId == self.me then
		if damageElementType == FightEnum.ElementType.Gold then		
			...
			if self.myState == self.myStateEnum.charging then
				if self.currentElementValue < self.maxElementValue then
					...
					--受击充能特效
					self.chargingEffect = BehaviorFunctions.CreateEntity(203080403,self.me)											
					--能量蓄满时进入激活状态
					if self.currentElementValue >= self.maxElementValue then
