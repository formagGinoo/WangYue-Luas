LevelBehavior1011 = BaseClass("LevelBehavior1011",LevelBehaviorBase)
--fight初始化
function LevelBehavior1011:__init(fight)
	self.fight = fight
end

--预加载
function LevelBehavior1011.GetGenerates()
	local generates = {9001}
	return generates
end

--参数初始化
function LevelBehavior1011:Init()
	self.role = 0       --当前角色
...
--帧事件
function LevelBehavior1011:Update()
	...
	if self.missionState == 53 and self.time - self.timeStart >=1 then
		BehaviorFunctions.ExitLevel()
		self.missionState = 999
	end

end
function LevelBehavior1011:CreatMonster(...)
