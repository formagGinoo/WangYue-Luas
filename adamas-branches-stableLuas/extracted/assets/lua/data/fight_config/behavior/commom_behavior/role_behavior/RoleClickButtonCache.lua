...
function RoleClickButtonCache:ClickButton(KeyEvent)
	...
	if ((BF.CheckKeyDown(FK.Dodge) and self.RAP.CtrlRole == self.Me) or KeyEvent == FK.Dodge)
		and BF.CheckBtnUseSkill(self.Me,FK.Dodge) and not BF.IsStaminaOverdrawn() 
		and BF.GetPlayerAttrVal(FEPA.CurStaminaValue) >= math.abs(BF.GetPlayerAttrVal(FEPA.DodgeCost)) then
		self:ClickButtonRecord(FK.Dodge)
	end
	--核心按钮点击记录、增加缓存
	if self.RAP.partner and ((BF.CheckKeyDown(FK.Partner) and self.RAP.CtrlRole == self.Me) or KeyEvent == FK.Partner)
		and BF.CheckBtnUseSkill(self.RAP.partner,FK.Partner) and BF.CheckEntity(r1) and not BF.HasEntitySign(1,10000012) then
