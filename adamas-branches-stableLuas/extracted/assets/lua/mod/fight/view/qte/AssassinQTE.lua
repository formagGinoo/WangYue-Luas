...
function AssassinQTE:__onCache()
    ...
    self.UI_SkillPanel_danji:SetActive(false)
    self.UI_AssassinQTE_wanmeiqu:SetActive(false)
    self.UI_AssassinQTE_wanmei:SetActive(false)

end

function AssassinQTE:__delete()
...
function AssassinQTE:__Show()
    ...
    self.UI_AssassinQTE_wanmei:SetActive(false)
    self.UI_AssassinQTE_putong:SetActive(false)
...
function AssassinQTE:OnActionInput(key, value)
...
function AssassinQTE:_BeforeUpdate(deltaTime)
    ...
    UtilsUI.SetActive(self.UI_SkillPanel_danji, true)
    UtilsUI.SetActive(self.UI_AssassinQTE_wanmeiqu, isInPerfect)
    if scale < 0 then
        scale = 0
    end
...
function AssassinQTE:OnExitQTE(isSuccess)
    ...
    elseif result == 1 then-- 普通
        self.SuccessTip:SetActive(true)
        self.UI_AssassinQTE_putong:SetActive(true)
    elseif result == 2 then --完美
        self.PerfectTip:SetActive(true)
        -- self.Perfect_Kill_Section:SetActive(true)
        self.UI_AssassinQTE_wanmei:SetActive(true)
    end
    ...
    if not self.hideTimer then
        self.hideTimer = LuaTimerManager.Instance:AddTimer(1, 1.3, exitFunc)
    end
end