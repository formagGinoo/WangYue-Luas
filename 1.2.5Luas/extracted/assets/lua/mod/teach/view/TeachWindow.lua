...
function TeachWindow:__BindListener()
    self:SetHideNode("TeachWindow_Eixt")
...
function TeachWindow:ClickBackBtn()
    self.TeachWindow_Eixt:SetActive(true)
end

function TeachWindow:HideWindow()
	WindowManager.Instance:CloseWindow(self)
end
function TeachWindow:ClickBack()
    if self.tabPanel.CommonLeftTab_exit then
...
function TeachWindow:InitTagData()
    ...
    --     item:ClickSelect()
    -- end
    self.tabPanel = self:OpenPanel(CommonLeftTabPanel,{ name = TI18N("教学目录"), name2 = "jiaoxue", tabList = self.TeachMainToggleInfo})
end
...
function TeachWindow:CreateVideo(videoName)
    ...
    local callback = function()
        ...
        UnityUtils.SetSizeDelata(rawRect, 634, 360)
    ...
    self.assetLoader = AssetBatchLoader.New("TeachVideo"..videoName)
