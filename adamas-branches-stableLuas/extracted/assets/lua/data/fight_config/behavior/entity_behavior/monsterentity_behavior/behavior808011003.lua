Behavior808011003 = BaseClass("Behavior808011003",EntityBehaviorBase)
--资源预加载
function Behavior808011003.GetGenerates()
	local generates = {}
	return generates
end

function Behavior808011003.GetMagics()
	local generates = {}
	return generates
end
...
function Behavior808011003:Init()
	...
	--self.MonsterCommonParam.warnSkillId = 80801100304            --警告技能Id
	--self.MonsterCommonParam.warnSitSkillId = 80801100307            --坐下后起立警告技能Id
	--self.MonsterCommonParam.tauntSkillId = 80801100305            --嘲讽技能
	...
	self.MonsterCommonParam.initialSkillList = {
		...
		{id = 80002001,
			...
			maxDistance = 2.1,        --技能释放最大距离（无等号）
			angle = 30,              --技能释放角度
			cd = 8,                  --技能cd，单位：秒
			durationFrame = 82,      --技能持续帧数
		...
		--助跑敲击
		{id = 80002002,
			minDistance = 2.1,         --技能释放最小距离（有等号）
			maxDistance = 5,        --技能释放最大距离（无等号）
			angle = 30,              --技能释放角度
			cd = 10,                  --技能cd，单位：秒
			durationFrame = 78,      --技能持续帧数
			frame = 0,               --cd用帧数参数，默认为0，大于0相当于有单独的初始cd
			priority = 1,            --优先级，数值越大优先级越高
			weight = 1,              --随机权重
			isAuto = true,           --是否自动释放
	...
	self.MonsterCommonParam.LRWalkSwitchTime = 36 * 0.033        --左右走切换时间
	...
	self.MonsterCommonParam.defaultSkillId = 80801100306		        --默认技能id，追杀模式使用
	self.MonsterCommonParam.visionAngle = 50               --视野范围，不在视野范围内会进行转向
	--MonsterExitFight
...
function Behavior808011003:LateInit()
	self.MonsterCommonBehavior.MonsterPeace:LateInit()
end


function Behavior808011003:Update()
...
function Behavior808011003:DeathEnter(instanceId,isFormationRevive)
...
function Behavior808011003:Die(attackInstanceId,dieInstanceId)
