...
function MonsterExitFight:Update()
	if self.MonsterCommonParam.canExitFight == true  and BehaviorFunctions.CanCtrl(self.MonsterCommonParam.me) then
		...
		if self.MonsterCommonParam.inFight == true then
			--获取与目标的三维坐标距离
			self.battleTargetDistance3D = BehaviorFunctions.GetDistanceFromTargetWithY(self.MonsterCommonParam.me,self.MonsterCommonParam.battleTarget)
			
			--距离过远 或 战斗目标距离大于游荡超远距离
			if (self.bornDistance >= self.MonsterCommonParam.ExitFightRange --距离出生点太远
				or (self.MonsterCommonParam.targetMaxRange > 0 and self.battleTargetDistance3D >= self.MonsterCommonParam.targetMaxRange)) --距离（3维）超过最大游荡范围
				or BehaviorFunctions.GetCurAlivePlayerEntityCount() == 0 
				or math.abs(BehaviorFunctions.GetPositionP(self.MonsterCommonParam.me).y - BehaviorFunctions.GetPositionP(self.MonsterCommonParam.battleTarget).y) >= 10 then	--高度差超过10脱战
				...
				BehaviorFunctions.EnableEntityElementStateRuning(self.MonsterCommonParam.me, FightEnum.ElementState.Accumulation, false,-1)
			end
		end
		--脱战巡回
		if self.MonsterCommonParam.exitFightState == self.MonsterCommonParam.ExitFightStateEnum.Exiting then
...
function MonsterExitFight:Reborn()
	...
	--恢复元素积累判断
	BehaviorFunctions.EnableEntityElementStateRuning(self.MonsterCommonParam.me, FightEnum.ElementState.Accumulation,true,-1)
	--清除元素积累
	BehaviorFunctions.SetEntityElementStateAccumulation(self.MonsterCommonParam.me,self.MonsterCommonParam.me,-1,0)
