...
function ClientBuff:Init(clientFight, clientEntity, info, isActivity, buffId, timeScale)
	...
	self.loadToBones = info.loadToBones or false

	self:SetActivity(isActivity)

	if (not self.dontBindRotation) or self.loadToBones then
	...
	if self.effectUtil then
		...
		self.effectUtil:SetSpeed(self.ignoreTimeScale and 1 or self.timeScale)
	end
...
function ClientBuff:AfterUpdate()
	if self.dontBindRotation then
		if not self.parent then
			return
		end

		if self.loadToBones then
			--绑定到骨骼节点 不跟随旋转
			UnityUtils.SetRotation(self.transform, 0, 0, 0, 0)
		else
			--没有绑定到骨骼节点
