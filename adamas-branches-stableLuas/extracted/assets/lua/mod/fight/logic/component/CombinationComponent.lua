...
function CombinationComponent:LateInit()
	self.transform = self.entity.clientTransformComponent:GetTransform()
end
...
function CombinationComponent:RemoveCombination()
	if self.parentEntity then
		...
		local parentGo = self.parentEntity.clientTransformComponent:GetGameObject()
		CustomUnityUtils.SetShadowRenderersEnable(parentGo, true)

		self.parentEntity = nil

		if ctx then
			self.entity.clientTransformComponent:SetRotationFollower(nil)
		end
	end
...
function CombinationComponent:SetCombinationParent(entity, dmgParent)
	...
	if ctx then
		...
		local transformParent = self.parentEntity.clientTransformComponent:GetTransform(CombinationRoot)
		...
		clientEntity.clientTransformComponent:SetRotationFollower(self.parentEntity.clientTransformComponent:GetTransform())
	end

	self:Update()
end

function CombinationComponent:PlayAnimation(name, startFrame)
	if self.childEntity and self.childEntity.animatorComponent then
		self.childEntity.animatorComponent:PlayAnimation(name, startFrame)
	end
end
