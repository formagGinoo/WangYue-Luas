...
function MercenaryTipsPanel:__BindListener()
    self.TraceBtn_btn.onClick:AddListener(self:ToFunc("OnClick_TraceBtn"))
...
function MercenaryTipsPanel:__Show()
    ...
    self.makeInsId = self.args.markInstance

    self.ecoInfo = MercenaryHuntConfig:GetMercenaryEcoConfig(self.ecoId)
    self.hunterNameList = self.hunterInfo.name_list
    self.hunterLevel = Fight.Instance.mercenaryHuntManager:GetMercenaryLv(self.ecoId)
...
function MercenaryTipsPanel:UpdateTraceBtnState()
    -- if self.makeInsId then
    --     local inTrace = mod.WorldMapCtrl:GetMarkInTrace(self.makeInsId)
    -- end
end

function MercenaryTipsPanel:OnClick_TraceBtn()
    local inTrace = mod.WorldMapCtrl:GetMarkInTrace(self.makeInsId)
    mod.WorldMapCtrl:ChangeMarkTraceState(self.makeInsId, not inTrace)
end
