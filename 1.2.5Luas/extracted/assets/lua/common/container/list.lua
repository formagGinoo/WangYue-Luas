...
function List:Push(value,param)
	if not value then return end
	local node = {value = value,param = param}
...
function List:SetIndex(key,iter)
	if iter and self.indexIters[key] then

		LogError("List禁止重复设置索引节点")

		return
	end

	self.indexIters[key] = iter
end

function List:GetIterByIndex(key)
	return self.indexIters[key]
end

function List:RemoveIndex(key)
	if key then
		self.indexIters[key] = nil
	end
end

function List:PushHead(value,param)
	if not value then return end
	local node = {value = value,param = param}
...
function List:Insert(value, iter)
	if not value then return end
