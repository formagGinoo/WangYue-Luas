...
function AnimatorMoveComponent:Init(moveComponent, moveConfig)
	...
	self.applyPivotY = true
	self.ingorePivotOffset = moveConfig.IngorePivotOffset and moveConfig.IngorePivotOffset or {}
	self.pivotPos = moveConfig.pivot and moveConfig.pivot or 0
	if ctx then
...
function AnimatorMoveComponent:ApplyAnimation()
	...
	self.applyPivotY = true
	for k, v in pairs(self.ingorePivotOffset) do
		if v == animName then
			self.applyPivotY = false
			break
		end
	end
	...
	local lastXOffset = model and model.localPosition.x or 0
	local xOffset = 0
	if not self.entity.logicMove and self.animationPositionX then
		xOffset = self.animationPositionX[frame]
	end
	
	if lastXOffset ~= xOffset then
		self.clientTransformComponent:DoAnimationXFusion(lastXOffset)
	end
	
	if self.moveConfig & moveX > 0 or self.entity.logicMove then
...
function AnimatorMoveComponent:UseAnimatorMove()
	return not self.entity.logicMove and not self.entity.stateComponent:IsState(FightEnum.EntityState.Perform) and self.enabled and self.moveComponent.animatorMoveState
end
...
function AnimatorMoveComponent:Update()
	...
	if scale == 1 and self.timeComponent:GetCurSplitFrameInfo() ~= 0 then
		local passFrame = self.timeComponent:GetCurSplitFrameInfo()
		scale = scale - passFrame
	end

	--self.time = self.time + scale * FightUtil.deltaTime
	if not self.animatorComponent or not self.animatorComponent.animationName then
		return
	end	
	...
	if self.moveConfig & moveX > 0 then
		...
		if ctx then
			if clientMoveX == 0 then
				...
				self.clientTransformComponent:DoMoveX(clientMoveX)
			end
		else
			self.clientTransformComponent:ClearMoveX()
		end
	...
	local useAnimatorMove = not self.entity.skillComponent or self.entity.skillComponent:CheckAnimationMove()
	local yUseAnimatorMove = not self.entity.skillComponent or self.entity.skillComponent:CheckAnimationMoveY()
	if self.moveConfig & moveY > 0 and self.yAxisMoveEnable and yUseAnimatorMove then
	...
	if not useAnimatorMove or not self.xzAxisMoveEnable then
...
function AnimatorMoveComponent:GetOffset()
	...
	-- TODO 没有root点的临时处理 先让表现看起来是对的
	local animationName = self.animatorComponent:GetAnimationName()
	if animationName == "Attack003" and self.entity.entityId == 1010 and frame >= 9 then
		self.offset.x = 0
		self.offset.y = 0
		self.offset.z = 0
		return self.offset, 0, 0
	...
	if frame == 0 then
		...
		local model = self.entity.clientTransformComponent.model
		local xOffset = model and model.localPosition.x or self.lastOffsetX
	...
	if self.animationPositionY then
		...
		if self.animationPivotY[frame] and self.applyPivotY then
