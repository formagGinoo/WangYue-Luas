LevelBehavior405010804 = BaseClass("LevelBehavior405010804",LevelBehaviorBase)
--烽火挑战

function LevelBehavior405010804.GetGenerates()
	local generates = {
		790014000,790012000,790013000,790008000, --小怪
		910040,910024, --精英
		2041001,--信号塔
		2030206, --道具
		2030205,2030228 --场景破坏
	}
	return generates
end


function LevelBehavior405010804:__init(fight)
	----敌人参数------------------------------------------------------------------------------------------------------------------------------------------
	...
	self.posList =
	{
		[1] = {posName = "Pos1", state = self.towerState.Open, deathCount = 0, totalNum = 0, towerId = nil, allEnemyIdList = {}, canInteract = false, guide = nil, showedTip = false},
		[2] = {posName = "Pos2", state = self.towerState.Open, deathCount = 0, totalNum = 0, towerId = nil, allEnemyIdList = {}, canInteract = false, guide = nil, showedTip = false},
		[3] = {posName = "Pos3", state = self.towerState.Open, deathCount = 0, totalNum = 0, towerId = nil, allEnemyIdList = {}, canInteract = false, guide = nil, showedTip = false},
	}

	--怪物list
	self.enemyList = {
		--Pos1敌人和道具选择
		[1] = {towerPos = "Pos1", state = self.monsterStateEnum.Default, monsterId = 790014000, posName = "_EnemyBorn1", Id = nil, tage = "Enemy"},
		[2] = {towerPos = "Pos1", state = self.monsterStateEnum.Default, monsterId = 790014000, posName = "_EnemyBorn2", Id = nil, tage = "Enemy"},
		[3] = {towerPos = "Pos1", state = self.monsterStateEnum.Default, monsterId = 790014000, posName = "_EnemyBorn3", Id = nil, tage = "Enemy"},
		[4] = {towerPos = "Pos1", state = self.monsterStateEnum.Default, monsterId = 2030206, posName = "_ItemBorn1", Id = nil, tage = "Item"},
		[5] = {towerPos = "Pos1", state = self.monsterStateEnum.Default, monsterId = 2030205, posName = "_ItemBorn2", Id = nil, tage = "Item"},
		[6] = {towerPos = "Pos1", state = self.monsterStateEnum.Default, monsterId = 2030228, posName = "_ItemBorn3", Id = nil, tage = "Item"},
		--Pos2敌人和道具选择
		[7] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 790013000, posName = "_EnemyBorn1", Id = nil, tage = "Enemy"},
		[8] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 790013000, posName = "_EnemyBorn2", Id = nil, tage = "Enemy"},
		[9] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 790012000, posName = "_EnemyBorn3", Id = nil, tage = "Enemy"},
		[10] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 2030206, posName = "_ItemBorn1", Id = nil, tage = "Item"},
		[11] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 2030205, posName = "_ItemBorn2", Id = nil, tage = "Item"},
		[12] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 2030205, posName = "_ItemBorn3", Id = nil, tage = "Item"},
		[13] = {towerPos = "Pos2", state = self.monsterStateEnum.Default, monsterId = 2030228, posName = "_ItemBorn4", Id = nil, tage = "Item"},
		--Pos3敌人和道具选择
		[14] = {towerPos = "Pos3", state = self.monsterStateEnum.Default, monsterId = 790014000, posName = "_EnemyBorn1", Id = nil, tage = "Enemy"},
		[15] = {towerPos = "Pos3", state = self.monsterStateEnum.Default, monsterId = 790012000, posName = "_EnemyBorn2", Id = nil, tage = "Enemy"},
		[16] = {towerPos = "Pos3", state = self.monsterStateEnum.Default, monsterId = 790014000, posName = "_EnemyBorn3", Id = nil, tage = "Enemy"},
	}
	...
	self.missionUnloadDis = 50 --肉鸽玩法未开始的卸载距离
	self.unloaded = false

	------追踪标--- -----------------------------------------------------------------------------------------------------
	self.guide = nil
	self.guideEntity = nil
	self.guideDistance = 500
	self.guidePos = nil
...
function LevelBehavior405010804:Init()
	self.missionState = 0
...
function LevelBehavior405010804:Update()
...
--生成敌人
function LevelBehavior405010804:enemySpawn(enemyList,PosList)
	for x,y in ipairs(PosList) do
		...
		for i,v in ipairs(enemyList) do
			...
			if y.posName == v.towerPos then
				...
				local Rotation = BehaviorFunctions.GetTerrainRotationP(FinalBornPos,self.levelId)

				if Location ~= nil then
					v.Id = BehaviorFunctions.CreateEntity(v.monsterId,nil,Location.x,Location.y,Location.z,nil,nil,nil,self.levelId)
					BehaviorFunctions.SetEntityEuler(v.Id,Rotation.x,Rotation.y,Rotation.z)
...
--生成信号塔
function LevelBehavior405010804:TowerSpawn(towerList,PosList)
	for x,y in ipairs(PosList) do
...
function LevelBehavior405010804:EnterArea(triggerInstanceId, areaName, logicName)
	...
	--LogError("TriggerInstance"..triggerInstanceId)

	for i,v in ipairs(self.enemyList) do
		if triggerInstanceId == v.Id then
			if  areaName == "towerAreaPos1" or areaName == "towerAreaPos2" or areaName == "towerAreaPos3" then
				--LogError(areaName)
				BehaviorFunctions.SetEntityAttr(v.Id,1001,0,1)
			end
		end
	end
end
...
function LevelBehavior405010804:ExitArea(triggerInstanceId, areaName, logicName)
	if areaName == "eventZone" and triggerInstanceId == self.role then
		self.leaveArea = true
		--LogError("playerExit")
	end
...
function LevelBehavior405010804:Die(attackInstanceId,dieInstanceId)
	for i,v in ipairs(self.posList) do
		for x,y in ipairs(v.allEnemyIdList) do
			if dieInstanceId == y then
				v.deathCount = v.deathCount + 1
				self:CheckTowerStateShowTip()
				self:TowerFinishedGuidePointer()
			end
		end
	end
end
...
--检查塔对应敌人的状态
function LevelBehavior405010804:CheckTowerState()
	for i,v in ipairs(self.posList) do
...
--检查塔对应敌人的状态
function LevelBehavior405010804:CheckTowerStateShowTip()
	for i,v in ipairs(self.posList) do
		if v.deathCount == v.totalNum and v.state ~= self.towerState.Close and v.showedTip == false then
			BehaviorFunctions.ShowTip(302060514)
			v.showedTip = true
		end
	end
end
...
--玩家点击后，检查哪些塔完成了
function LevelBehavior405010804:WorldInteractClick(uniqueId,instanceId)
	for i,v in ipairs(self.posList) do
...
--全队死亡判定失败
function LevelBehavior405010804:Death(instanceId,isFormationRevive)
...
--任务前置肉鸽追踪指标
function LevelBehavior405010804:RogueGuidePointer(guidePos,guideDistance,guideType)
	local playerPos = BehaviorFunctions.GetPositionP(self.role)
...
--肉鸽追踪（给信号塔添加标记）
function LevelBehavior405010804:TowerGuidePointer()
	for i,v in ipairs(self.posList) do
		--如果信号塔已经被关闭则不添加标记
...
--信号塔完成时添加指标
function LevelBehavior405010804:TowerFinishedGuidePointer()
	for i,v in ipairs(self.posList) do
		--如果信号塔已经被关闭则不添加标记
		if v.guide == nil and v.state ~= self.towerState.Close and v.deathCount == v.totalNum then
			local pos = BehaviorFunctions.GetPositionP(v.towerId)
			local target = BehaviorFunctions.CreateEntity(2001,nil,pos.x,pos.y,pos.z,nil,nil,nil,self.levelId)
			BehaviorFunctions.AddEntityGuidePointer(target,self.GuideTypeEnum.Police,0,false)
			v.guide = target
