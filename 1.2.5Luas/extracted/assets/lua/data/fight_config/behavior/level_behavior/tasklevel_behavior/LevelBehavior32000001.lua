...
function LevelBehavior32000001:Init()
	...
	{
		--进入副本后叙慕问这是什么地方
		[1] = {Id = 102050301,state = self.dialogStateEnum.NotPlaying},
		--司命告知叙慕有新技能可以用
		[2] = {Id = 102050901,state = self.dialogStateEnum.NotPlaying},
		--进入副本后，刷出第一只怪物
		[3] = {Id = 102050401,state = self.dialogStateEnum.NotPlaying},
		--司命提醒叙慕放新技能
		[4] = {Id = 102051001,state = self.dialogStateEnum.NotPlaying},
		--所有敌人死完后
		[5] = {Id = 102170101,state = self.dialogStateEnum.NotPlaying},
		--精英怪刷出来后播
		[6] = {Id = 102160101,state = self.dialogStateEnum.NotPlaying},
	}
...
function LevelBehavior32000001:Update()
	...
	--if BehaviorFunctions.CheckKeyDown(FightEnum.KeyEvent.Interaction) then
		...
		--self:WeakGuide(2104)
		--BehaviorFunctions.PlayGuide(2104,1,1)
		----BehaviorFunctions.PlayBackGroundText(1)
		----self:LevelCamChangeTargetPos(self.levelCam,"huan")
	--end
	...
	--走到第二个点停下
	elseif self.missionState == 0.1 then
	...
	elseif self.missionState == 12 then
		if self.dialogList[3].state == self.dialogStateEnum.NotPlaying then
			--介绍战斗规则
			--BehaviorFunctions.StartStoryDialog(self.dialogList[3].Id)
			BehaviorFunctions.AddDelayCallByFrame(80,BehaviorFunctions,BehaviorFunctions.StartStoryDialog,self.dialogList[3].Id)
		...
		for i,v in ipairs (monsterList) do
			if self.monsterList[v].state ~= self.monsterStateEnum.Dead then
				...
				if i == listLenth then
					...
					--BehaviorFunctions.AddDelayCallByFrame(90,self,self.Assignment,"missionState",13.5)
	...
	elseif self.missionState == 15 then
		--BehaviorFunctions.AddDelayCallByTime(1.5,self,self.Assignment,"missionState",17)
...
function LevelBehavior32000001:StoryEndEvent(dialogId)
	for i,v in ipairs(self.dialogList) do
		if dialogId == v.Id then
			if dialogId == self.dialogList[1].Id then
				if self.monsterList[1].state == self.monsterStateEnum.Default then
					--创造小怪
					self:CreatMonster(1)
					--停止行为树
					BehaviorFunctions.AddDelayCallByFrame(1,BehaviorFunctions,BehaviorFunctions.AddBuff,self.role,self.monsterList[1].Id,900000012)
			...
			--elseif dialogId == self.dialogList[2].Id then
				--BehaviorFunctions.ShowGuideImageTips(30005)
				--self.missionState = 6
			elseif dialogId == self.dialogList[4].Id then
			...
			elseif dialogId == self.dialogList[6].Id then
				self.missionState = 13.5
			elseif dialogId == self.dialogList[5].Id then
...
function LevelBehavior32000001:OnGuideImageTips(tipsId,isOpen)
	if tipsId == 20023 and isOpen == false then
		BehaviorFunctions.ResetDelayCallByFrame(self.wave2DelayCall,90)
...
--释放技能
function LevelBehavior32000001:CastSkill(instanceId,skillId,skillType)
...
function LevelBehavior32000001:WorldInteractClick(uniqueId)
	if uniqueId == self.levelFinishInteractID then
		BehaviorFunctions.WorldInteractRemove(self.levelFinishInteractID)
...
function LevelBehavior32000001:EnterTrigger(triggerInstanceId,triggerEntityId,roleInstanceId)
	if triggerInstanceId == self.finishGuide and self.InteractionSwitch == false then
		self.levelFinishInteractID = BehaviorFunctions.WorldInteractActive(WorldEnum.InteractType.Talk,nil,"退出副本",1)
