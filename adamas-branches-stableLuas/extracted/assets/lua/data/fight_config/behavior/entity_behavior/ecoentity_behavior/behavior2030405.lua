Behavior2030405 = BaseClass("Behavior2030405",EntityBehaviorBase)
--骇入解救npc

function Behavior2030405.GetGenerates()
	local generates = {8011001}
	return generates
end

function Behavior2030405:Init()
...
function Behavior2030405:Update()
	...
	if self.npc1Id then
		...
		if self.enter then
			...
			self.interactUniqueId = BehaviorFunctions.WorldInteractActive(self.me,WorldEnum.InteractType.Unlock,nil,"对话",1)
		else
			if self.isTrigger  then
...
--挑战开始
--function Behavior2030405:ChallengeStart(challangeName)
	----开始挑战
	--self.challengeStart = true
--end

----挑战结束
--function Behavior2030405:ChallengeFinish(challangeName,isSuccese)
	--if isSuccese then
		--BehaviorFunctions.ShowCommonTitle(5,challangeName,true)
	--end
--end

function Behavior2030405:WorldInteractClick(uniqueId, instanceId)
	if instanceId ~= self.me then
		...
		if self.interactUniqueId and self.interactUniqueId == uniqueId then
			...
			elseif self.missionState == 4 then
				...
				--显示宝箱
				BehaviorFunctions.AddDelayCallByTime(4,BehaviorFunctions,BehaviorFunctions.ChangeEcoEntityCreateState,2002060090,true)
				--移除交互列表
...
function Behavior2030405:Remake()
	--黑幕+传送
...
function Behavior2030405:CameraEnd()
	--看向终点镜头
...
function Behavior2030405:RemoveEntity(instancedId)
	--!!!!逻辑实体被移除后需要移除npc!!!!
