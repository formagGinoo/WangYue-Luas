FSMBehavior90009006 = BaseClass("FSMBehavior90009006",FSMBehaviorBase)
--资源预加载
...
function FSMBehavior90009006.GetGenerates()
	local generates = {}
	return generates
end

function FSMBehavior90009006.GetMagics()
...
function FSMBehavior90009006:Init()
	...
	self.MonsterCommonParam.LRWalkSwitchTime = 3.54       --左右走切换时间
...
function FSMBehavior90009006:LateInit()
	self.MonsterCommonBehavior.MonsterPeace:LateInit()
end


--动画帧事件判断
function FSMBehavior90009006:OnAnimEvent(instanceId,eventType,params,animationName)
	--左手盾牌显隐判断
...
function FSMBehavior90009006:Update()
	...
	if self.magicState == 0  then
		local magicList = self.ParentBehavior.magicList
		for i = 1, #magicList do
			if BehaviorFunctions.HasBuffKind(self.me,magicList[i]) then
				BehaviorFunctions.RemoveBuff(self.me,magicList[i])
			end
		end
		self.magicState = 1
	end
...
function FSMBehavior90009006:FirstCollide(attackInstanceId,hitInstanceId,damageType,magicId,damageElementType,damageVal)
...
function FSMBehavior90009006:DeathEnter(instanceId,isFormationRevive)
...
function FSMBehavior90009006:Die(attackInstanceId,dieInstanceId)
...
function FSMBehavior90009006:CastSkill(instanceId,skillId,skillSign,skillType)
...
function FSMBehavior90009006:BreakSkill(instanceId,skillId,skillSign,skillType)
	if instanceId == self.me then
		self.Inskill = false
		if skillId == 90009006 then
			self.inblock = false
...
function FSMBehavior90009006:FinishSkill(instanceId,skillId,skillSign,skillType)
	if instanceId == self.me then
		self.Inskill = false
		if skillId == 90009006 then
			self.inblock = false
...
--格挡5次后反击
function FSMBehavior90009006:BlocakFightBack()
	if self.blockCombo >= 5 then
		...
		elseif self.battleTargetDistance < 3 then
			if R == 1 then
...
function FSMBehavior90009006:SerchSkillList(skillid,table)
	for i = 1,#table do
...
--修改技能frame值
function FSMBehavior90009006:SetSkillFrame(skillId)
	--找到这个技能
...
--受击次数记录
function FSMBehavior90009006:HitTime()
