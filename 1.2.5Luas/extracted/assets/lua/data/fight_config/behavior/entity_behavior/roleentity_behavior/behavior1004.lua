...
--技能释放判断
function Behavior1004:CastSkill(instanceId,skillId,skillType)
	--核心技能释放判断，添加阻止打断时间
	...
	if instanceId == self.Me and not BF.HasEntitySign(1,10000009) then
		...
		if skillType == 20 then
			BF.DoMagic(self.Me,self.Me,1004081,1)
		end
		if skillType == 70 then
...
--技能打断判断
function Behavior1004:BreakSkill(instanceId,skillId,skillType)	
