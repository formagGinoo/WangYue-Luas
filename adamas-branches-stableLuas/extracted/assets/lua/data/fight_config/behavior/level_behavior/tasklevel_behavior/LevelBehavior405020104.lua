LevelBehavior405020104 = BaseClass("LevelBehavior405020104",LevelBehaviorBase)
--普通任务刷怪关

function LevelBehavior405020104:__init(fight)
    self.fight = fight
end

function LevelBehavior405020104.GetGenerates()
    local generates = {790012000,790013000,2041114,2041111,2041112,2041113,203040701,2041115,791004000,
    }
    return generates
end

function LevelBehavior405020104.GetMagics()
	local generates = {900000001}
	return generates
end

function LevelBehavior405020104:Init()
	self.role = 1
	self.missionState = 0
	self.monster ={
		[1] = 0,
	...
	self.monsterentityid3 = 791004000
	...
	self.goldCreate = 0
	self.goldlimit = 10
	self.ecoId = 2003001200004		--生态实体
	self.eco = nil
	self.ecorewardId = 3003001080008		--生态奖励实体
	self.ecoreward = nil
...
function LevelBehavior405020104:Update()
    ...
    if self.missionState == 1 then
		...
		self.monster[1] = BehaviorFunctions.CreateEntity(self.monsterentityid2,nil,self.pos.x+ 8,self.pos.y,self.pos.z+5,nil,nil,nil,self.levelId)
		...
		self.monster[2] = BehaviorFunctions.CreateEntity(self.monsterentityid1,nil,self.pos.x+5,self.pos.y,self.pos.z+8,nil,nil,nil,self.levelId)
		...
		self.monster[3] = BehaviorFunctions.CreateEntity(self.monsterentityid1,nil,self.pos.x- 5,self.pos.y,self.pos.z+8,nil,nil,nil,self.levelId)
		...
		self.monster[5] = BehaviorFunctions.CreateEntity(self.monsterentityid3,nil,self.pos.x,self.pos.y,self.pos.z+10,nil,nil,nil,self.levelId)
		...
		do
			...
			BehaviorFunctions.CameraEntityLockTarget(levelCam,self.monster[5])
			BehaviorFunctions.DoLookAtTargetImmediately(self.role,self.monster[5])
    ...
    if self.missionState == 2 then
		...
		if BehaviorFunctions.CheckEntityInArea(self.role,'JumpArea','Logic_Level405020104') then
...
function LevelBehavior405020104:Hit(attackInstanceId,hitInstanceId,hitType,camp, bulletInstanceId)
...
function LevelBehavior405020104:CastSkill(instanceId,skillId,SkillConfigSign,skillType)
...
function LevelBehavior405020104:__delete()
end

-------------------------函数-------------------------
--关卡结束函数
function LevelBehavior405020104:LevelFinish()
	BehaviorFunctions.RemoveEntity(self.FXJumpArea)
    BehaviorFunctions.RemoveLevel(self.levelId)
end

--怪物全部死亡
function LevelBehavior405020104:MonsterDeath()
...
function LevelBehavior405020104:Timer(timeLimit)
    -- 如果倒计时还没有开始，先开始倒计时  
...
--死亡动画播完
function LevelBehavior405020104:DeathEnter(instanceId)
	if self.Gameing == true then
...
--死亡事件
function LevelBehavior405020104:RemoveEntity(instanceId)
	if self.Gameing == true then
		...
		if instanceId == self.monster[2] then
			self.monster[2] = BehaviorFunctions.CreateEntity(self.monsterentityid1,nil,self.pos.x,self.pos.y,self.pos.z+ 10,nil,nil,nil,self.levelId)
			BehaviorFunctions.DoLookAtTargetImmediately(self.monster[2], self.role)
			BehaviorFunctions.SetEntityValue(self.monster[2],"haveWarn",false)   --关闭警戒
		end
